...
static int app_init_port(uint16_t portid, struct rte_mempool *mp) {
  ...
  ret = rte_eth_tx_queue_setup(portid, 0, (uint16_t)ring_conf.tx_size,
  ...
  if (ret < 0)
    rte_exit(EXIT_FAILURE, "rte_pmd_port_start: err=%d, port=%u\n", ret,
             portid);
...
static struct rte_sched_pipe_params pipe_profiles[MAX_SCHED_PIPE_PROFILES] = {
    {
        ...
        .tc_rate = {305175, 305175, 305175, 305175, 305175, 305175, 305175,
...
static struct rte_sched_port *app_init_sched_port(uint32_t portid,
  ...
  for (subport = 0; subport < port_params.n_subports_per_port; subport++) {
    ...
    if (pipe_count == 0)
      rte_exit(EXIT_FAILURE,
               "Error: invalid config, no pipes enabled for sched subport %u\n",
               subport);
  }
...
int app_init(void) {
  ...
  for (i = 0; i < nb_pfc; i++) {
    ...
    if (ring == NULL)
      qos_conf[i].rx_ring =
          rte_ring_create(ring_name, ring_conf.ring_size, socket,
    ...
    qos_conf[i].mbuf_pool = rte_pktmbuf_pool_create(
        pool_name, mp_size, burst_conf.rx_burst * 4, 0,
        RTE_MBUF_DEFAULT_BUF_SIZE, rte_eth_dev_socket_id(qos_conf[i].rx_port));
  ...
  RTE_LOG(INFO, APP,
          ...
          "NIC TX = %u\n",
          ring_conf.rx_size, mp_size, ring_conf.ring_size, ring_conf.tx_size);
