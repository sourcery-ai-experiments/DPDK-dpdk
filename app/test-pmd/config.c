...
const struct rss_type_info rss_type_table[] = {
    ...
    {"all", RTE_ETH_RSS_ETH | RTE_ETH_RSS_VLAN | RTE_ETH_RSS_IP |
                ...
                RTE_ETH_RSS_ECPRI | RTE_ETH_RSS_MPLS | RTE_ETH_RSS_L2TPV2},
    {"none", 0},
    {"ip", RTE_ETH_RSS_IP},
    {"udp", RTE_ETH_RSS_UDP},
    {"tcp", RTE_ETH_RSS_TCP},
...
static void nic_xstats_display_periodic(portid_t port_id) {
  ...
  rc = rte_eth_xstats_get_by_id(port_id, ids_supp, curr_values, ids_supp_sz);
  ...
  for (i = 0; i < ids_supp_sz; i++) {
    ...
    value_rate = diff_ns > 0 ? (double)diff_value / diff_ns * NS_PER_SEC : 0;

    printf("  %-25s%12" PRIu64 " %15" PRIu64 "\n", xstats_display[i].name,
           curr_values[i], value_rate);
  }
}
...
void nic_stats_display(portid_t port_id) {
  ...
  if (ret != 0) {
    fprintf(stderr, "%s: Error: failed to get stats (port %u): %d", __func__,
            port_id, ret);
    return;
  }
...
void nic_stats_clear(portid_t port_id) {
  ...
  if (ret != 0) {
    fprintf(stderr, "%s: Error: failed to reset stats (port %u): %s", __func__,
  ...
  if (ret != 0) {
    if (ret < 0)
      ret = -ret;
    fprintf(stderr, "%s: Error: failed to get stats (port %u): %s", __func__,
            port_id, strerror(ret));
    return;
  }
...
void nic_xstats_display(portid_t port_id) {
  ...
  if (cnt_xstats !=
      rte_eth_xstats_get_names(port_id, xstats_names, cnt_xstats)) {
...
void nic_xstats_clear(portid_t port_id) {
  ...
  if (ret != 0) {
    fprintf(stderr, "%s: Error: failed to reset xstats (port %u): %s\n",
...
static const char *get_queue_state_name(uint8_t queue_state) {
...
void rx_queue_infos_display(portid_t port_id, uint16_t queue_id) {
  ...
  printf("\n%s Infos for port %-2u, RX queue %-2u %s", info_border, port_id,
         queue_id, info_border);
  ...
  printf("\nRX drop packets: %s", (qinfo.conf.rx_drop_en != 0) ? "on" : "off");
  printf("\nRX deferred start: %s",
...
void tx_queue_infos_display(portid_t port_id, uint16_t queue_id) {
  ...
  printf("\n%s Infos for port %-2u, TX queue %-2u %s", info_border, port_id,
         queue_id, info_border);
...
static void device_infos_display_speeds(uint32_t speed_capa) {
...
static void print_dev_capabilities(uint64_t capabilities) {
  ...
  for (bit = begin; bit < end; bit++) {
    if (capabilities & single_capa)
      printf(" %s", rte_eth_dev_capability_name(single_capa));
    single_capa <<= 1;
  }
}
...
static void rss_offload_types_display(uint64_t offload_types,
  ...
  for (i = 0; i < sizeof(offload_types) * CHAR_BIT; i++) {
    ...
    if ((offload_types & rss_offload) != 0) {
      ...
      user_defined_str_len = strlen("user-defined-") + (i / 10 + 1);
...
void port_infos_display(portid_t port_id) {
  ...
  if (rte_dev_devargs(dev_info.device) &&
      rte_dev_devargs(dev_info.device)->args)
  ...
  if (port_numa[port_id] != NUMA_NO_CONFIG) {
    ...
    if (mp)
      printf("\nmemory allocation on the socket: %d", port_numa[port_id]);
  } else
  ...
  printf("Autoneg status: %s\n",
         (link.link_autoneg == RTE_ETH_LINK_AUTONEG) ? ("On") : ("Off"));
...
void port_summary_display(portid_t port_id) {
  ...
  printf("%-4d " RTE_ETHER_ADDR_PRT_FMT " %-12s %-14s %-8s %s\n", port_id,
         RTE_ETHER_ADDR_BYTES(&mac_addr), name, dev_info.driver_name,
         (link.link_status) ? ("up") : ("down"),
         rte_eth_link_speed_to_str(link.link_speed));
}
...
void port_eeprom_display(portid_t port_id) {
  ...
  if (!einfo.data) {
    fprintf(stderr, "Allocation of port %u eeprom data failed\n", port_id);
    return;
  }
...
void print_valid_ports(void) {
  ...
  RTE_ETH_FOREACH_DEV(pid) { printf(" %d", pid); }
  printf(" ]\n");
}

static int vlan_id_is_invalid(uint16_t vlan_id) {
  if (vlan_id < 4096)
    return 0;
...
static int eth_dev_validate_mtu(uint16_t port_id, uint16_t mtu) {
  struct rte_eth_dev_info dev_info;
  uint32_t overhead_len;
  uint32_t frame_size;
  int ret;
  ...
  if (mtu < dev_info.min_mtu) {
    fprintf(stderr, "MTU (%u) < device min MTU (%u) for port_id %u\n", mtu,
            dev_info.min_mtu, port_id);
    return -EINVAL;
  }
  if (mtu > dev_info.max_mtu) {
    fprintf(stderr, "MTU (%u) > device max MTU (%u) for port_id %u\n", mtu,
            dev_info.max_mtu, port_id);
    return -EINVAL;
  }
...
struct port_flow_tunnel *port_flow_locate_tunnel(uint16_t port_id,
                                                 struct rte_flow_tunnel *tun) {
  struct rte_port *port = &ports[port_id];
  struct port_flow_tunnel *flow_tunnel;
...
void port_flow_tunnel_list(portid_t port_id) {
  ...
  LIST_FOREACH(flt, &port->flow_tunnel_list, chain) {
    printf("port %u tunnel #%u type=%s", port_id, flt->id,
           port_flow_tunnel_type(&flt->tunnel));
...
void port_flow_tunnel_create(portid_t port_id, const struct tunnel_ops *ops) {
  ...
  if (!flt) {
    ...
    flt->id = LIST_EMPTY(&port->flow_tunnel_list)
                  ? 1
...
static struct port_flow *port_flow_new(const struct rte_flow_attr *attr,
  ...
  if (rte_flow_conv(RTE_FLOW_CONV_OP_RULE, &pf->rule, ret, &rule, error) >= 0)
    return pf;
  free(pf);
  return NULL;
}

static struct port_flow *port_flow_locate(struct port_flow *flows_list,
                                          uint32_t flow_id) {
...
static int port_flow_complain(struct rte_flow_error *error) {
  ...
  fprintf(stderr, "%s(): Caught PMD error type %d (%s): %s%s: %s\n", __func__,
          error->type, errstr,
          error->cause
              ? (snprintf(buf, sizeof(buf), "cause: %p, ", error->cause), buf)
              : "",
...
static void rss_types_display(uint64_t rss_types, uint16_t char_num_per_line) {
  uint16_t total_len = 0;
  uint16_t str_len;
  uint16_t i;
...
static void rss_config_display(struct rte_flow_action_rss *rss_conf) {
  uint8_t i;
...
static struct port_indirect_action *action_get_by_id(portid_t port_id,
                                                     uint32_t id) {
  struct rte_port *port;
  ...
  if (!pia)
    fprintf(stderr, "Failed to find indirect action #%u on port %u\n", id,
            port_id);
  return pia;
}

static int action_alloc(portid_t port_id, uint32_t id,
  ...
  *action = NULL;
  ...
  if (id == UINT32_MAX) {
    ...
    if (port->actions_list) {
      if (port->actions_list->id == UINT32_MAX - 1) {
        fprintf(stderr, "Highest indirect action ID is already assigned, "
                        "delete it first\n");
        return -ENOMEM;
      }
  ...
  if (!pia) {
    fprintf(stderr, "Allocation of port %u indirect action failed\n", port_id);
    return -ENOMEM;
  }
  ppia = &port->actions_list;
...
static int template_alloc(uint32_t id, struct port_template **template,
...
static int table_alloc(uint32_t id, struct port_table **table,
...
static struct port_table *port_table_locate(struct port_table *tables_list,
                                            uint32_t table_id) {
...
int port_flow_get_info(portid_t port_id) {
  ...
  printf("Flow engine resources on port %u:\n"
         ...
         port_id, port_info.max_nb_queues, queue_info.max_size,
         port_info.max_nb_counters, port_info.max_nb_aging_objects,
...
static int action_handle_create(portid_t port_id,
  ...
  pia->handle = rte_flow_action_handle_create(port_id, conf, action, error);
  return pia->handle ? 0 : -1;
}

static int
action_list_handle_create(portid_t port_id, struct port_indirect_action *pia,
                          const struct rte_flow_indir_action_conf *conf,
                          const struct rte_flow_action *actions,
                          struct rte_flow_error *error) {
  pia->type = RTE_FLOW_ACTION_TYPE_INDIRECT_LIST;
  pia->list_handle =
      rte_flow_action_list_handle_create(port_id, conf, actions, error);
...
int port_action_handle_destroy(portid_t port_id, uint32_t n,
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (pia->handle) {
        ret =
            pia->type == RTE_FLOW_ACTION_TYPE_INDIRECT_LIST
                ? rte_flow_action_list_handle_destroy(port_id, pia->list_handle,
                                                      &error)
                : rte_flow_action_handle_destroy(port_id, pia->handle, &error);
        if (ret) {
...
int port_action_handle_update(portid_t port_id, uint32_t id,
  ...
  if (rte_flow_action_handle_update(port_id, action_handle, update, &error)) {
    return port_flow_complain(&error);
  }
...
port_action_handle_query_dump(portid_t port_id,
  ...
  case RTE_FLOW_ACTION_TYPE_AGE:
    printf("Indirect AGE action:\n"
           ...
           query->age.aged, query->age.sec_since_last_hit_valid,
  ...
  case RTE_FLOW_ACTION_TYPE_COUNT:
    printf("Indirect COUNT action:\n"
           ...
           " hits: %" PRIu64 "\n"
           " bytes: %" PRIu64 "\n",
           query->count.hits_set, query->count.bytes_set, query->count.hits,
           query->count.bytes);
    break;
  case RTE_FLOW_ACTION_TYPE_CONNTRACK:
    printf("Conntrack Context:\n"
           ...
           query->ct.peer_port,
           query->ct.is_original_dir ? "Original" : "Reply", query->ct.enable,
           query->ct.live_connection, query->ct.selective_ack,
           query->ct.challenge_ack_passed,
           query->ct.last_direction ? "Original" : "Reply",
           query->ct.liberal_mode, query->ct.state, query->ct.max_ack_window,
           query->ct.retransmission_limit, query->ct.last_index,
           query->ct.last_seq, query->ct.last_ack, query->ct.last_window,
           query->ct.last_end);
    printf("  Original Dir:\n"
           "    scale: %u, fin: %u, ack seen: %u\n"
           " unacked data: %u\n    Sent end: %u,"
           "    Reply end: %u, Max win: %u, Max ACK: %u\n",
           query->ct.original_dir.scale, query->ct.original_dir.close_initiated,
           query->ct.original_dir.last_ack_seen,
           query->ct.original_dir.data_unacked, query->ct.original_dir.sent_end,
           query->ct.original_dir.reply_end, query->ct.original_dir.max_win,
           query->ct.original_dir.max_ack);
    printf("  Reply Dir:\n"
           "    scale: %u, fin: %u, ack seen: %u\n"
           " unacked data: %u\n    Sent end: %u,"
           "    Reply end: %u, Max win: %u, Max ACK: %u\n",
           query->ct.reply_dir.scale, query->ct.reply_dir.close_initiated,
           query->ct.reply_dir.last_ack_seen, query->ct.reply_dir.data_unacked,
           query->ct.reply_dir.sent_end, query->ct.reply_dir.reply_end,
           query->ct.reply_dir.max_win, query->ct.reply_dir.max_ack);
    break;
...
void port_action_handle_query_update(portid_t port_id, uint32_t id,
...
port_flow_tunnel_offload_cmd_prep(portid_t port_id,
  ...
  if (tunnel_ops->actions) {
    ...
    ret = rte_flow_tunnel_decap_set(port_id, &pft->tunnel, &pft->pmd_actions,
    ...
    for (aptr = actions, num_actions = 1;
         aptr->type != RTE_FLOW_ACTION_TYPE_END; aptr++, num_actions++)
      ;
    pft->actions =
        malloc((num_actions + pft->num_pmd_actions) * sizeof(actions[0]));
    if (!pft->actions) {
      rte_flow_tunnel_action_decap_release(port_id, pft->actions,
                                           pft->num_pmd_actions, &error);
      return NULL;
    }
  ...
  if (tunnel_ops->items) {
    ...
    ret = rte_flow_tunnel_match(port_id, &pft->tunnel, &pft->pmd_items,
                                &pft->num_pmd_items, &error);
    if (ret) {
      port_flow_complain(&error);
      return NULL;
    }
    for (iptr = pattern, num_items = 1; iptr->type != RTE_FLOW_ITEM_TYPE_END;
         iptr++, num_items++)
      ;
    pft->items = malloc((num_items + pft->num_pmd_items) * sizeof(pattern[0]));
    if (!pft->items) {
      rte_flow_tunnel_item_release(port_id, pft->pmd_items, pft->num_pmd_items,
                                   &error);
      return NULL;
    }
...
port_flow_tunnel_offload_cmd_release(portid_t port_id,
  ...
  if (tunnel_ops->actions) {
    free(pft->actions);
    rte_flow_tunnel_action_decap_release(port_id, pft->pmd_actions,
                                         pft->num_pmd_actions, &error);
    pft->actions = NULL;
...
/** Validate flow rule. */
int port_flow_validate(portid_t port_id, const struct rte_flow_attr *attr,
                       const struct rte_flow_item *pattern,
                       const struct rte_flow_action *actions,
                       const struct tunnel_ops *tunnel_ops) {
  struct rte_flow_error error;
  ...
  if (tunnel_ops->enabled) {
    pft = port_flow_tunnel_offload_cmd_prep(port_id, pattern, actions,
                                            tunnel_ops);
    if (!pft)
      return -ENOENT;
    if (pft->items)
...
int port_flow_pattern_template_create(
    portid_t port_id, uint32_t id,
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (pit->template.pattern_template &&
          rte_flow_pattern_template_destroy(
              port_id, pit->template.pattern_template, &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
      *tmp = pit->next;
...
int port_flow_actions_template_create(
    portid_t port_id, uint32_t id,
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (pat->template.actions_template &&
          rte_flow_actions_template_destroy(
              port_id, pat->template.actions_template, &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
      *tmp = pat->next;
...
/** Create table */
int port_flow_template_table_create(
    portid_t port_id, uint32_t id,
  ...
  for (i = 0; i < nb_actions_templates; ++i) {
    ...
    while (temp) {
      if (actions_templates[i] == temp->id) {
        flow_actions_templates[i] = temp->template.actions_template;
        found = true;
        break;
      }
      temp = temp->next;
  ...
  /* Poisoning to make sure PMDs update it in case of error. */
  memset(&error, 0x22, sizeof(error));
  pt->table = rte_flow_template_table_create(
      port_id, table_attr, flow_pattern_templates, nb_pattern_templates,
...
/** Destroy table */
int port_flow_template_table_destroy(portid_t port_id, uint32_t n,
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (pt->table &&
          rte_flow_template_table_destroy(port_id, pt->table, &error)) {
        ret = port_flow_complain(&error);
        continue;
      }
      *tmp = pt->next;
...
int port_flow_template_table_resize_complete(portid_t port_id,
                                             uint32_t table_id) {
  struct rte_port *port;
  struct port_table *pt;
  struct rte_flow_error error = {
  ...
  if (!pt)
    return -EINVAL;
  ret = rte_flow_template_table_resize_complete(port_id, pt->table, &error);
  return !ret ? 0 : port_flow_complain(&error);
}

int port_flow_template_table_resize(portid_t port_id, uint32_t table_id,
                                    uint32_t flows_num) {
  struct rte_port *port;
  struct port_table *pt;
  struct rte_flow_error error = {
...
int port_queue_flow_create(portid_t port_id, queueid_t queue_id, bool postpone,
                           uint32_t table_id, uint32_t rule_idx,
  ...
  if (rule_idx == UINT32_MAX)
    flow =
        rte_flow_async_create(port_id, queue_id, &op_attr, pt->table, pattern,
                              pattern_idx, actions, actions_idx, job, &error);
  else
    flow = rte_flow_async_create_by_index(port_id, queue_id, &op_attr,
                                          pt->table, rule_idx, actions,
                                          actions_idx, job, &error);
  if (!flow) {
    free(pf);
    free(job);
    return port_flow_complain(&error);
...
int port_queue_flow_update_resized(portid_t port_id, queueid_t queue_id,
  ...
  job->pf = pf;
  ret = rte_flow_async_update_resized(port_id, queue_id, &op_attr, pf->flow,
                                      job, &error);
  if (ret) {
    free(job);
    return port_flow_complain(&error);
...
int port_queue_flow_destroy(portid_t port_id, queueid_t queue_id, bool postpone,
                            uint32_t n, const uint64_t *rule) {
  struct rte_flow_op_attr op_attr = {.postpone = postpone};
  struct rte_port *port;
  ...
  while (*tmp) {
    ...
    for (i = 0; i != n; ++i) {
      ...
      if (rte_flow_async_destroy(port_id, queue_id, &op_attr, pf->flow, job,
                                 &error)) {
        free(job);
        ret = port_flow_complain(&error);
        continue;
...
static void queue_action_handle_create(
    portid_t port_id, uint32_t queue_id, struct port_indirect_action *pia,
    struct queue_job *job, const struct rte_flow_op_attr *attr,
    const struct rte_flow_indir_action_conf *conf,
  ...
  /* Poisoning to make sure PMDs update it in case of error. */
  pia->handle = rte_flow_async_action_handle_create(port_id, queue_id, attr,
                                                    conf, action, job, error);
  pia->type = action->type;
}

static void queue_action_list_handle_create(
    portid_t port_id, uint32_t queue_id, struct port_indirect_action *pia,
    struct queue_job *job, const struct rte_flow_op_attr *attr,
    const struct rte_flow_indir_action_conf *conf,
    const struct rte_flow_action *action, struct rte_flow_error *error) {
  ...
  pia->list_handle = rte_flow_async_action_list_handle_create(
      port_id, queue_id, attr, conf, action, job, error);
}

/** Enqueue update flow rule operation. */
int port_queue_flow_update(portid_t port_id, queueid_t queue_id, bool postpone,
                           uint32_t rule_idx, uint32_t actions_idx,
                           const struct rte_flow_action *actions) {
  struct rte_flow_op_attr op_attr = {.postpone = postpone};
  struct rte_port *port;
...
int port_queue_action_handle_create(
    portid_t port_id, uint32_t queue_id, bool postpone, uint32_t id,
    bool indirect_list, const struct rte_flow_indir_action_conf *conf,
    const struct rte_flow_action *action) {
  ...
  if (indirect_list)
    queue_action_list_handle_create(port_id, queue_id, pia, job, &attr, conf,
                                    action, &error);
  else
    queue_action_handle_create(port_id, queue_id, pia, job, &attr, conf, action,
                               &error);

  if (!pia->handle) {
    uint32_t destroy_id = pia->id;
    port_queue_action_handle_destroy(port_id, queue_id, postpone, 1,
                                     &destroy_id);
    free(job);
    return port_flow_complain(&error);
  }
...
int port_queue_action_handle_destroy(portid_t port_id, uint32_t queue_id,
                                     bool postpone, uint32_t n,
                                     const uint32_t *actions) {
  const struct rte_flow_op_attr attr = {.postpone = postpone};
  struct rte_port *port;
  struct port_indirect_action **tmp;
    ...
    for (i = 0; i != n; ++i) {
      ...
      ret = pia->type == RTE_FLOW_ACTION_TYPE_INDIRECT_LIST
                ? rte_flow_async_action_list_handle_destroy(
                      port_id, queue_id, &attr, pia->list_handle, job, &error)
                : rte_flow_async_action_handle_destroy(
                      port_id, queue_id, &attr, pia->handle, job, &error);
      if (ret) {
        free(job);
        ret = port_flow_complain(&error);
        continue;
...
void port_queue_action_handle_query_update(
    portid_t port_id, uint32_t queue_id, bool postpone, uint32_t id,
    enum rte_flow_query_update_mode qu_mode,
    const struct rte_flow_action *action) {
  int ret;
  ...
  ret = rte_flow_async_action_handle_query_update(
      port_id, queue_id, &attr, pia->handle, action, &job->query, qu_mode, job,
      &error);
  if (ret) {
    port_flow_complain(&error);
    free(job);
...
int port_queue_action_handle_query(portid_t port_id, uint32_t queue_id,
  ...
  if (rte_flow_async_action_handle_query(
          port_id, queue_id, &attr, action_handle, &job->query, job, &error)) {
    free(job);
    return port_flow_complain(&error);
  }
  printf("Indirect action #%u update queued\n", id);
...
static int port_queue_aged_flow_destroy(portid_t port_id, queueid_t queue_id,
  ...
  while (nb_flows > 0) {
    ...
    while (success < nb_flows) {
      ret = rte_flow_pull(port_id, queue_id, res, port->queue_sz, &error);
      if (ret < 0) {
...
void port_queue_flow_aged(portid_t port_id, uint32_t queue_id,
                          uint8_t destroy) {
  void **contexts;
  ...
  nb_context =
      rte_flow_get_q_aged_flows(port_id, queue_id, contexts, total, &error);
  if (nb_context > total) {
    printf("Port %u queue %u get aged flows count(%d) > total(%d)\n", port_id,
           queue_id, nb_context, total);
    free(contexts);
    return;
  }
  if (destroy) {
  ...
  for (idx = 0; idx < nb_context; idx++) {
    ...
    case ACTION_AGE_CONTEXT_TYPE_FLOW:
      ...
      printf("%-20s\t%" PRIu64 "\t%" PRIu32 "\t%" PRIu32 "\t%c%c%c\t\n", "Flow",
             ctx.pf->id, ctx.pf->rule.attr->group, ctx.pf->rule.attr->priority,
...
int port_queue_flow_pull(portid_t port_id, queueid_t queue_id) {
  ...
  for (i = 0; i < ret; i++) {
    ...
    if (job->type == QUEUE_JOB_TYPE_FLOW_DESTROY ||
      ...
      port_action_handle_query_dump(port_id, job->pia, &job->query);
    free(job);
  }
  printf("Queue #%u pulled %u operations (%u failed, %u succeeded)\n", queue_id,
         ret, ret - success, success);
  free(res);
  return ret;
}

/* Set group miss actions */
int port_queue_group_set_miss_actions(portid_t port_id,
  ...
  memset(&error, 0x66, sizeof(error));
  ret = rte_flow_group_set_miss_actions(port_id, attr->group, &gattr, actions,
                                        &error);
...
/** Create flow rule. */
int port_flow_create(portid_t port_id, const struct rte_flow_attr *attr,
                     const struct rte_flow_item *pattern,
                     const struct rte_flow_action *actions,
                     const struct tunnel_ops *tunnel_ops, uintptr_t user_id) {
  struct rte_flow *flow;
  struct rte_port *port;
  struct port_flow *pf;
  uint32_t id = 0;
  ...
  if (port->flow_list) {
    if (port->flow_list->id == UINT32_MAX) {
      fprintf(stderr, "Highest rule ID is already assigned, delete it first");
      return -ENOMEM;
    }
    id = port->flow_list->id + 1;
  }
...
int port_flow_update(portid_t port_id, uint32_t rule_id,
  ...
  while (*flow_list) {
     ...
     */
    memset(&error, 0x33, sizeof(error));
    if (rte_flow_actions_update(port_id, flow->flow, actions, &error))
      return port_flow_complain(&error);
    if (is_user_id)
...
/** Dump flow rules. */
int port_flow_dump(portid_t port_id, bool dump_all, uint64_t rule_id,
  ...
  if (!dump_all) {
    ...
    while (pflow) {
      if (rule_id != (is_user_id ? pflow->user_id : pflow->id)) {
        pflow = pflow->next;
      } else {
        tmpFlow = pflow->flow;
        if (tmpFlow)
...
int port_flow_query(portid_t port_id, uint64_t rule,
  ...
  ret = rte_flow_conv(RTE_FLOW_CONV_OP_ACTION_NAME_PTR, &name, sizeof(name),
  ...
  case RTE_FLOW_ACTION_TYPE_AGE:
    printf("%s:\n"
           ...
           " sec_since_last_hit_valid: %u\n"
           " sec_since_last_hit: %" PRIu32 "\n",
           name, query.age.aged, query.age.sec_since_last_hit_valid,
           query.age.sec_since_last_hit);
    break;
  default:
    fprintf(stderr, "Cannot display result for action type %d (%s)\n",
            action->type, name);
    break;
  }
  return 0;
  ...
  for (idx = 0; idx < nb_context; idx++) {
    ...
    case ACTION_AGE_CONTEXT_TYPE_FLOW:
      ...
      printf("%-20s\t%" PRIu64 "\t%" PRIu32 "\t%" PRIu32 "\t%c%c%c\t\n", "Flow",
             ...
             ctx.pf->rule.attr->egress ? 'e' : '-',
             ctx.pf->rule.attr->transfer ? 't' : '-');
      if (destroy && !port_flow_destroy(port_id, 1, &ctx.pf->id, false))
        total++;
      break;
    case ACTION_AGE_CONTEXT_TYPE_INDIRECT_ACTION:
      ctx.pia = container_of(type, struct port_indirect_action, age_type);
...
void port_flow_list(portid_t port_id, uint32_t n, const uint32_t *group) {
  ...
  for (pf = list; pf != NULL; pf = pf->tmp) {
    ...
    printf("%" PRIu64 "\t%" PRIu32 "\t%" PRIu32 "\t%c%c%c\t", pf->id,
           pf->rule.attr->group, pf->rule.attr->priority,
    ...
    while (item->type != RTE_FLOW_ITEM_TYPE_END) {
      if ((uint32_t)item->type > INT_MAX)
        name = "PMD_INTERNAL";
      else if (rte_flow_conv(RTE_FLOW_CONV_OP_ITEM_NAME_PTR, &name,
                             sizeof(name), (void *)(uintptr_t)item->type,
                             NULL) <= 0)
        name = "[UNKNOWN]";
    ...
    while (action->type != RTE_FLOW_ACTION_TYPE_END) {
      if ((uint32_t)action->type > INT_MAX)
        name = "PMD_INTERNAL";
      else if (rte_flow_conv(RTE_FLOW_CONV_OP_ACTION_NAME_PTR, &name,
                             sizeof(name), (void *)(uintptr_t)action->type,
                             NULL) <= 0)
        name = "[UNKNOWN]";
...
static int get_rx_ring_size(portid_t port_id, queueid_t rxq_id,
                            uint16_t *ring_size) {
  struct rte_port *port = &ports[port_id];
...
static int get_tx_ring_size(portid_t port_id, queueid_t txq_id,
                            uint16_t *ring_size) {
  struct rte_port *port = &ports[port_id];
...
static int rx_desc_id_is_invalid(portid_t port_id, queueid_t rxq_id,
                                 uint16_t rxdesc_id) {
  uint16_t ring_size;
  int ret;
...
static int tx_desc_id_is_invalid(portid_t port_id, queueid_t txq_id,
                                 uint16_t txdesc_id) {
  uint16_t ring_size;
  int ret;
...
static void ring_rxd_display_dword(union igb_ring_dword dword) {
  printf("    0x%08X - 0x%08X\n", (unsigned)dword.words.lo,
         (unsigned)dword.words.hi);
}

static void ring_rx_descriptor_display(const struct rte_memzone *ring_mz,
...
void rxtx_config_display(void) {
  ...
  printf("  %s%s%s packet forwarding%s packets/burst=%d\n",
         cur_fwd_eng->fwd_mode_name, cur_fwd_eng->status ? "-" : "",
         cur_fwd_eng->status ? cur_fwd_eng->status : "",
         retry_enabled == 0 ? "" : " with retry", nb_pkt_per_burst);

  if (cur_fwd_eng == &tx_only_engine || cur_fwd_eng == &flow_gen_engine)
    printf("  packet len=%u - nb packet segments=%d\n", (unsigned)tx_pkt_length,
           (int)tx_pkt_nb_segs);
  ...
  RTE_ETH_FOREACH_DEV(pid) {
    ...
    for (qid = 0; qid < 1; qid++) {
      ...
      if (rc) {
        ...
        rx_free_thresh_tmp = rx_conf[qid].rx_free_thresh;
        ...
        rx_free_thresh_tmp = rx_qinfo.conf.rx_free_thresh;
      ...
      if (rx_conf->share_group > 0)
        printf(" share_group=%u share_qid=%u", rx_conf->share_group,
               rx_conf->share_qid);
      printf("\n");
    }
    ...
    for (qid = 0; qid < 1; qid++) {
      ...
      if (rc) {
        ...
        tx_free_thresh_tmp = tx_conf[qid].tx_free_thresh;
        ...
        tx_free_thresh_tmp = tx_qinfo.conf.tx_free_thresh;
...
void port_rss_reta_info(portid_t port_id,
  ...
  if (ret != 0) {
    fprintf(stderr, "Failed to get RSS RETA info, return code = %d\n", ret);
    return;
  }
 ...
 */
static bool fwd_stream_on_other_lcores(uint16_t domain_id, lcoreid_t src_lc,
  ...
  for (lc_id = src_lc + 1; lc_id < nb_fc; lc_id++) {
    ...
    for (; sm_id < fwd_lcores[lc_id]->stream_idx + nb_fs_per_lcore; sm_id++) {
      fs = fwd_streams[sm_id];
      port = &ports[fs->rx_port];
      dev_info = &port->dev_info;
      rxq_conf = &port->rxq[fs->rx_queue].conf;
      if ((dev_info->dev_capa & RTE_ETH_DEV_CAPA_RXQ_SHARE) == 0 ||
          rxq_conf->share_group == 0)
        /* Not shared rxq. */
        continue;
      ...
      if (rxq_conf->share_qid != share_rxq)
        continue;
      printf("Shared Rx queue group %u queue %hu can't be scheduled on "
             "different cores:\n",
...
bool pkt_fwd_shared_rxq_check(void) {
  ...
  for (lc_id = 0; lc_id < nb_fc; lc_id++) {
    ...
    for (; sm_id < fwd_lcores[lc_id]->stream_idx + nb_fs_per_lcore; sm_id++) {
      ...
      if (fwd_stream_on_other_lcores(domain_id, lc_id, fs->rx_port,
                                     fs->rx_queue, rxq_conf->share_group,
                                     rxq_conf->share_qid))
        return false;
    }
  }
 ...
 */
static void setup_fwd_config_of_each_lcore(struct fwd_config *cfg) {
  streamid_t nb_fs_per_lcore;
  streamid_t nb_fs;
  streamid_t sm_id;
  lcoreid_t nb_extra;
...
static portid_t fwd_topology_tx_port_get(portid_t rxp) {
...
static void simple_fwd_config_setup(void) {
  portid_t i;
  ...
  for (i = 0; i < cur_fwd_config.nb_fwd_ports; i++) {
    ...
    fwd_streams[i]->tx_port = fwd_ports_ids[fwd_topology_tx_port_get(i)];
 ...
 */
static void rss_fwd_config_setup(void) {
  portid_t rxp;
  portid_t txp;
  queueid_t rxq;
  queueid_t nb_q;
  ...
  if (cur_fwd_config.nb_fwd_streams < cur_fwd_config.nb_fwd_lcores)
    cur_fwd_config.nb_fwd_lcores = (lcoreid_t)cur_fwd_config.nb_fwd_streams;
  ...
  if (proc_id > 0 && nb_q % num_procs != 0)
    printf("Warning! queue numbers should be multiple of processes, or packet "
           "loss will happen.\n");
 ...
 * the same core
 */
static void dcb_fwd_config_setup(void) {
  ...
  for (pid = 0; pid < nb_fwd_ports; pid++) {
    ...
    port = &ports[pid];
    ret = rte_eth_dev_configure(pid, nb_rxq, nb_txq, &port->dev_conf);
    if (ret < 0) {
      fprintf(stderr, "Failed to re-configure port %d, ret = %d.\n", pid, ret);
      return;
    }
  }
  ...
  for (lc_id = 0; lc_id < cur_fwd_config.nb_fwd_lcores; lc_id++) {
    ...
    for (i = 0; i < RTE_ETH_MAX_VMDQ_POOL; i++) {
      ...
      if (rxp_dcb_info.tc_queue.tc_rxq[i][tc].nb_queue == 0)
        break;
      k = fwd_lcores[lc_id]->stream_nb + fwd_lcores[lc_id]->stream_idx;
...
static void icmp_echo_config_setup(void) {
  portid_t rxp;
  queueid_t rxq;
  lcoreid_t lc_id;
  uint16_t sm_id;
  ...
  if (verbose_level > 0) {
    printf("%s fwd_cores=%d fwd_ports=%d fwd_streams=%d\n", __FUNCTION__,
           cur_fwd_config.nb_fwd_lcores, cur_fwd_config.nb_fwd_ports,
           cur_fwd_config.nb_fwd_streams);
  }
  ...
  if ((nb_rxq > 1) && (nb_txq > 1)) {
    if (dcb_config) {
      for (i = 0; i < nb_fwd_ports; i++) {
        ...
        port = &ports[pt_id];
        if (!port->dcb_flag) {
          fprintf(stderr, "In DCB mode, all forwarding ports must be "
...
static const char *mp_alloc_to_str(uint8_t mode) {
  switch (mode) {
...
void pkt_fwd_config_display(struct fwd_config *cfg) {
  ...
  printf("%s%s%s packet forwarding%s - ports=%d - cores=%d - streams=%d - "
         ...
         cfg->fwd_eng->fwd_mode_name, cfg->fwd_eng->status ? "-" : "",
         cfg->fwd_eng->status ? cfg->fwd_eng->status : "",
         retry_enabled == 0 ? "" : " with retry", cfg->nb_fwd_ports,
         cfg->nb_fwd_lcores, cfg->nb_fwd_streams,
  ...
  for (lc_id = 0; lc_id < cfg->nb_fwd_lcores; lc_id++) {
    ...
    for (sm_id = 0; sm_id < fwd_lcores[lc_id]->stream_nb; sm_id++) {
      ...
      printf("\n  RX P=%d/Q=%d (socket %u) -> TX "
             "P=%d/Q=%d (socket %u) ",
             fs->rx_port, fs->rx_queue, ports[fs->rx_port].socket_id,
             fs->tx_port, fs->tx_queue, ports[fs->tx_port].socket_id);
      print_ethaddr("peer=", &peer_eth_addrs[fs->peer_addr]);
    }
    printf("\n");
  }
  printf("\n");
...
again:
  for (i = 0; i < nb_lc; i++) {
    ...
    if (lcore_cpuid == rte_get_main_lcore()) {
      fprintf(
          stderr,
          "lcore %u cannot be masked on for running packet forwarding, which "
          "is the main lcore and reserved for command line parsing only\n",
          lcore_cpuid);
      return -1;
    }
    if (record_now)
 ...
 */
static unsigned int parse_port_list(const char *list, unsigned int *values,
...
static const char *tx_split_get_name(enum tx_pkt_split split) {
  uint32_t i;
...
void show_fec_capability(unsigned int num,
  ...
  for (i = 0; i < num; i++) {
    ...
    for (j = 0; j < RTE_DIM(fec_mode_name); j++) {
      if (RTE_ETH_FEC_MODE_TO_CAPA(j) & speed_fec_capa[i].capa)
...
void set_rx_pkt_hdrs(unsigned int *seg_hdrs, unsigned int nb_segs) {
  unsigned int i;
...
static bool nb_segs_is_invalid(unsigned int nb_segs) {
  uint16_t ring_size;
  uint16_t queue_id;
  uint16_t port_id;
  int ret;
...
void set_tx_pkt_segments(unsigned int *seg_lengths, unsigned int nb_segs) {
  ...
  if (nb_segs > 1 && nb_segs_is_invalid(nb_segs)) {
    fprintf(stderr, "Tx segment size(%u) is not supported - txpkts ignored\n",
            nb_segs);
    return;
  }
  ...
  for (i = 0; i < nb_segs; i++) {
    if (seg_lengths[i] > mbuf_data_size[0]) {
      fprintf(stderr, "length[%u]=%u > mbuf_data_size=%u - give up\n", i,
              seg_lengths[i], mbuf_data_size[0]);
      return;
    }
  ...
  if (tx_pkt_len < (sizeof(struct rte_ether_hdr) + 20 + 8)) {
    fprintf(stderr, "total packet length=%u < %d - give up\n",
            (unsigned)tx_pkt_len, (int)(sizeof(struct rte_ether_hdr) + 20 + 8));
    return;
  }
...
void setup_gro(const char *onoff, portid_t port_id) {
  ...
  if (strcmp(onoff, "on") == 0) {
    if (gro_ports[port_id].enable != 0) {
      fprintf(stderr, "Port %u has enabled GRO. Please disable GRO first\n",
              port_id);
      return;
    }
    if (gro_flush_cycles == GRO_DEFAULT_FLUSH_CYCLES) {
      gro_ports[port_id].param.gro_types = RTE_GRO_TCP_IPV4;
      gro_ports[port_id].param.max_flow_num = GRO_DEFAULT_FLOW_NUM;
...
void setup_gro_flush_cycles(uint8_t cycles) {
  if (test_done == 0) {
    fprintf(stderr, "Before change flush interval for GRO, please stop "
                    "forwarding first.\n");
    return;
  }

  if (cycles > GRO_MAX_FLUSH_CYCLES || cycles < GRO_DEFAULT_FLUSH_CYCLES) {
    fprintf(stderr,
            "The flushing cycle be in the range of 1 to %u. Revert to the "
            "default value %u.\n",
...
void show_gro(portid_t port_id) {
  ...
  if (gro_ports[port_id].enable) {
    ...
    if (gro_flush_cycles == GRO_DEFAULT_FLUSH_CYCLES) {
      max_pkts_num = param->max_flow_num * param->max_item_per_flow;
    } else
...
char *list_pkt_forwarding_modes(void) {
  ...
  if (strlen(fwd_modes) == 0) {
    while ((fwd_eng = fwd_engines[i++]) != NULL) {
      ...
      strncat(fwd_modes, separator, sizeof(fwd_modes) - strlen(fwd_modes) - 1);
    }
  ...
  for (queue = 0; queue < dev_info.nb_rx_queues; queue++)
    if (!ports[portid].rx_dump_cb[queue])
      ports[portid].rx_dump_cb[queue] =
          rte_eth_add_rx_callback(portid, queue, dump_rx_pkts, NULL);
}
  ...
  for (queue = 0; queue < dev_info.nb_tx_queues; queue++)
    if (!ports[portid].tx_dump_cb[queue])
      ports[portid].tx_dump_cb[queue] =
          rte_eth_add_tx_callback(portid, queue, dump_tx_pkts, NULL);
}
...
void vlan_extend_set(portid_t port_id, int on) {
  ...
  if (diag < 0) {
    fprintf(stderr, "rx_vlan_extend_set(port_pi=%d, on=%d) failed diag=%d\n",
            port_id, on, diag);
    return;
  }
...
void rx_vlan_strip_set_on_queue(portid_t port_id, uint16_t queue_id, int on) {
  ...
  if (diag < 0)
    fprintf(stderr, "%s(port_pi=%d, queue_id=%d, on=%d) failed diag=%d\n",
...
void tx_vlan_set(portid_t port_id, uint16_t vlan_id) {
  ...
  if ((dev_info.tx_offload_capa & RTE_ETH_TX_OFFLOAD_VLAN_INSERT) == 0) {
    fprintf(stderr, "Error: vlan insert is not supported by port %d\n",
            port_id);
    return;
  }
...
void tx_qinq_set(portid_t port_id, uint16_t vlan_id, uint16_t vlan_id_outer) {
  ...
  if ((dev_info.tx_offload_capa & RTE_ETH_TX_OFFLOAD_QINQ_INSERT) == 0) {
    fprintf(stderr, "Error: qinq insert not supported by port %d\n", port_id);
    return;
  }
...
void set_qmap(portid_t port_id, uint8_t is_rx, uint16_t queue_id,
  ...
  if (!is_rx) { /* tx */
    ret = rte_eth_dev_set_tx_queue_stats_mapping(port_id, queue_id, map_value);
    if (ret) {
      fprintf(stderr, "failed to set tx queue stats mapping.\n");
      return;
    }
  } else { /* rx */
    ret = rte_eth_dev_set_rx_queue_stats_mapping(port_id, queue_id, map_value);
    if (ret) {
...
void set_xstats_hide_zero(uint8_t on_off) { xstats_hide_zero = on_off; }

void set_record_core_cycles(uint8_t on_off) { record_core_cycles = on_off; }

void set_record_burst_stats(uint8_t on_off) { record_burst_stats = on_off; }
...
static inline void print_fdir_flow_type(uint32_t flow_types_mask) {
  int i;
  const char *p;
...
static int get_fdir_info(portid_t port_id, struct rte_eth_fdir_info *fdir_info,
...
void set_vf_traffic(portid_t port_id, uint8_t is_rx, uint16_t vf, uint8_t on) {
  ...
  if (diag == 0)
    return;
  fprintf(stderr, "rte_pmd_ixgbe_set_vf_%s for port_id=%d failed diag=%d\n",
...
int set_queue_rate_limit(portid_t port_id, uint16_t queue_idx, uint32_t rate) {
  ...
  if (ret < 0)
    return 1;
  if (link.link_speed != RTE_ETH_SPEED_NUM_UNKNOWN && rate > link.link_speed) {
...
int set_vf_rate_limit(portid_t port_id, uint16_t vf, uint32_t rate,
  ...
  fprintf(stderr, "%s for port_id=%d failed diag=%d\n", __func__, port_id,
          diag);
  return diag;
}
...
static int mcast_addr_pool_extend(struct rte_port *port) {
   ...
   */
  mc_pool_size =
      sizeof(struct rte_ether_addr) * (port->mc_addr_nb + MCAST_POOL_INC);
  mc_pool = (struct rte_ether_addr *)realloc(port->mc_addr_pool, mc_pool_size);
  if (mc_pool == NULL) {
    fprintf(stderr, "allocation of pool of %u multicast addresses failed\n",
...
static void mcast_addr_pool_append(struct rte_port *port,
...
static void mcast_addr_pool_remove(struct rte_port *port, uint32_t addr_idx) {
  port->mc_addr_nb--;
  ...
  memmove(&port->mc_addr_pool[addr_idx], &port->mc_addr_pool[addr_idx + 1],
...
static int eth_port_multicast_addr_list_set(portid_t port_id) {
  struct rte_port *port;
  int diag;
