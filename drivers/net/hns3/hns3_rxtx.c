...
static void hns3_rx_queue_release_mbufs(struct hns3_rx_queue *rxq) {
  uint16_t i;
  ...
  if (rxq->rx_rearm_nb == 0) {
    ...
    for (i = rxq->next_to_use; i != rxq->rx_rearm_start;
...
static void hns3_tx_queue_release_mbufs(struct hns3_tx_queue *txq) {
  uint16_t i;
...
static void hns3_rx_queue_release(void *queue) {
...
static void hns3_tx_queue_release(void *queue) {
...
static void hns3_rx_queue_release_lock(void *queue) {
  struct hns3_rx_queue *rxq = queue;
...
static void hns3_tx_queue_release_lock(void *queue) {
  struct hns3_tx_queue *txq = queue;
  struct hns3_adapter *hns;
...
static void hns3_fake_rx_queue_release(struct hns3_rx_queue *queue) {
  struct hns3_rx_queue *rxq = queue;
  struct hns3_adapter *hns;
  struct hns3_hw *hw;
  uint16_t idx;
...
static void hns3_fake_tx_queue_release(struct hns3_tx_queue *queue) {
  struct hns3_tx_queue *txq = queue;
  struct hns3_adapter *hns;
  struct hns3_hw *hw;
  uint16_t idx;
...
static void hns3_free_rx_queues(struct rte_eth_dev *dev) {
...
static void hns3_free_tx_queues(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_fake_queue_data *fkq_data;
  struct hns3_hw *hw = &hns->hw;
  uint16_t nb_tx_q;
...
static int hns3_alloc_rx_queue_mbufs(struct hns3_hw *hw,
  ...
  for (i = 0; i < rxq->nb_rx_desc; i++) {
    ...
    if (unlikely(mbuf == NULL)) {
      hns3_err(hw, "Failed to allocate RXD[%u] for rx queue!", i);
      hns3_rx_queue_release_mbufs(rxq);
      return -ENOMEM;
    }
...
static int hns3_buf_size2type(uint32_t buf_size) {
  int bd_size_type;
...
static void hns3_init_rx_queue_hw(struct hns3_rx_queue *rxq) {
  ...
  hns3_write_dev(rxq, HNS3_RING_RX_BASEADDR_H_REG, (uint32_t)(dma_addr >> 32));

  hns3_write_dev(rxq, HNS3_RING_RX_BD_LEN_REG, hns3_buf_size2type(rx_buf_len));
  hns3_write_dev(rxq, HNS3_RING_RX_BD_NUM_REG,
                 HNS3_CFG_DESC_NUM(rxq->nb_rx_desc));
}

static void hns3_init_tx_queue_hw(struct hns3_tx_queue *txq) {
  uint64_t dma_addr = txq->tx_ring_phys_addr;

  hns3_write_dev(txq, HNS3_RING_TX_BASEADDR_L_REG, (uint32_t)dma_addr);
  hns3_write_dev(txq, HNS3_RING_TX_BASEADDR_H_REG, (uint32_t)(dma_addr >> 32));
...
static void hns3_stop_unused_queue(void *tqp_base,
                                   enum hns3_ring_type queue_type) {
  uint32_t reg_offset;
  uint32_t reg;

  reg_offset = queue_type == HNS3_RING_TYPE_TX ? HNS3_RING_TX_EN_REG
                                               : HNS3_RING_RX_EN_REG;
...
void hns3_enable_all_queues(struct hns3_hw *hw, bool en) {
  ...
  for (i = 0; i < hw->cfg_max_queues; i++) {
    if (hns3_dev_get_support(hw, INDEP_TXRX)) {
      ...
      tqp_base = (void *)((char *)hw->io_base + hns3_get_tqp_reg_offset(i));
      /*
       ...
       */
      if (rxq == NULL)
        hns3_stop_unused_queue(tqp_base, HNS3_RING_TYPE_RX);
      if (txq == NULL)
        hns3_stop_unused_queue(tqp_base, HNS3_RING_TYPE_TX);
    } else {
...
static void hns3_enable_txq(struct hns3_tx_queue *txq, bool en) {
...
static void hns3_enable_rxq(struct hns3_rx_queue *rxq, bool en) {
...
static int hns3_tqp_enable(struct hns3_hw *hw, uint16_t queue_id, bool enable) {
...
static int hns3_send_reset_tqp_cmd(struct hns3_hw *hw, uint16_t queue_id,
                                   bool enable) {
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret)
    hns3_err(hw, "send tqp reset cmd error, queue_id = %u, ret = %d", queue_id,
             ret);

  return ret;
}

static int hns3_get_tqp_reset_status(struct hns3_hw *hw, uint16_t queue_id,
...
static int hns3pf_reset_tqp(struct hns3_hw *hw, uint16_t queue_id) {
#define HNS3_TQP_RESET_TRY_MS 200
  ...
  if (!reset_status) {
    ret = -ETIMEDOUT;
    hns3_err(hw, "reset tqp timeout, queue_id = %u, ret = %d", queue_id, ret);
    goto tqp_reset_fail;
  }
...
static int hns3vf_reset_tqp(struct hns3_hw *hw, uint16_t queue_id) {
  ...
  if (ret)
    hns3_err(hw, "fail to reset tqp, queue_id = %u, ret = %d.", queue_id, ret);
  return ret;
}

static int hns3_reset_rcb_cmd(struct hns3_hw *hw, uint8_t *reset_status) {
...
static int hns3pf_reset_all_tqps(struct hns3_hw *hw) {
...
#define HNS3_RESET_ALL_TQP_SUCCESS 1U
  ...
  if (reset_status == HNS3_RESET_RCB_NOT_SUPPORT) {
    for (i = 0; i < hw->cfg_max_queues; i++) {
      ...
      if (ret) {
        hns3_err(hw, "fail to reset tqp, queue_id = %u, ret = %d.", i, ret);
        return ret;
      }
    }
  } else if (reset_status != HNS3_RESET_ALL_TQP_SUCCESS) {
    hns3_err(hw, "fail to reset all tqps, reset_status = %u.", reset_status);
    return -EIO;
  }
...
static int hns3vf_reset_all_tqps(struct hns3_hw *hw) {
#define HNS3VF_RESET_ALL_TQP_DONE 1U
  ...
  hns3vf_mbx_setup(&req, HNS3_MBX_QUEUE_RESET, 0);
  ret = hns3vf_mbx_send(hw, &req, true, &reset_status, sizeof(reset_status));
  if (ret) {
...
int hns3_reset_all_tqps(struct hns3_adapter *hns) {
  ...
  for (i = 0; i < hw->cfg_max_queues; i++) {
    ...
    if (ret) {
      hns3_err(hw, "fail to disable tqps before tqps reset, ret = %d.", ret);
      return ret;
    }
  }
...
static int hns3_send_reset_queue_cmd(struct hns3_hw *hw, uint16_t queue_id,
                                     enum hns3_ring_type queue_type,
                                     bool enable) {
  struct hns3_reset_tqp_queue_cmd *req;
  struct hns3_cmd_desc desc;
  ...
  if (ret)
    hns3_err(hw,
             "send queue reset cmd error, queue_id = %u, "
             "queue_type = %s, ret = %d.",
             queue_id, queue_type == HNS3_RING_TYPE_TX ? "Tx" : "Rx", ret);
  return ret;
}

static int hns3_get_queue_reset_status(struct hns3_hw *hw, uint16_t queue_id,
                                       enum hns3_ring_type queue_type,
                                       uint8_t *reset_status) {
  struct hns3_reset_tqp_queue_cmd *req;
  struct hns3_cmd_desc desc;
  ...
  if (ret) {
    hns3_err(hw,
             "get queue reset status error, queue_id = %u "
             "queue_type = %s, ret = %d.",
             queue_id, queue_type == HNS3_RING_TYPE_TX ? "Tx" : "Rx", ret);
    return ret;
  }
...
static int hns3_reset_queue(struct hns3_hw *hw, uint16_t queue_id,
                            enum hns3_ring_type queue_type) {
#define HNS3_QUEUE_RESET_TRY_MS 200
  ...
  while (reset_wait_times < max_wait_times) {
    ...
    rte_delay_ms(HNS3_POLL_RESPONE_MS);
    ret = hns3_get_queue_reset_status(hw, queue_id, queue_type, &reset_status);
    if (ret)
  ...
  if (!reset_status) {
    hns3_err(hw, "reset queue timeout, queue_id = %u, queue_type = %s",
             queue_id, queue_type == HNS3_RING_TYPE_TX ? "Tx" : "Rx");
    ret = -ETIMEDOUT;
    goto queue_reset_fail;
  }
...
uint32_t hns3_get_tqp_intr_reg_offset(uint16_t tqp_intr_id) {
  ...
  if (tqp_intr_id < HNS3_MIN_EXT_TQP_INTR_ID)
    reg_offset =
        HNS3_TQP_INTR_REG_BASE + tqp_intr_id * HNS3_TQP_INTR_LOW_ORDER_OFFSET;
  else
    reg_offset =
        HNS3_TQP_INTR_EXT_REG_BASE +
        tqp_intr_id / HNS3_MIN_EXT_TQP_INTR_ID *
            HNS3_TQP_INTR_HIGH_ORDER_OFFSET +
        tqp_intr_id % HNS3_MIN_EXT_TQP_INTR_ID * HNS3_TQP_INTR_LOW_ORDER_OFFSET;
...
void hns3_set_queue_intr_gl(struct hns3_hw *hw, uint16_t queue_id,
  ...
  uint32_t offset[] = {HNS3_TQP_INTR_GL0_REG, HNS3_TQP_INTR_GL1_REG,
...
static void hns3_queue_intr_enable(struct hns3_hw *hw, uint16_t queue_id,
                                   bool en) {
  uint32_t addr, value;
...
static int hns3_init_rxq(struct hns3_adapter *hns, uint16_t idx) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_rx_queue *rxq;
  int ret;
  ...
  if (ret) {
    hns3_err(hw, "fail to alloc mbuf for Rx queue %u, ret = %d.", idx, ret);
    return ret;
  }
...
static void hns3_init_fake_rxq(struct hns3_adapter *hns, uint16_t idx) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_rx_queue *rxq;
...
static void hns3_init_txq(struct hns3_tx_queue *txq) {
...
static void hns3_init_tx_ring_tc(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_tx_queue *txq;
  int i, num;
...
static int hns3_init_rx_queues(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_rx_queue *rxq;
  uint16_t i, j;
  int ret;
  ...
  for (i = 0; i < hw->data->nb_rx_queues; i++) {
    ...
    if (ret) {
      hns3_err(hw, "failed to init Rx queue %u, ret = %d.", i, ret);
      goto out;
    }
  }
...
static int hns3_init_tx_queues(struct hns3_adapter *hns) {
  struct hns3_hw *hw = &hns->hw;
  struct hns3_tx_queue *txq;
  uint16_t i;
...
int hns3_init_queues(struct hns3_adapter *hns, bool reset_queue) {
  ...
  if (reset_queue) {
    ...
    if (ret) {
      hns3_err(hw, "failed to reset all queues, ret = %d.", ret);
      return ret;
    }
  }
...
void hns3_start_tqps(struct hns3_hw *hw) {
  ...
  for (i = 0; i < hw->data->nb_tx_queues; i++) {
    txq = hw->data->tx_queues[i];
    if (txq->enabled)
      hw->data->tx_queue_state[i] = RTE_ETH_QUEUE_STATE_STARTED;
  }

  for (i = 0; i < hw->data->nb_rx_queues; i++) {
    rxq = hw->data->rx_queues[i];
    if (rxq->enabled)
      hw->data->rx_queue_state[i] = RTE_ETH_QUEUE_STATE_STARTED;
  }
}
...
int hns3_rxq_iterate(struct rte_eth_dev *dev,
                     int (*callback)(struct hns3_rx_queue *, void *),
                     void *arg) {
  uint32_t i;
  int ret;
...
static void *hns3_alloc_rxq_and_dma_zone(struct rte_eth_dev *dev,
  ...
  if (rxq == NULL) {
    hns3_err(hw, "Failed to allocate memory for No.%u rx ring!", q_info->idx);
    return NULL;
  }
   ...
   */
  rx_desc =
      (rxq->nb_rx_desc + HNS3_DEFAULT_RX_BURST) * sizeof(struct hns3_desc);
  rx_mz = rte_eth_dma_zone_reserve(dev, q_info->ring_name, q_info->idx, rx_desc,
                                   HNS3_RING_BASE_ALIGN, q_info->socket_id);
  if (rx_mz == NULL) {
...
static int hns3_fake_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx,
...
static void *hns3_alloc_txq_and_dma_zone(struct rte_eth_dev *dev,
                                         struct hns3_queue_info *q_info) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  txq = rte_zmalloc_socket(q_info->type, sizeof(struct hns3_tx_queue),
                           RTE_CACHE_LINE_SIZE, q_info->socket_id);
  if (txq == NULL) {
    hns3_err(hw, "Failed to allocate memory for No.%u tx ring!", q_info->idx);
    return NULL;
  }
  ...
  tx_mz = rte_eth_dma_zone_reserve(dev, q_info->ring_name, q_info->idx, tx_desc,
                                   HNS3_RING_BASE_ALIGN, q_info->socket_id);
  if (tx_mz == NULL) {
...
static int hns3_fake_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx,
                                    uint16_t nb_desc, unsigned int socket_id) {
  struct hns3_adapter *hns = dev->data->dev_private;
  struct hns3_hw *hw = &hns->hw;
  struct hns3_queue_info q_info;
...
static int hns3_fake_rx_queue_config(struct hns3_hw *hw, uint16_t nb_queues) {
  ...
  if (hw->fkq_data.rx_queues == NULL && nb_queues != 0) {
    ...
    hw->fkq_data.rx_queues =
        rte_zmalloc("fake_rx_queues", size, RTE_CACHE_LINE_SIZE);
    ...
    rxq = rte_realloc(rxq, sizeof(rxq[0]) * nb_queues, RTE_CACHE_LINE_SIZE);
    if (rxq == NULL)
      return -ENOMEM;
...
static int hns3_fake_tx_queue_config(struct hns3_hw *hw, uint16_t nb_queues) {
  ...
  if (hw->fkq_data.tx_queues == NULL && nb_queues != 0) {
    ...
    hw->fkq_data.tx_queues =
        rte_zmalloc("fake_tx_queues", size, RTE_CACHE_LINE_SIZE);
...
static int hns3_rx_buf_len_calc(struct rte_mempool *mp, uint16_t *rx_buf_len) {
  ...
  static const uint16_t hw_rx_buf_size[] = {
      HNS3_4K_BD_BUF_SIZE, HNS3_2K_BD_BUF_SIZE, HNS3_1K_BD_BUF_SIZE,
      HNS3_512_BD_BUF_SIZE};

  vld_buf_size =
      (uint16_t)(rte_pktmbuf_data_room_size(mp) - RTE_PKTMBUF_HEADROOM);
...
static int hns3_rxq_conf_runtime_check(struct hns3_hw *hw, uint16_t buf_size,
                                       uint16_t nb_desc) {
  struct rte_eth_dev *dev = &rte_eth_devices[hw->data->port_id];
  ...
  if (pkt_burst == hns3_recv_pkts_vec || pkt_burst == hns3_recv_pkts_vec_sve) {
    min_vec_bds = HNS3_DEFAULT_RXQ_REARM_THRESH + HNS3_DEFAULT_RX_BURST;
    if (nb_desc < min_vec_bds || nb_desc % HNS3_DEFAULT_RXQ_REARM_THRESH) {
      hns3_err(hw,
               "if Rx burst mode is vector, "
...
static int hns3_rx_queue_conf_check(struct hns3_hw *hw,
  ...
  if (nb_desc > HNS3_MAX_RING_DESC || nb_desc < HNS3_MIN_RING_DESC ||
    ...
    hns3_err(hw, "Number (%u) of rx descriptors is invalid", nb_desc);
    return -EINVAL;
  }
  ...
  if (hns3_rx_buf_len_calc(mp, buf_size)) {
    hns3_err(hw,
             "rxq mbufs' data room size (%u) is not enough! "
...
uint32_t hns3_get_tqp_reg_offset(uint16_t queue_id) {
  ...
  if (queue_id < HNS3_MIN_EXTEND_QUEUE_ID)
    ...
    reg_offset = HNS3_TQP_REG_OFFSET + HNS3_TQP_EXT_REG_OFFSET +
                 (queue_id - HNS3_MIN_EXTEND_QUEUE_ID) * HNS3_TQP_REG_SIZE;
...
int hns3_rx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t nb_desc,
  ...
  rxq = hns3_alloc_rxq_and_dma_zone(dev, &q_info);
  if (rxq == NULL) {
    hns3_err(hw, "Failed to alloc mem and reserve DMA mem for rx ring!");
    return -ENOMEM;
  }
  ...
  rx_entry_len =
      (rxq->nb_rx_desc + HNS3_DEFAULT_RX_BURST) * sizeof(struct hns3_entry);
  ...
  if (hns->is_vf || hw->vlan_mode == HNS3_SW_SHIFT_AND_DISCARD_MODE)
    rxq->pvid_sw_discard_en =
        hw->port_base_vlan_cfg.state == HNS3_PORT_BASE_VLAN_ENABLE;
  else
    rxq->pvid_sw_discard_en = false;
  rxq->ptype_en = hns3_dev_get_support(hw, RXD_ADV_LAYOUT) ? true : false;
  rxq->configured = true;
  rxq->io_base = (void *)((char *)hw->io_base + hns3_get_tqp_reg_offset(idx));
  rxq->io_head_reg =
      (volatile void *)((char *)rxq->io_base + HNS3_RING_RX_HEAD_REG);
...
void hns3_rx_scattered_calc(struct rte_eth_dev *dev) {
  ...
  for (queue_id = 0; queue_id < dev->data->nb_rx_queues; queue_id++) {
    ...
    if (hw->rx_buf_len == 0)
      ...
      hw->rx_buf_len = RTE_MIN(hw->rx_buf_len, rxq->rx_buf_len);
  }
...
static void hns3_init_non_tunnel_ptype_tbl(struct hns3_ptype_table *tbl) {
...
static void hns3_init_tunnel_ptype_tbl(struct hns3_ptype_table *tbl) {
  tbl->inner_l3table[0] = RTE_PTYPE_INNER_L2_ETHER | RTE_PTYPE_INNER_L3_IPV4;
  tbl->inner_l3table[1] = RTE_PTYPE_INNER_L2_ETHER | RTE_PTYPE_INNER_L3_IPV6;
...
static void hns3_init_adv_layout_ptype(struct hns3_ptype_table *tbl) {
  ...
  ptype[18] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_NONFRAG;
  ptype[19] =
  ...
  ptype[21] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_TUNNEL_GRE;
  ptype[22] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_SCTP;
  ptype[23] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_IGMP;
  ptype[24] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_L4_ICMP;
  ...
  ptype[32] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_TUNNEL_GRENAT | RTE_PTYPE_INNER_L2_ETHER |
              RTE_PTYPE_INNER_L3_IPV4_EXT_UNKNOWN | RTE_PTYPE_INNER_L4_NONFRAG;
  ...
  ptype[40] = RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4_EXT_UNKNOWN |
              RTE_PTYPE_TUNNEL_GRENAT | RTE_PTYPE_INNER_L2_ETHER |
              RTE_PTYPE_INNER_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_INNER_L4_NONFRAG;
  ...
  /* Non-tunnel IPv6 */
  ptype[111] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_FRAG;
  ptype[112] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_NONFRAG;
  ptype[113] =
  ...
  ptype[115] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_TUNNEL_GRE;
  ptype[116] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_SCTP;
  ptype[117] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_IGMP;
  ptype[118] =
      RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV6_EXT_UNKNOWN | RTE_PTYPE_L4_ICMP;
...
static inline void hns3_rxd_to_vlan_tci(struct hns3_rx_queue *rxq,
                                        struct rte_mbuf *mb, uint32_t l234_info,
                                        const struct hns3_desc *rxd) {
...
#define HNS3_OUTER_STRP_VLAN_VLD 0x2
  ...
  static const uint32_t report_type[][HNS3_STRP_STATUS_NUM] = {
      {HNS3_NO_STRP_VLAN_VLD, HNS3_OUTER_STRP_VLAN_VLD,
       HNS3_INNER_STRP_VLAN_VLD, HNS3_OUTER_STRP_VLAN_VLD},
      {HNS3_NO_STRP_VLAN_VLD, HNS3_NO_STRP_VLAN_VLD, HNS3_NO_STRP_VLAN_VLD,
       HNS3_INNER_STRP_VLAN_VLD}};
  strip_status =
      hns3_get_field(l234_info, HNS3_RXD_STRP_TAGP_M, HNS3_RXD_STRP_TAGP_S);
...
static inline void recalculate_data_len(struct rte_mbuf *first_seg,
  ...
  if (data_len <= crc_len) {
    ...
    first_seg->nb_segs--;
    last_seg->data_len = (uint16_t)(last_seg->data_len - (crc_len - data_len));
    last_seg->next = NULL;
  } else
    rxm->data_len = (uint16_t)(data_len - crc_len);
}

static inline struct rte_mbuf *hns3_rx_alloc_buffer(struct hns3_rx_queue *rxq) {
  int ret;
...
static void hns3_rx_ptp_timestamp_handle(struct hns3_rx_queue *rxq,
                                         struct rte_mbuf *mbuf,
                                         uint64_t timestamp) {
...
uint16_t hns3_recv_pkts_simple(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    rxd =
        rxdp[(bd_base_info & (1u << HNS3_RXD_VLD_B)) - (1u << HNS3_RXD_VLD_B)];
    ...
    if (unlikely(bd_base_info & BIT(HNS3_RXD_TS_VLD_B)))
      hns3_rx_ptp_timestamp_handle(rxq, rxm, rte_le_to_cpu_64(rxdp->timestamp));
    ...
    rxm->pkt_len = (uint16_t)(rte_le_to_cpu_16(rxd.rx.pkt_len)) - rxq->crc_len;
    ...
    if (unlikely(bd_base_info & BIT(HNS3_RXD_LUM_B))) {
      rxm->hash.fdir.hi = rte_le_to_cpu_16(rxd.rx.fd_id);
...
uint16_t hns3_recv_scattered_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    if (unlikely(rxq->crc_len > 0)) {
      ...
      recalculate_data_len(first_seg, last_seg, rxm, rxq, rxm->data_len);
    }
    ...
    if (unlikely(bd_base_info & BIT(HNS3_RXD_LUM_B))) {
      first_seg->hash.fdir.hi = rte_le_to_cpu_16(rxd.rx.fd_id);
    ...
    l234_info = rte_le_to_cpu_32(rxd.rx.l234_info);
    ol_info = rte_le_to_cpu_32(rxd.rx.ol_info);
    ret = hns3_handle_bdinfo(rxq, first_seg, bd_base_info, l234_info);
    if (unlikely(ret))
      goto pkt_err;

    first_seg->packet_type = hns3_rx_calc_ptype(rxq, l234_info, ol_info);
...
void __rte_weak hns3_rxq_vec_setup(__rte_unused struct hns3_rx_queue *rxq) {}

int __rte_weak hns3_rx_check_vec_support(__rte_unused struct rte_eth_dev *dev) {
  return -ENOTSUP;
}

uint16_t __rte_weak hns3_recv_pkts_vec(__rte_unused void *rx_queue,
                                       __rte_unused struct rte_mbuf **rx_pkts,
                                       __rte_unused uint16_t nb_pkts) {
  return 0;
}

uint16_t __rte_weak hns3_recv_pkts_vec_sve(
    __rte_unused void *rx_queue, __rte_unused struct rte_mbuf **rx_pkts,
    __rte_unused uint16_t nb_pkts) {
  return 0;
}
...
int hns3_rx_burst_mode_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < RTE_DIM(burst_infos); i++) {
    if (pkt_burst == burst_infos[i].pkt_burst) {
      snprintf(mode->info, sizeof(mode->info), "%s", burst_infos[i].info);
      ret = 0;
      break;
    }
  }
...
static bool hns3_get_default_vec_support(void) {
#if defined(RTE_ARCH_ARM64)
...
static bool hns3_get_sve_support(void) {
...
static eth_rx_burst_t hns3_get_rx_function(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  ...
  simple_allowed =
      !dev->data->scattered_rx && (offloads & RTE_ETH_RX_OFFLOAD_TCP_LRO) == 0;
...
static int hns3_tx_queue_conf_check(struct hns3_hw *hw,
...
#define HNS3_TX_RS_FREE_THRESH_GAP 8
  ...
  if (nb_desc > HNS3_MAX_RING_DESC || nb_desc < HNS3_MIN_RING_DESC ||
      nb_desc % HNS3_ALIGN_RING_DESC) {
    hns3_err(hw, "number (%u) of tx descriptors is invalid", nb_desc);
    return -EINVAL;
  }

  rs_thresh =
      (conf->tx_rs_thresh > 0) ? conf->tx_rs_thresh : HNS3_DEFAULT_TX_RS_THRESH;
  free_thresh = (conf->tx_free_thresh > 0) ? conf->tx_free_thresh
                                           : HNS3_DEFAULT_TX_FREE_THRESH;
  if (rs_thresh + free_thresh > nb_desc || nb_desc % rs_thresh ||
    ...
    hns3_err(hw,
             "tx_rs_thresh (%u) tx_free_thresh (%u) nb_desc "
             "(%u) of tx descriptors for port=%u queue=%u check "
             "fail!",
             rs_thresh, free_thresh, nb_desc, hw->data->port_id, idx);
    return -EINVAL;
  }
  ...
  if (conf->tx_free_thresh == 0) {
    ...
    if (fast_free_thresh >=
      ...
      free_thresh = fast_free_thresh - HNS3_TX_FAST_FREE_AHEAD;
  }
...
static void *hns3_tx_push_get_queue_tail_reg(struct rte_eth_dev *dev,
                                             uint16_t queue_id) {
...
static void hns3_tx_push_queue_init(struct rte_eth_dev *dev, uint16_t queue_id,
                                    struct hns3_tx_queue *txq) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  if (!hns3_dev_get_support(hw, TX_PUSH)) {
  ...
  txq->io_tail_reg =
      (volatile void *)hns3_tx_push_get_queue_tail_reg(dev, queue_id);
...
int hns3_tx_queue_setup(struct rte_eth_dev *dev, uint16_t idx, uint16_t nb_desc,
  ...
  ret = hns3_tx_queue_conf_check(hw, conf, nb_desc, &tx_rs_thresh,
                                 &tx_free_thresh, idx);
  if (ret)
    return ret;
  ...
  txq = hns3_alloc_txq_and_dma_zone(dev, &q_info);
  if (txq == NULL) {
    hns3_err(hw, "Failed to alloc mem and reserve DMA mem for tx ring!");
    return -ENOMEM;
  }
  ...
  txq->configured = true;
  txq->io_base = (void *)((char *)hw->io_base + hns3_get_tqp_reg_offset(idx));
  txq->io_tail_reg =
      (volatile void *)((char *)txq->io_base + HNS3_RING_TX_TAIL_REG);
...
static void hns3_tx_free_useless_buffer(struct hns3_tx_queue *txq) {
...
int hns3_config_gro(struct hns3_hw *hw, bool en) {
  ...
  ret = hns3_cmd_send(hw, &desc, 1);
  if (ret)
    hns3_err(hw, "%s hardware GRO failed, ret = %d", en ? "enable" : "disable",
             ret);
...
static inline bool hns3_pkt_is_tso(struct rte_mbuf *m) {
  return (m->tso_segsz != 0 && m->ol_flags & RTE_MBUF_F_TX_TCP_SEG);
}

static void hns3_set_tso(struct hns3_desc *desc, uint32_t paylen,
                         struct rte_mbuf *rxm) {
...
static inline void hns3_fill_per_desc(struct hns3_desc *desc,
                                      struct rte_mbuf *rxm) {
...
static void hns3_fill_first_desc(struct hns3_tx_queue *txq,
                                 struct hns3_desc *desc, struct rte_mbuf *rxm) {
  ...
  hdr_len += (ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
                 ? rxm->outer_l2_len + rxm->outer_l3_len
                 : 0;
  ...
  if (!txq->pvid_sw_shift_en &&
    ...
    desc->tx.ol_type_vlan_len_msec |= rte_cpu_to_le_32(BIT(HNS3_TXD_OVLAN_B));
    if (ol_flags & RTE_MBUF_F_TX_QINQ)
      desc->tx.outer_vlan_tag = rte_cpu_to_le_16(rxm->vlan_tci_outer);
    else
      desc->tx.outer_vlan_tag = rte_cpu_to_le_16(rxm->vlan_tci);
  }

  if (ol_flags & RTE_MBUF_F_TX_QINQ ||
      ((ol_flags & RTE_MBUF_F_TX_VLAN) && txq->pvid_sw_shift_en)) {
    desc->tx.type_cs_vlan_tso_len |= rte_cpu_to_le_32(BIT(HNS3_TXD_VLAN_B));
    desc->tx.vlan_tag = rte_cpu_to_le_16(rxm->vlan_tci);
  }

  if (ol_flags & RTE_MBUF_F_TX_IEEE1588_TMST)
    desc->tx.tp_fe_sc_vld_ra_ri |= rte_cpu_to_le_16(BIT(HNS3_TXD_TSYN_B));
}

static inline int hns3_tx_alloc_mbufs(struct rte_mempool *mb_pool,
                                      uint16_t nb_new_buf,
...
static inline void hns3_pktmbuf_copy_hdr(struct rte_mbuf *new_pkt,
...
static int hns3_reassemble_tx_pkts(struct rte_mbuf *tx_pkt,
...
static void hns3_parse_outer_params(struct rte_mbuf *m,
  ...
  if (ol_flags & RTE_MBUF_F_TX_OUTER_IPV4) {
    ...
    tmp |= hns3_gen_field_val(HNS3_TXD_OL3T_M, HNS3_TXD_OL3T_S, HNS3_OL3T_IPV6);
  }
...
static int hns3_parse_inner_params(struct rte_mbuf *m,
                                   uint32_t *ol_type_vlan_len_msec,
...
#define HNS3_NVGRE_HLEN 8
  ...
  case RTE_MBUF_F_TX_TUNNEL_VXLAN:
    ...
    tmp_outer |= hns3_gen_field_val(HNS3_TXD_TUNTYPE_M, HNS3_TXD_TUNTYPE_S,
                                    HNS3_TUN_MAC_IN_UDP);
    /*
     ...
     */
    tmp_outer |=
        hns3_gen_field_val(HNS3_TXD_L4LEN_M, HNS3_TXD_L4LEN_S,
                           (uint8_t)RTE_ETHER_VXLAN_HLEN >> HNS3_L4_LEN_UNIT);
...
static int hns3_parse_tunneling_params(struct hns3_tx_queue *txq,
                                       struct rte_mbuf *m,
                                       uint16_t tx_desc_id) {
  ...
  if (!(ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)) {
     ...
     */
    if (unlikely(!(ol_flags & (RTE_MBUF_F_TX_OUTER_IP_CKSUM |
  ...
  tmp_ol4cs =
      ol_flags & RTE_MBUF_F_TX_OUTER_UDP_CKSUM ? BIT(HNS3_TXD_OL4CS_B) : 0;
  desc->tx.paylen_fd_dop_ol4cs = rte_cpu_to_le_32(tmp_ol4cs);

  return 0;
}

static void hns3_parse_l3_cksum_params(struct rte_mbuf *m,
                                       uint32_t *type_cs_vlan_tso_len) {
  uint64_t ol_flags = m->ol_flags;
  uint32_t l3_type;
  uint32_t tmp;
...
static void hns3_parse_l4_cksum_params(struct rte_mbuf *m,
                                       uint32_t *type_cs_vlan_tso_len) {
  uint64_t ol_flags = m->ol_flags;
  uint32_t tmp;
  ...
  case RTE_MBUF_F_TX_TCP_SEG:
    tmp = *type_cs_vlan_tso_len;
    tmp |= hns3_gen_field_val(HNS3_TXD_L4T_M, HNS3_TXD_L4T_S, HNS3_L4T_TCP);
    break;
  case RTE_MBUF_F_TX_UDP_CKSUM:
    tmp = *type_cs_vlan_tso_len;
    tmp |= hns3_gen_field_val(HNS3_TXD_L4T_M, HNS3_TXD_L4T_S, HNS3_L4T_UDP);
    break;
  case RTE_MBUF_F_TX_SCTP_CKSUM:
    tmp = *type_cs_vlan_tso_len;
    tmp |= hns3_gen_field_val(HNS3_TXD_L4T_M, HNS3_TXD_L4T_S, HNS3_L4T_SCTP);
    break;
  default:
    return;
  }
...
static void hns3_txd_enable_checksum(struct hns3_tx_queue *txq,
                                     struct rte_mbuf *m, uint16_t tx_desc_id) {
  struct hns3_desc *tx_ring = txq->tx_ring;
  struct hns3_desc *desc = &tx_ring[tx_desc_id];
  uint32_t value = 0;
...
static bool hns3_pkt_need_linearized(struct rte_mbuf *tx_pkts, uint32_t bd_num,
  ...
  for (i = 0; m_last && i < max_non_tso_bd_num - 1; i++, m_last = m_last->next)
  ...
  hdr_len += (tx_pkts->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
                 ? tx_pkts->outer_l2_len + tx_pkts->outer_l3_len
                 : 0;
...
static bool hns3_outer_ipv4_cksum_prepared(struct rte_mbuf *m,
                                           uint64_t ol_flags,
                                           uint32_t *l4_proto) {
  struct rte_ipv4_hdr *ipv4_hdr;
  ipv4_hdr = rte_pktmbuf_mtod_offset(m, struct rte_ipv4_hdr *, m->outer_l2_len);
  if (ol_flags & RTE_MBUF_F_TX_OUTER_IP_CKSUM)
...
static bool hns3_outer_ipv6_cksum_prepared(struct rte_mbuf *m,
                                           uint64_t ol_flags,
                                           uint32_t *l4_proto) {
  struct rte_ipv6_hdr *ipv6_hdr;
  ipv6_hdr = rte_pktmbuf_mtod_offset(m, struct rte_ipv6_hdr *, m->outer_l2_len);
  if (ol_flags & RTE_MBUF_F_TX_OUTER_UDP_CKSUM) {
    struct rte_udp_hdr *udp_hdr;
    /*
     * If OUTER_UDP_CKSUM is support, HW can calculate the pseudo
...
static void hns3_outer_header_cksum_prepare(struct rte_mbuf *m) {
  uint64_t ol_flags = m->ol_flags;
...
static int hns3_check_tso_pkt_valid(struct rte_mbuf *m) {
...
static inline int hns3_vld_vlan_chk(struct hns3_tx_queue *txq,
                                    struct rte_mbuf *m) {
...
static uint16_t hns3_udp_cksum_help(struct rte_mbuf *m) {
  uint64_t ol_flags = m->ol_flags;
  uint16_t cksum = 0;
  uint32_t l4_len;
  ...
  if (ol_flags & RTE_MBUF_F_TX_IPV4) {
    ...
    struct rte_ipv6_hdr *ipv6_hdr =
        rte_pktmbuf_mtod_offset(m, struct rte_ipv6_hdr *, m->l2_len);
...
static bool hns3_validate_tunnel_cksum(struct hns3_tx_queue *tx_queue,
                                       struct rte_mbuf *m) {
  uint64_t ol_flags = m->ol_flags;
  struct rte_udp_hdr *udp_hdr;
  uint16_t dst_port;
   ...
   */
  udp_hdr =
      rte_pktmbuf_mtod_offset(m, struct rte_udp_hdr *, m->l2_len + m->l3_len);
...
static int hns3_prep_pkt_proc(struct hns3_tx_queue *tx_queue,
                              struct rte_mbuf *m) {
  int ret;
  ...
  if (hns3_pkt_is_tso(m)) {
    if (hns3_pkt_need_linearized(m, m->nb_segs, tx_queue->max_non_tso_bd_num) ||
...
static inline int hns3_handle_simple_bd(struct hns3_tx_queue *txq,
                                        struct hns3_desc *desc,
                                        struct rte_mbuf *m) {
...
#define HNS3_UDP_CSUM_OFFSET 6
  ...
  if (txq->simple_bd_enable && !(m->ol_flags & RTE_MBUF_F_TX_IP_CKSUM) &&
    ...
    hns3_set_field(desc->tx.type_cs_vlan_tso_len, HNS3_TXD_L4_START_M,
                   HNS3_TXD_L4_START_S,
                   (m->l2_len + m->l3_len) >> HNS3_SIMPLE_BD_UNIT);
    hns3_set_field(desc->tx.ol_type_vlan_len_msec, HNS3_TXD_L4_CKS_OFFSET_M,
...
static int hns3_parse_cksum(struct hns3_tx_queue *txq, uint16_t tx_desc_id,
                            struct rte_mbuf *m) {
  struct hns3_desc *tx_ring = txq->tx_ring;
  struct hns3_desc *desc = &tx_ring[tx_desc_id];
...
static int hns3_check_non_tso_pkt(uint16_t nb_buf, struct rte_mbuf **m_seg,
  ...
  if (unlikely(nb_buf > max_non_tso_bd_num)) {
    ...
    ret = hns3_reassemble_tx_pkts(tx_pkt, &new_pkt, max_non_tso_bd_num);
    if (ret) {
...
static inline void hns3_tx_free_buffer_simple(struct hns3_tx_queue *txq) {
  ...
  while (1) {
     ...
     */
    tx_next_clean =
        (txq->next_to_clean + txq->tx_rs_thresh - 1) % txq->nb_tx_desc;
    desc = &txq->tx_ring[tx_next_clean];
    for (i = 0; i < txq->tx_rs_thresh; i++) {
      if (rte_le_to_cpu_16(desc->tx.tp_fe_sc_vld_ra_ri) & BIT(HNS3_TXD_VLD_B))
        return;
      desc--;
    }
...
static inline void hns3_tx_backup_1mbuf(struct hns3_entry *tx_entry,
                                        struct rte_mbuf **pkts) {
  tx_entry->mbuf = pkts[0];
}

static inline void hns3_tx_backup_4mbuf(struct hns3_entry *tx_entry,
                                        struct rte_mbuf **pkts) {
...
static inline void hns3_tx_setup_4bd(struct hns3_desc *txdp,
                                     struct rte_mbuf **pkts) {
...
static inline void hns3_tx_setup_1bd(struct hns3_desc *txdp,
                                     struct rte_mbuf **pkts) {
  uint16_t bd_flag = BIT(HNS3_TXD_VLD_B) | BIT(HNS3_TXD_FE_B);
  uint64_t dma_addr;
...
static inline void hns3_tx_fill_hw_ring(struct hns3_tx_queue *txq,
                                        struct rte_mbuf **pkts,
                                        uint16_t nb_pkts) {
#define PER_LOOP_NUM 4
#define PER_LOOP_MASK (PER_LOOP_NUM - 1)
  ...
  if (unlikely(leftover > 0)) {
    for (i = 0; i < leftover; i++) {
      hns3_tx_backup_1mbuf(tx_entry + mainpart + i, pkts + mainpart + i);
      hns3_tx_setup_1bd(txdp + mainpart + i, pkts + mainpart + i);
...
uint16_t hns3_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < tx_pkt_num; nb_tx++) {
     ...
     * error.
     */
    if (unlikely(rte_pktmbuf_pkt_len(tx_pkt) < txq->min_tx_pkt_len)) {
      uint16_t add_len;
      char *appended;
    ...
    desc->tx.tp_fe_sc_vld_ra_ri |= rte_cpu_to_le_16(BIT(HNS3_TXD_FE_B));
    TX_BD_LOG(&txq->hns->hw, DEBUG, desc);
...
int __rte_weak hns3_tx_check_vec_support(__rte_unused struct rte_eth_dev *dev) {
  return -ENOTSUP;
}

uint16_t __rte_weak hns3_xmit_pkts_vec(__rte_unused void *tx_queue,
                                       __rte_unused struct rte_mbuf **tx_pkts,
                                       __rte_unused uint16_t nb_pkts) {
  return 0;
}

uint16_t __rte_weak hns3_xmit_pkts_vec_sve(
    void __rte_unused *tx_queue, struct rte_mbuf __rte_unused **tx_pkts,
...
static bool hns3_tx_check_simple_support(struct rte_eth_dev *dev) {
  uint64_t offloads = dev->data->dev_conf.txmode.offloads;

  return (offloads == (offloads & RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE));
}

static bool hns3_get_tx_prep_needed(struct rte_eth_dev *dev) {
#define HNS3_DEV_TX_CSKUM_TSO_OFFLOAD_MASK                                     \
  (RTE_ETH_TX_OFFLOAD_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_TCP_CKSUM |              \
   ...
   RTE_ETH_TX_OFFLOAD_GRE_TNL_TSO | RTE_ETH_TX_OFFLOAD_GENEVE_TNL_TSO)
...
static eth_tx_prep_t hns3_get_tx_prepare(struct rte_eth_dev *dev) {
  return hns3_get_tx_prep_needed(dev) ? hns3_prep_pkts : NULL;
}

static eth_tx_burst_t hns3_get_tx_function(struct rte_eth_dev *dev) {
  struct hns3_adapter *hns = dev->data->dev_private;
  bool vec_allowed, sve_allowed, simple_allowed;
  bool vec_support;
...
static void hns3_trace_rxtx_function(struct rte_eth_dev *dev) {
  struct hns3_hw *hw = HNS3_DEV_PRIVATE_TO_HW(dev->data->dev_private);
  ...
  hns3_dbg(hw, "using rx_pkt_burst: %s, tx_pkt_burst: %s.", rx_mode.info,
           tx_mode.info);
}

static void hns3_eth_dev_fp_ops_config(const struct rte_eth_dev *dev) {
...
void hns3_set_rxtx_function(struct rte_eth_dev *eth_dev) {
  ...
  if (hns->hw.adapter_state == HNS3_NIC_STARTED &&
    ...
    eth_dev->tx_pkt_burst = hw->set_link_down ? rte_eth_pkt_burst_dummy
...
int hns3_dev_rx_queue_start(struct rte_eth_dev *dev, uint16_t rx_queue_id) {
  ...
  if (ret) {
    hns3_err(hw, "fail to reset Rx queue %u, ret = %d.", rx_queue_id, ret);
    rte_spinlock_unlock(&hw->lock);
    return ret;
  }
  ...
  if (ret) {
    hns3_err(hw, "fail to init Rx queue %u, ret = %d.", rx_queue_id, ret);
    rte_spinlock_unlock(&hw->lock);
    return ret;
  }
...
static void hns3_reset_sw_rxq(struct hns3_rx_queue *rxq) {
  rxq->next_to_use = 0;
  rxq->rx_rearm_start = 0;
  rxq->rx_free_hold = 0;
  rxq->rx_rearm_nb = 0;
...
int hns3_dev_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  if (ret) {
    hns3_err(hw, "fail to reset Tx queue %u, ret = %d.", tx_queue_id, ret);
    rte_spinlock_unlock(&hw->lock);
    return ret;
  }
...
static int hns3_tx_done_cleanup_full(struct hns3_tx_queue *txq,
                                     uint32_t free_cnt) {
  ...
  for (idx = 0; idx < free_cnt; idx++) {
    if (next_to_clean == next_to_use)
      break;
    if (desc->tx.tp_fe_sc_vld_ra_ri & rte_cpu_to_le_16(BIT(HNS3_TXD_VLD_B)))
      break;
    if (tx_pkt->mbuf != NULL) {
...
static int hns3_monitor_callback(
