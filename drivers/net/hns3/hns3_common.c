...
int hns3_fw_version_get(struct rte_eth_dev *eth_dev, char *fw_version,
  ...
  ret = snprintf(
      fw_version, fw_size, "%lu.%lu.%lu.%lu",
      hns3_get_field(version, HNS3_FW_VERSION_BYTE3_M, HNS3_FW_VERSION_BYTE3_S),
      hns3_get_field(version, HNS3_FW_VERSION_BYTE2_M, HNS3_FW_VERSION_BYTE2_S),
      hns3_get_field(version, HNS3_FW_VERSION_BYTE1_M, HNS3_FW_VERSION_BYTE1_S),
...
int hns3_dev_infos_get(struct rte_eth_dev *eth_dev,
  ...
  info->rx_offload_capa =
      (RTE_ETH_RX_OFFLOAD_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_CKSUM |
       RTE_ETH_RX_OFFLOAD_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_SCTP_CKSUM |
       RTE_ETH_RX_OFFLOAD_OUTER_IPV4_CKSUM |
       RTE_ETH_RX_OFFLOAD_OUTER_UDP_CKSUM | RTE_ETH_RX_OFFLOAD_SCATTER |
       RTE_ETH_RX_OFFLOAD_VLAN_STRIP | RTE_ETH_RX_OFFLOAD_VLAN_FILTER |
       RTE_ETH_RX_OFFLOAD_RSS_HASH);
  info->tx_offload_capa =
      (RTE_ETH_TX_OFFLOAD_OUTER_IPV4_CKSUM | RTE_ETH_TX_OFFLOAD_IPV4_CKSUM |
       RTE_ETH_TX_OFFLOAD_TCP_CKSUM | RTE_ETH_TX_OFFLOAD_UDP_CKSUM |
       RTE_ETH_TX_OFFLOAD_SCTP_CKSUM | RTE_ETH_TX_OFFLOAD_MULTI_SEGS |
...
static int hns3_parse_io_hint_func(const char *key, const char *value,
                                   void *extra_args) {
...
static const char *hns3_get_io_hint_func_name(uint32_t hint) {
  switch (hint) {
...
static int hns3_parse_dev_caps_mask(const char *key, const char *value,
                                    void *extra_args) {
  uint64_t val;
...
static int hns3_parse_mbx_time_limit(const char *key, const char *value,
                                     void *extra_args) {
  uint64_t val;
...
static int hns3_parse_vlan_match_mode(const char *key, const char *value,
                                      void *args) {
  uint8_t mode;
...
void hns3_parse_devargs(struct rte_eth_dev *dev) {
  ...
  if (!hns->is_vf)
    (void)rte_kvargs_process(kvlist, HNS3_DEVARG_FDIR_VALN_MATCH_MODE,
  ...
  if (dev_caps_mask != 0)
    hns3_warn(hw, "parsed %s = 0x%" PRIx64 ".", HNS3_DEVARG_DEV_CAPS_MASK,
              dev_caps_mask);
...
void hns3_ether_format_addr(char *buf, uint16_t size,
  ...
  (void)snprintf(buf, size, "%02X:**:**:**:%02X:%02X",
                 ether_addr->addr_bytes[0], ether_addr->addr_bytes[4],
                 ether_addr->addr_bytes[5]);
}

static int hns3_set_mc_addr_chk_param(struct hns3_hw *hw,
  ...
  if (nb_mc_addr > HNS3_MC_MACADDR_NUM) {
    hns3_err(hw,
             "failed to set mc mac addr, nb_mc_addr(%u) "
  ...
  for (i = 0; i < nb_mc_addr; i++) {
    ...
    if (!rte_is_multicast_ether_addr(addr)) {
      hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, addr);
      hns3_err(hw, "failed to set mc mac addr, addr(%s) invalid.", mac_str);
      return -EINVAL;
    }
     ...
     * and mc_addr_set
     */
    mac_addrs_capa = hns->is_vf ? HNS3_VF_UC_MACADDR_NUM : HNS3_UC_MACADDR_NUM;
    for (j = 0; j < mac_addrs_capa; j++) {
      if (rte_is_same_ether_addr(addr, &hw->data->mac_addrs[j])) {
        hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, addr);
        hns3_err(hw,
                 "failed to set mc mac addr, "
...
int hns3_configure_all_mac_addr(struct hns3_adapter *hns, bool del) {
  ...
  for (i = 0; i < mac_addrs_capa; i++) {
    ...
    if (rte_is_multicast_ether_addr(addr))
      ret =
          del ? ops->del_mc_mac_addr(hw, addr) : ops->add_mc_mac_addr(hw, addr);
    else
      ret =
          del ? ops->del_uc_mac_addr(hw, addr) : ops->add_uc_mac_addr(hw, addr);
...
static bool hns3_find_duplicate_mc_addr(struct hns3_hw *hw,
  ...
  for (i = 0; i < hw->mc_addrs_num; i++) {
    ...
    if (rte_is_same_ether_addr(addr, mc_addr)) {
      hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, addr);
      hns3_err(hw,
               "failed to add mc mac addr, same addrs"
...
int hns3_add_mac_addr(struct rte_eth_dev *dev, struct rte_ether_addr *mac_addr,
  ...
  if (ret) {
    ...
    hns3_err(hw, "failed to add mac addr(%s), ret = %d", mac_str, ret);
  }
...
void hns3_remove_mac_addr(struct rte_eth_dev *dev, uint32_t idx) {
  ...
  if (ret) {
    hns3_ether_format_addr(mac_str, RTE_ETHER_ADDR_FMT_SIZE, mac_addr);
    hns3_err(hw, "failed to remove mac addr(%s), ret = %d", mac_str, ret);
  }
}
...
int hns3_init_mac_addrs(struct rte_eth_dev *dev) {
  ...
  dev->data->mac_addrs = rte_zmalloc(
      memory_name, sizeof(struct rte_ether_addr) * mac_addrs_capa, 0);
  ...
  if (!hns->is_vf) {
    if (!rte_is_valid_assigned_ether_addr(eth_addr)) {
      ...
      hns3_warn(hw,
                "default mac_addr from firmware is an invalid "
...
int hns3_init_ring_with_vector(struct hns3_hw *hw) {
  ...
  for (i = 0; i < hw->intr_tqps_num; i++) {
     ...
     */
    hns3_set_queue_intr_gl(hw, i, HNS3_RING_GL_RX, HNS3_TQP_INTR_GL_DEFAULT);
    ...
    ret = hw->ops.bind_ring_with_vector(hw, vec, false, HNS3_RING_TYPE_TX, i);
    if (ret) {
      PMD_INIT_LOG(ERR, "fail to unbind TX ring(%u) with vector: %u, ret=%d", i,
                   vec, ret);
      return ret;
    }

    ret = hw->ops.bind_ring_with_vector(hw, vec, false, HNS3_RING_TYPE_RX, i);
    if (ret) {
      PMD_INIT_LOG(ERR, "fail to unbind RX ring(%d) with vector: %u, ret=%d", i,
                   vec, ret);
      return ret;
    }
  }
...
int hns3_map_rx_interrupt(struct rte_eth_dev *dev) {
  ...
  if (rte_intr_vec_list_alloc(intr_handle, "intr_vec", hw->used_rx_queues)) {
  ...
  for (q_id = 0; q_id < hw->used_rx_queues; q_id++) {
    ret = hw->ops.bind_ring_with_vector(hw, vec, true, HNS3_RING_TYPE_RX, q_id);
    if (ret)
...
void hns3_unmap_rx_interrupt(struct rte_eth_dev *dev) {
  ...
  if (rte_intr_dp_is_en(intr_handle)) {
    for (q_id = 0; q_id < hw->used_rx_queues; q_id++) {
      (void)hw->ops.bind_ring_with_vector(hw, vec, false, HNS3_RING_TYPE_RX,
                                          q_id);
      if (vec < base + rte_intr_nb_efd_get(intr_handle) - 1)
        vec++;
    }
...
int hns3_restore_rx_interrupt(struct hns3_hw *hw) {
  ...
  if (rte_intr_dp_is_en(intr_handle)) {
    for (q_id = 0; q_id < hw->used_rx_queues; q_id++) {
      ret = hw->ops.bind_ring_with_vector(
          hw, rte_intr_vec_list_index_get(intr_handle, q_id), true,
...
static void hns3_parse_dev_specifications(struct hns3_hw *hw,
...
static int hns3_check_dev_specifications(struct hns3_hw *hw) {
  if (hw->rss_ind_tbl_size == 0 ||
      hw->rss_ind_tbl_size > HNS3_RSS_IND_TBL_SIZE_MAX) {
    hns3_err(hw,
             "the indirection table size obtained (%u) is invalid, and should "
             "not be zero or exceed the maximum(%u)",
             hw->rss_ind_tbl_size, HNS3_RSS_IND_TBL_SIZE_MAX);
    return -EINVAL;
  }
  ...
  if (hw->rss_key_size == 0 || hw->rss_key_size > HNS3_RSS_KEY_SIZE_MAX) {
    hns3_err(hw,
             "the RSS key size obtained (%u) is invalid, and should not be "
             "zero or exceed the maximum(%u)",
  ...
  if (hw->rss_key_size > HNS3_RSS_KEY_SIZE)
    hns3_warn(hw,
              "the RSS key size obtained (%u) is greater than the default key "
              "size (%u)",
              hw->rss_key_size, HNS3_RSS_KEY_SIZE);
