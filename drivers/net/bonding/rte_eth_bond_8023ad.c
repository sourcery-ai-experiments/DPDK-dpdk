...
#define MODE4_DEBUG(fmt, ...)                                                  \
  rte_log(RTE_LOG_DEBUG, bond_logtype, "%6u [Port %u: %s] " fmt,               \
          bond_dbg_get_time_diff_ms(), member_id, __func__, ##__VA_ARGS__)

static uint64_t start_time;

static unsigned bond_dbg_get_time_diff_ms(void) {
  uint64_t now;
...
static void bond_print_lacp(struct lacpdu *l) {
  char a_address[18];
  char p_address[18];
  char a_state[256] = {0};
  char p_state[256] = {0};

  static const char *const state_labels[] = {"ACT", "TIMEOUT", "AGG", "SYNC",
                                             "COL", "DIST",    "DEF", "EXP"};
  ...
  snprintf(a_address, sizeof(a_address), RTE_ETHER_ADDR_PRT_FMT, addr[0],
           addr[1], addr[2], addr[3], addr[4], addr[5]);

  addr = l->partner.port_params.system.addr_bytes;
  snprintf(p_address, sizeof(p_address), RTE_ETHER_ADDR_PRT_FMT, addr[0],
           addr[1], addr[2], addr[3], addr[4], addr[5]);
  ...
  RTE_BOND_LOG(
      ...
      l->subtype, l->version_number, l->actor.tlv_type_info,
      l->actor.info_length, l->actor.port_params.system_priority, a_address,
      l->actor.port_params.key, l->actor.port_params.port_priority,
      l->actor.port_params.port_number, a_state, l->partner.tlv_type_info,
      l->partner.info_length, l->partner.port_params.system_priority, p_address,
      l->partner.port_params.key, l->partner.port_params.port_priority,
      l->partner.port_params.port_number, p_state, l->tlv_type_collector_info,
      l->collector_info_length, l->collector_max_delay, l->tlv_type_terminator,
      l->terminator_length);
}

#define BOND_PRINT_LACP(lacpdu) bond_print_lacp(lacpdu)
#else
#define BOND_PRINT_LACP(lacpdu)                                                \
  do {                                                                         \
  } while (0)
#define MODE4_DEBUG(fmt, ...)                                                  \
  do {                                                                         \
...
static void timer_cancel(uint64_t *timer) { *timer = 0; }

static void timer_set(uint64_t *timer, uint64_t timeout) {
  *timer = rte_rdtsc() + timeout;
}

/* Forces given timer to be in expired state. */
static void timer_force_expired(uint64_t *timer) { *timer = rte_rdtsc(); }

static bool timer_is_stopped(uint64_t *timer) { return *timer == 0; }

static bool timer_is_expired(uint64_t *timer) { return *timer < rte_rdtsc(); }

/* Timer is in running state if it is not stopped nor expired */
static bool timer_is_running(uint64_t *timer) {
  return !timer_is_stopped(timer) && !timer_is_expired(timer);
}

static void set_warning_flags(struct port *port, uint16_t flags) {
  int retval;
  uint16_t old;
  uint16_t new_flag = 0;
...
static void show_warnings(uint16_t member_id) {
  ...
  timer_set(&port->warning_timer,
            BOND_8023AD_WARNINGS_PERIOD_MS * rte_get_tsc_hz() / 1000);
  ...
  if (warnings & WRN_TX_QUEUE_FULL) {
    RTE_BOND_LOG(DEBUG,
                 ...
                 "Receive and transmit functions must be invoked on bonding"
                 "interface at least 10 times per second or LACP will not work "
                 "correctly",
                 member_id);
  }

  if (warnings & WRN_RX_MARKER_TO_FAST)
    RTE_BOND_LOG(INFO, "Member %u: marker to early - ignoring.", member_id);

  if (warnings & WRN_UNKNOWN_SLOW_TYPE) {
    RTE_BOND_LOG(INFO, "Member %u: ignoring unknown slow protocol frame type",
                 member_id);
  }

  if (warnings & WRN_UNKNOWN_MARKER_TYPE)
    RTE_BOND_LOG(INFO, "Member %u: ignoring unknown marker type", member_id);

  if (warnings & WRN_NOT_LACP_CAPABLE)
    MODE4_DEBUG("Port %u is not LACP capable!\n", member_id);
}

static void record_default(struct port *port) {
 ...
 */
static void rx_machine(struct bond_dev_private *internals, uint16_t member_id,
                       struct lacpdu *lacp) {
  ...
  if (lacp) {
    ...
    bool match =
        port->actor.system_priority ==
            lacp->partner.port_params.system_priority &&
        rte_is_same_ether_addr(&agg->actor.system,
                               &lacp->partner.port_params.system) &&
        port->actor.port_priority == lacp->partner.port_params.port_priority &&
        port->actor.port_number == lacp->partner.port_params.port_number;
    ...
    else if (!PARTNER_STATE(port, AGGREGATION) &&
  ...
  if (!timer_is_running(&port->current_while_timer)) {
    if (SM_FLAG(port, EXPIRED)) {
      port->selected = UNSELECTED;
      memcpy(&port->partner, &port->partner_admin, sizeof(struct port_params));
      record_default(port);
      ACTOR_STATE_CLR(port, EXPIRED);
      timer_cancel(&port->current_while_timer);
    } else {
 ...
 */
static void periodic_machine(struct bond_dev_private *internals,
                             uint16_t member_id) {
  struct port *port = &bond_mode_8023ad_ports[member_id];
  /* Calculate if either site is LACP enabled */
  uint64_t timeout;
  uint8_t active =
      ACTOR_STATE(port, LACP_ACTIVE) || PARTNER_STATE(port, LACP_ACTIVE);
 ...
 * @param port			Port to handle state machine.
 */
static void mux_machine(struct bond_dev_private *internals,
                        uint16_t member_id) {
  struct port *port = &bond_mode_8023ad_ports[member_id];

  /* Save current state for later use */
  const uint8_t state_mask =
      STATE_SYNCHRONIZATION | STATE_DISTRIBUTING | STATE_COLLECTING;

  /* Enter DETACHED state on BEGIN condition or from any other state if
   * port was unselected */
  if (SM_FLAG(port, BEGIN) || port->selected == UNSELECTED ||
      (port->selected == STANDBY && (port->actor_state & state_mask) != 0)) {
    ...
    if (SM_FLAG(port, BEGIN) || !timer_is_stopped(&port->wait_while_timer)) {
      SM_FLAG_SET(port, NTT);
  ...
  if (!ACTOR_STATE(port, SYNCHRONIZATION)) {
    ...
    RTE_ASSERT((port->actor_state & (STATE_COLLECTING | STATE_DISTRIBUTING)) ==
               0);
    ...
    if (!ACTOR_STATE(port, DISTRIBUTING)) {
      if (PARTNER_STATE(port, COLLECTING)) {
        ...
        RTE_BOND_LOG(INFO, "Bond %u: member id %u distributing started.",
      ...
      if (!PARTNER_STATE(port, COLLECTING)) {
        ...
        RTE_BOND_LOG(INFO, "Bond %u: member id %u distributing stopped.",
                     internals->port_id, member_id);
      }
    }
  }
 ...
 * @param port
 */
static void tx_machine(struct bond_dev_private *internals, uint16_t member_id) {
  struct port *agg, *port = &bond_mode_8023ad_ports[member_id];
  ...
  memcpy(&hdr->lacpdu.actor.port_params, &port->actor, sizeof(port->actor));
  agg = &bond_mode_8023ad_ports[port->aggregator_port_id];
  ...
  if (internals->mode4.dedicated_queues.enabled == 0) {
    ...
    uint16_t pkts_sent = rte_eth_tx_prepare(
        member_id, internals->mode4.dedicated_queues.tx_qid, &lacp_pkt, 1);
    pkts_sent =
        rte_eth_tx_burst(member_id, internals->mode4.dedicated_queues.tx_qid,
                         &lacp_pkt, pkts_sent);
    if (pkts_sent != 1) {
      rte_pktmbuf_free(lacp_pkt);
      set_warning_flags(port, WRN_TX_QUEUE_FULL);
...
static uint16_t max_index(uint64_t *a, uint16_t n) {
  uint16_t i, max_i = 0;
  uint64_t max = a[0];
 ...
 */
static void selection_logic(struct bond_dev_private *internals,
                            uint16_t member_id) {
  ...
  for (i = 0; i < members_count; ++i) {
    ...
    if ((agg->actor.key == port->actor.key &&
         ...
         rte_is_same_ether_addr(&agg->partner.system, &port->partner.system) ==
             1 &&
         (agg->partner.key == port->partner.key)) &&
        rte_is_zero_ether_addr(&port->partner.system) != 1 &&
        (agg->actor.key & rte_cpu_to_be_16(BOND_LINK_FULL_DUPLEX_KEY)) != 0) {
  ...
  if (new_agg_id != port->aggregator_port_id) {
    ...
    MODE4_DEBUG("-> SELECTED: ID=%3u\n"
                ...
                port->aggregator_port_id == member_id
                    ? "aggregator not found, using default"
                    : "aggregator found",
                port->aggregator_port_id);
  }
...
static uint16_t link_speed_key(uint16_t speed) {
  uint16_t key_speed;
...
static void rx_machine_update(struct bond_dev_private *internals,
                              uint16_t member_id, struct rte_mbuf *lacp_pkt) {
  ...
  if (lacp_pkt != NULL) {
    ...
    if (rte_is_zero_ether_addr(&partner->port_params.system) ||
      ...
      rte_ether_format_addr(preferred_system_name, RTE_ETHER_ADDR_FMT_SIZE,
                            &partner->port_params.system);
      rte_ether_format_addr(self_system_name, RTE_ETHER_ADDR_FMT_SIZE,
                            &agg->actor.system);
...
#define DEDICATED_QUEUE_BURST_SIZE 32
  ...
  uint16_t rx_count =
      rte_eth_rx_burst(member_id, internals->mode4.dedicated_queues.rx_qid,
  ...
  if (rx_count) {
    ...
    for (i = 0; i < rx_count; i++)
      bond_mode_8023ad_handle_slow_pkt(internals, member_id, lacp_pkt[i]);
  } else {
    rx_machine_update(internals, member_id, NULL);
  }
}

static void bond_mode_8023ad_periodic_cb(void *arg) {
  ...
  for (i = 0; i < internals->active_member_count; i++) {
    ...
    if ((port->actor.key & rte_cpu_to_be_16(BOND_LINK_FULL_DUPLEX_KEY)) == 0) {
    ...
    if (internals->mode4.dedicated_queues.enabled == 0) {
       ...
       */
      int retval = rte_ring_dequeue(port->rx_ring, (void **)&lacp_pkt);
      ...
      bond_mode_8023ad_dedicated_rxq_process(internals, member_id);
    }
...
static int bond_mode_8023ad_register_lacp_mac(uint16_t member_id) {
  int ret;

  ret = rte_eth_allmulticast_enable(member_id);
  if (ret != 0) {
    RTE_BOND_LOG(ERR, "failed to enable allmulti mode for port %u: %s",
                 member_id, rte_strerror(-ret));
  }
  if (rte_eth_allmulticast_get(member_id)) {
    RTE_BOND_LOG(DEBUG, "forced allmulti for port %u", member_id);
  ...
  if (ret != 0) {
    RTE_BOND_LOG(ERR, "failed to enable promiscuous mode for port %u: %s",
                 member_id, rte_strerror(-ret));
  }
  if (rte_eth_promiscuous_get(member_id)) {
    RTE_BOND_LOG(DEBUG, "forced promiscuous for port %u", member_id);
    bond_mode_8023ad_ports[member_id].forced_rx_flags =
...
static void bond_mode_8023ad_unregister_lacp_mac(uint16_t member_id) {
  int ret;
  ...
  case BOND_8023AD_FORCED_ALLMULTI:
    ...
    if (ret != 0)
      RTE_BOND_LOG(ERR, "failed to disable allmulti mode for port %u: %s",
                   member_id, rte_strerror(-ret));
    break;
  ...
  case BOND_8023AD_FORCED_PROMISC:
    ...
    if (ret != 0)
      RTE_BOND_LOG(ERR, "failed to disable promiscuous mode for port %u: %s",
                   member_id, rte_strerror(-ret));
    break;
...
void bond_mode_8023ad_activate_member(struct rte_eth_dev *bond_dev,
                                      uint16_t member_id) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
  ...
  RTE_ASSERT(find_member_by_id(internals->active_members,
                               ...
                               member_id) == internals->active_member_count);
  ...
  if (bond_mode_8023ad_register_lacp_mac(member_id) < 0) {
    RTE_BOND_LOG(
        WARNING,
        "member %u is most likely broken and won't receive LACP packets",
        member_id);
  }
  ...
  element_size = sizeof(struct slow_protocol_frame) + RTE_PKTMBUF_HEADROOM;
  ...
  port->mbuf_pool = rte_pktmbuf_pool_create(
      mem_name, total_tx_desc,
      RTE_MEMPOOL_CACHE_MAX_SIZE >= 32 ? 32 : RTE_MEMPOOL_CACHE_MAX_SIZE, 0,
      element_size, socket_id);
  ...
  port->rx_ring = rte_ring_create(
      mem_name, rte_align32pow2(BOND_MODE_8023AX_MEMBER_RX_PKTS), socket_id, 0);
  ...
  port->tx_ring = rte_ring_create(
      mem_name, rte_align32pow2(BOND_MODE_8023AX_MEMBER_TX_PKTS + 1), socket_id,
      0);
...
int bond_mode_8023ad_deactivate_member(
    struct rte_eth_dev *bond_dev __rte_unused, uint16_t member_id) {
  void *pkt = NULL;
  ...
  if (!((old_partner_state ^ port->partner_state) & STATE_LACP_SHORT_TIMEOUT))
    timer_cancel(&port->current_while_timer);
...
void bond_mode_8023ad_mac_address_update(struct rte_eth_dev *bond_dev) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
...
static void bond_mode_8023ad_conf_get(struct rte_eth_dev *dev,
...
void bond_mode_8023ad_setup(struct rte_eth_dev *dev,
                            struct rte_eth_bond_8023ad_conf *conf) {
...
int bond_mode_8023ad_enable(struct rte_eth_dev *bond_dev) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
  uint16_t i;

  for (i = 0; i < internals->active_member_count; i++)
    bond_mode_8023ad_activate_member(bond_dev, internals->active_members[i]);

  return 0;
}

int bond_mode_8023ad_start(struct rte_eth_dev *bond_dev) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
  struct mode8023ad_private *mode4 = &internals->mode4;
  ...
  if (mode4->slowrx_cb)
    return rte_eal_alarm_set(us, &bond_mode_8023ad_ext_periodic_cb, bond_dev);

  return rte_eal_alarm_set(us, &bond_mode_8023ad_periodic_cb, bond_dev);
}

void bond_mode_8023ad_stop(struct rte_eth_dev *bond_dev) {
  struct bond_dev_private *internals = bond_dev->data->dev_private;
  struct mode8023ad_private *mode4 = &internals->mode4;
...
void bond_mode_8023ad_handle_slow_pkt(struct bond_dev_private *internals,
                                      uint16_t member_id,
                                      struct rte_mbuf *pkt) {
  struct mode8023ad_private *mode4 = &internals->mode4;
  struct port *port = &bond_mode_8023ad_ports[member_id];
  ...
  subtype = rte_pktmbuf_mtod(pkt, struct slow_protocol_frame *)
                ->slow_protocol.subtype;
  ...
  if (subtype == SLOW_SUBTYPE_MARKER) {
    ...
    do {
      ...
      retval = rte_atomic64_cmpset(&port->rx_marker_timer, old_marker_timer,
                                   marker_timer);
    } while (unlikely(retval == 0));
    ...
    if (internals->mode4.dedicated_queues.enabled == 0) {
      ...
      uint16_t tx_count = rte_eth_tx_prepare(
          member_id, internals->mode4.dedicated_queues.tx_qid, &pkt, 1);
      tx_count = rte_eth_tx_burst(
          member_id, internals->mode4.dedicated_queues.tx_qid, &pkt, tx_count);
      if (tx_count != 1) {
        /* reset timer */
        port->rx_marker_timer = 0;
        wrn = WRN_TX_QUEUE_FULL;
...
int rte_eth_bond_8023ad_conf_get(uint16_t port_id,
                                 struct rte_eth_bond_8023ad_conf *conf) {
...
int rte_eth_bond_8023ad_agg_selection_set(
    uint16_t port_id, enum rte_bond_8023ad_agg_selection agg_selection) {
  struct rte_eth_dev *bond_dev;
...
static int bond_8023ad_setup_validate(uint16_t port_id,
                                      struct rte_eth_bond_8023ad_conf *conf) {
  if (valid_bonding_port_id(port_id) != 0)
    return -EINVAL;
  ...
  if (conf != NULL) {
    ...
    if (conf->slow_periodic_ms == 0 ||
        ...
        conf->aggregate_wait_timeout_ms == 0 || conf->tx_period_ms == 0 ||
        conf->rx_marker_period_ms == 0 || conf->update_timeout_ms == 0) {
...
int rte_eth_bond_8023ad_setup(uint16_t port_id,
                              struct rte_eth_bond_8023ad_conf *conf) {
  struct rte_eth_dev *bond_dev;
  int err;
...
int rte_eth_bond_8023ad_member_info(
    uint16_t port_id, uint16_t member_id,
...
static int bond_8023ad_ext_validate(uint16_t port_id, uint16_t member_id) {
  struct rte_eth_dev *bond_dev;
  struct bond_dev_private *internals;
  struct mode8023ad_private *mode4;
...
int rte_eth_bond_8023ad_ext_collect(uint16_t port_id, uint16_t member_id,
                                    int enabled) {
  struct port *port;
  int res;
...
int rte_eth_bond_8023ad_ext_distrib(uint16_t port_id, uint16_t member_id,
                                    int enabled) {
  struct port *port;
  int res;
...
int rte_eth_bond_8023ad_ext_distrib_get(uint16_t port_id, uint16_t member_id) {
  struct port *port;
  int err;
...
int rte_eth_bond_8023ad_ext_collect_get(uint16_t port_id, uint16_t member_id) {
  struct port *port;
  int err;
...
int rte_eth_bond_8023ad_ext_slowtx(uint16_t port_id, uint16_t member_id,
...
int rte_eth_bond_8023ad_dedicated_queues_disable(uint16_t port) {
  int retval = 0;
  struct rte_eth_dev *dev;
  struct bond_dev_private *internals;
