...
static inline void ngbe_rxq_rearm(struct ngbe_rx_queue *rxq) {
  int i;
  uint16_t rx_id;
  ...
  if (rte_mempool_get_bulk(rxq->mb_pool, (void *)rxep,
    ...
    if (rxq->rxrearm_nb + RTE_NGBE_RXQ_REARM_THRESH >= rxq->nb_rx_desc) {
...
static inline void desc_to_olflags_v(__m128i descs[4], __m128i mbuf_init,
  ...
  const __m128i vlan_csum_msk = _mm_set_epi16(
      ...
      (NGBE_RXD_ERR_L4CS | NGBE_RXD_ERR_IPCS) >> 16,
      (NGBE_RXD_ERR_L4CS | NGBE_RXD_ERR_IPCS) >> 16,
      (NGBE_RXD_ERR_L4CS | NGBE_RXD_ERR_IPCS) >> 16, NGBE_RXD_STAT_VLAN,
      NGBE_RXD_STAT_VLAN, NGBE_RXD_STAT_VLAN, NGBE_RXD_STAT_VLAN);
...
static inline void desc_to_ptype_v(__m128i descs[4], uint16_t pkt_type_mask,
  ...
  ptype0 =
      _mm_and_si128(_mm_srli_epi32(ptype0, NGBE_RXD_PTID_SHIFT), ptype_mask);
 ...
 */
static inline uint16_t _recv_raw_pkts_vec(struct ngbe_rx_queue *rxq,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts,
  ...
  for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
       pos += RTE_NGBE_DESCS_PER_LOOP, rxdp += RTE_NGBE_DESCS_PER_LOOP) {
 ...
 * - floor align nb_pkts to a RTE_NGBE_DESC_PER_LOOP power-of-two
 */
static uint16_t ngbe_recv_scattered_burst_vec(void *rx_queue,
                                              struct rte_mbuf **rx_pkts,
                                              uint16_t nb_pkts) {
 ...
 * vPMD receive routine that reassembles scattered packets.
 */
uint16_t ngbe_recv_scattered_pkts_vec(void *rx_queue, struct rte_mbuf **rx_pkts,
                                      uint16_t nb_pkts) {
  uint16_t retval = 0;
  ...
  while (nb_pkts > RTE_NGBE_MAX_RX_BURST) {
    ...
    burst = ngbe_recv_scattered_burst_vec(rx_queue, rx_pkts + retval,
  ...
  return retval +
         ngbe_recv_scattered_burst_vec(rx_queue, rx_pkts + retval, nb_pkts);
}

static inline void vtx1(volatile struct ngbe_tx_desc *txdp,
...
static inline void vtx(volatile struct ngbe_tx_desc *txdp,
...
void __rte_cold ngbe_rx_queue_release_mbufs_vec(struct ngbe_rx_queue *rxq) {
  _ngbe_rx_queue_release_mbufs_vec(rxq);
}

static void __rte_cold ngbe_tx_free_swring(struct ngbe_tx_queue *txq) {
  _ngbe_tx_free_swring_vec(txq);
}

static void __rte_cold ngbe_reset_tx_queue(struct ngbe_tx_queue *txq) {
...
int __rte_cold ngbe_rxq_vec_setup(struct ngbe_rx_queue *rxq) {
  return ngbe_rxq_vec_setup_default(rxq);
}

int __rte_cold ngbe_txq_vec_setup(struct ngbe_tx_queue *txq) {
  return ngbe_txq_vec_setup_default(txq, &vec_txq_ops);
}

int __rte_cold ngbe_rx_vec_dev_conf_condition_check(struct rte_eth_dev *dev) {
