...
static inline void desc_to_olflags_v(uint8x16x2_t sterr_tmp1,
  ...
  const uint8x16_t rsstype_msk = {0x0F, 0x0F, 0x0F, 0x0F, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00};

  const uint8x16_t rss_flags = {0,
   ...
   */
  csum = vsetq_lane_u32(vgetq_lane_u32(vreinterpretq_u32_u8(temp_csum), 3),
                        csum, 0);
...
static inline void desc_to_ptype_v(uint64x2_t descs[4], uint16_t pkt_type_mask,
                                   struct rte_mbuf **rx_pkts) {
...
static inline uint16_t _recv_raw_pkts_vec(struct ngbe_rx_queue *rxq,
  ...
  int pos;
  uint8x16_t shuf_msk = {
      0xFF, 0xFF, 0xFF, 0xFF, /* skip 32 bits pkt_type */
  ...
  for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
    ...
    staterr = vreinterpretq_u8_s8(
        vshrq_n_s8(vreinterpretq_s8_u8(staterr), NGBE_UINT8_BIT - 1));
    stat = ~vgetq_lane_u32(vreinterpretq_u32_u8(staterr), 0);
