...
static const struct reg_info ngbe_regs_nvm[] = {{0, 0, 0, ""}};

static const struct reg_info ngbe_regs_interrupt[] = {{0, 0, 0, ""}};

static const struct reg_info ngbe_regs_fctl_others[] = {{0, 0, 0, ""}};

static const struct reg_info ngbe_regs_rxdma[] = {{0, 0, 0, ""}};

static const struct reg_info ngbe_regs_rx[] = {{0, 0, 0, ""}};

static struct reg_info ngbe_regs_tx[] = {{0, 0, 0, ""}};

static const struct reg_info ngbe_regs_wakeup[] = {{0, 0, 0, ""}};

static const struct reg_info ngbe_regs_mac[] = {{0, 0, 0, ""}};
...
/* PF registers */
static const struct reg_info *ngbe_regs_others[] = {ngbe_regs_general,
                                                    ngbe_regs_nvm,
                                                    ngbe_regs_interrupt,
...
static int ngbe_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
#define HW_XSTAT(m)                                                            \
  { #m, offsetof(struct ngbe_hw_stats, m) }
#define HW_XSTAT_NAME(m, n)                                                    \
  { n, offsetof(struct ngbe_hw_stats, m) }
...
#define NGBE_NB_HW_STATS                                                       \
  (sizeof(rte_ngbe_stats_strings) / sizeof(rte_ngbe_stats_strings[0]))

/* Per-queue statistics */
#define QP_XSTAT(m)                                                            \
  { #m, offsetof(struct ngbe_hw_stats, qp[0].m) }
...
#define NGBE_NB_QP_STATS                                                       \
  (sizeof(rte_ngbe_qp_strings) / sizeof(rte_ngbe_qp_strings[0]))

static inline int32_t ngbe_pf_reset_hw(struct ngbe_hw *hw) {
  uint32_t ctrl_ext;
  int32_t status;
...
static inline void ngbe_enable_intr(struct rte_eth_dev *dev) {
...
static void ngbe_disable_intr(struct ngbe_hw *hw) {
  PMD_INIT_FUNC_TRACE();
 ...
 */
static void ngbe_swfw_lock_reset(struct ngbe_hw *hw) {
  uint16_t mask;
   ...
   * semaphore.
   */
  mask = NGBE_MNGSEM_SWPHY | NGBE_MNGSEM_SWMBX | NGBE_MNGSEM_SWFLASH;
...
static int eth_ngbe_dev_init(struct rte_eth_dev *eth_dev,
  ...
  mz = rte_eth_dma_zone_reserve(eth_dev, "ngbe_driver", -1, NGBE_ISB_SIZE,
                                NGBE_ALIGN, SOCKET_ID_ANY);
  if (mz == NULL)
    return -ENOMEM;
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("ngbe", RTE_ETHER_ADDR_LEN * hw->mac.num_rar_entries, 0);
  ...
  eth_dev->data->hash_mac_addrs =
      rte_zmalloc("ngbe", RTE_ETHER_ADDR_LEN * NGBE_VMDQ_NUM_UC_MAC, 0);
  ...
  PMD_INIT_LOG(DEBUG, "MAC: %d, PHY: %d", (int)hw->mac.type, (int)hw->phy.type);

  PMD_INIT_LOG(DEBUG, "port %d vendorID=0x%x deviceID=0x%x",
               eth_dev->data->port_id, pci_dev->id.vendor_id,
               pci_dev->id.device_id);

  rte_intr_callback_register(intr_handle, ngbe_dev_interrupt_handler, eth_dev);
...
static int eth_ngbe_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                              struct rte_pci_device *pci_dev) {
  return rte_eth_dev_create(
      &pci_dev->device, pci_dev->device.name, sizeof(struct ngbe_adapter),
      eth_dev_pci_specific_init, pci_dev, eth_ngbe_dev_init, NULL);
}
...
static int ngbe_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                int on) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
...
static void ngbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue,
  ...
  if (rxq->offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP) {
    restart = (rxcfg & NGBE_RXCFG_ENA) && !(rxcfg & NGBE_RXCFG_VLAN);
...
static int ngbe_vlan_tpid_set(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_VLAN_TYPE_INNER:
    if (vlan_ext) {
      wr32m(hw, NGBE_VLANCTL, NGBE_VLANCTL_TPID_MASK, NGBE_VLANCTL_TPID(tpid));
      wr32m(hw, NGBE_DMATXCTRL, NGBE_DMATXCTRL_TPID_MASK,
            NGBE_DMATXCTRL_TPID(tpid));
    } else {
      ret = -ENOTSUP;
      PMD_DRV_LOG(ERR, "Inner type is not supported by single VLAN");
    }

    if (qinq) {
      wr32m(hw, NGBE_TAGTPID(0), NGBE_TAGTPID_LSB_MASK, NGBE_TAGTPID_LSB(tpid));
    }
    break;
  case RTE_ETH_VLAN_TYPE_OUTER:
    if (vlan_ext) {
      /* Only the high 16-bits is valid */
      wr32m(hw, NGBE_EXTAG, NGBE_EXTAG_VLAN_MASK, NGBE_EXTAG_VLAN(tpid));
    } else {
      wr32m(hw, NGBE_VLANCTL, NGBE_VLANCTL_TPID_MASK, NGBE_VLANCTL_TPID(tpid));
      wr32m(hw, NGBE_DMATXCTRL, NGBE_DMATXCTRL_TPID_MASK,
            NGBE_DMATXCTRL_TPID(tpid));
    ...
    if (qinq) {
      wr32m(hw, NGBE_TAGTPID(0), NGBE_TAGTPID_MSB_MASK, NGBE_TAGTPID_MSB(tpid));
    }
    break;
  default:
...
void ngbe_vlan_hw_strip_bitmap_set(struct rte_eth_dev *dev, uint16_t queue,
                                   bool on) {
...
static void ngbe_qinq_hw_strip_disable(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  uint32_t ctrl;
...
static void ngbe_qinq_hw_strip_enable(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  uint32_t ctrl;
...
void ngbe_config_vlan_strip_on_all_queues(struct rte_eth_dev *dev, int mask) {
  uint16_t i;
...
static void ngbe_dev_phy_intr_setup(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct ngbe_interrupt *intr = ngbe_dev_intr(dev);
...
static int ngbe_dev_start(struct rte_eth_dev *dev) {
  ...
  if (rte_intr_dp_is_en(intr_handle)) {
    if (rte_intr_vec_list_alloc(intr_handle, "intr_vec",
      ...
      PMD_INIT_LOG(ERR, "Failed to allocate %d rx_queues intr_vec",
                   dev->data->nb_rx_queues);
      return -ENOMEM;
    }
  }
...
skip_link_setup:
  ...
  if (rte_intr_allow_others(intr_handle)) {
    ...
    rte_intr_callback_unregister(intr_handle, ngbe_dev_interrupt_handler, dev);
    if (dev->data->dev_conf.intr_conf.lsc != 0)
 ...
 */
static int ngbe_dev_stop(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_allow_others(intr_handle))
    ...
    rte_intr_callback_register(intr_handle, ngbe_dev_interrupt_handler,
                               (void *)dev);
...
static int ngbe_dev_close(struct rte_eth_dev *dev) {
  ...
  do {
    ret = rte_intr_callback_unregister(intr_handle, ngbe_dev_interrupt_handler,
                                       dev);
    if (ret >= 0 || ret == -ENOENT) {
      break;
    } else if (ret != -EAGAIN) {
 ...
 * Reset PF device.
 */
static int ngbe_dev_reset(struct rte_eth_dev *dev) {
  int ret;
...
void ngbe_read_stats_registers(struct ngbe_hw *hw,
  ...
  for (i = 0; i < hw->nb_rx_queues; i++) {
    UPDATE_QP_COUNTER_32bit(NGBE_QPRXPKT(i), hw->qp_last[i].rx_qp_packets,
    ...
    UPDATE_QP_COUNTER_32bit(NGBE_QPRXMPKT(i), hw->qp_last[i].rx_qp_mc_packets,
                            hw_stats->qp[i].rx_qp_mc_packets);
    UPDATE_QP_COUNTER_32bit(NGBE_QPRXBPKT(i), hw->qp_last[i].rx_qp_bc_packets,
                            hw_stats->qp[i].rx_qp_bc_packets);
  }

  for (i = 0; i < hw->nb_tx_queues; i++) {
    UPDATE_QP_COUNTER_32bit(NGBE_QPTXPKT(i), hw->qp_last[i].tx_qp_packets,
    ...
    UPDATE_QP_COUNTER_32bit(NGBE_QPTXMPKT(i), hw->qp_last[i].tx_qp_mc_packets,
                            hw_stats->qp[i].tx_qp_mc_packets);
    UPDATE_QP_COUNTER_32bit(NGBE_QPTXBPKT(i), hw->qp_last[i].tx_qp_bc_packets,
  ...
  hw_stats->tx_macsec_pkts_encrypted += rd32(hw, NGBE_LSECTX_ENCPKT);
  hw_stats->tx_macsec_pkts_protected += rd32(hw, NGBE_LSECTX_PROTPKT);
  hw_stats->tx_macsec_octets_encrypted += rd32(hw, NGBE_LSECTX_ENCOCT);
  hw_stats->tx_macsec_octets_protected += rd32(hw, NGBE_LSECTX_PROTOCT);
  ...
  for (i = 0; i < 2; i++) {
    hw_stats->rx_macsec_sa_pkts_ok += rd32(hw, NGBE_LSECRX_OKPKT(i));
    hw_stats->rx_macsec_sa_pkts_invalid += rd32(hw, NGBE_LSECRX_INVPKT(i));
    hw_stats->rx_macsec_sa_pkts_notvalid += rd32(hw, NGBE_LSECRX_BADPKT(i));
  }
  for (i = 0; i < 4; i++) {
    hw_stats->rx_macsec_sa_pkts_unusedsa += rd32(hw, NGBE_LSECRX_INVSAPKT(i));
    hw_stats->rx_macsec_sa_pkts_notusingsa += rd32(hw, NGBE_LSECRX_BADSAPKT(i));
  }
  hw_stats->rx_total_missed_packets = hw_stats->rx_up_dropped;
}
...
static int ngbe_dev_stats_get(struct rte_eth_dev *dev,
  ...
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct ngbe_hw_stats *hw_stats = NGBE_DEV_STATS(dev);
  struct ngbe_stat_mappings *stat_mappings = NGBE_DEV_STAT_MAPPINGS(dev);
  uint32_t i, j;
  ...
  stats->ierrors = hw_stats->rx_crc_errors +
                   ...
                   hw_stats->rdb_drp_cnt + hw_stats->rx_illegal_byte_errors +
...
static unsigned ngbe_xstats_calc_num(struct rte_eth_dev *dev) {
  int nb_queues = max(dev->data->nb_rx_queues, dev->data->nb_tx_queues);
  return NGBE_NB_HW_STATS + NGBE_NB_QP_STATS * nb_queues;
}

static inline int ngbe_get_name_by_id(uint32_t id, char *name, uint32_t size) {
  int nb, st;

  /* Extended stats from ngbe_hw_stats */
  if (id < NGBE_NB_HW_STATS) {
    snprintf(name, size, "[hw]%s", rte_ngbe_stats_strings[id].name);
    return 0;
  }
  id -= NGBE_NB_HW_STATS;
  ...
  if (id < NGBE_NB_QP_STATS * NGBE_MAX_QP) {
    ...
    snprintf(name, size, "[q%u]%s", nb, rte_ngbe_qp_strings[st].name);
    return 0;
  }
...
static inline int ngbe_get_offset_by_id(uint32_t id, uint32_t *offset) {
  int nb, st;
...
static int
ngbe_dev_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                                struct rte_eth_xstat_name *xstats_names,
                                unsigned int limit) {
  unsigned int i;
...
static int ngbe_dev_xstats_get_(struct rte_eth_dev *dev, uint64_t *values,
                                unsigned int limit) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct ngbe_hw_stats *hw_stats = NGBE_DEV_STATS(dev);
  unsigned int i, count;
...
static int ngbe_dev_xstats_get_by_id(struct rte_eth_dev *dev,
...
static int ngbe_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                               size_t fw_size) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  int ret;
...
static int ngbe_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      (ngbe_get_rx_port_offloads(dev) | dev_info->rx_queue_offload_capa);
  ...
  dev_info->speed_capa =
      RTE_ETH_LINK_SPEED_1G | RTE_ETH_LINK_SPEED_100M | RTE_ETH_LINK_SPEED_10M;
...
static void ngbe_dev_overheat(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  s32 temp_state;
...
int ngbe_dev_link_update_share(struct rte_eth_dev *dev, int wait_to_complete) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct rte_eth_link link;
  ...
  link.link_autoneg =
      !(dev->data->dev_conf.link_speeds & ~RTE_ETH_LINK_SPEED_AUTONEG);
  ...
  if (hw->is_pf) {
    ...
    if (link_speed & (NGBE_LINK_SPEED_1GB_FULL | NGBE_LINK_SPEED_100M_FULL |
                      NGBE_LINK_SPEED_10M_FULL)) {
      wr32m(hw, NGBE_MACTXCFG, NGBE_MACTXCFG_SPEED_MASK,
            NGBE_MACTXCFG_SPEED_1G | NGBE_MACTXCFG_TE);
    }
    wr32m(hw, NGBE_MACRXFLT, NGBE_MACRXFLT_PROMISC, NGBE_MACRXFLT_PROMISC);
  }
...
static int ngbe_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  uint32_t fctrl;
...
static int ngbe_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  uint32_t fctrl;
...
static int ngbe_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  enum ngbe_fc_mode rte_fcmode_2_ngbe_fcmode[] = {
      ngbe_fc_none, ngbe_fc_rx_pause, ngbe_fc_tx_pause, ngbe_fc_full};
  ...
  if (err == 0 || err == NGBE_ERR_FC_NOT_NEGOTIATED) {
    wr32m(hw, NGBE_MACRXFLT, NGBE_MACRXFLT_CTL_MASK,
          (fc_conf->mac_ctrl_frame_fwd ? NGBE_MACRXFLT_CTL_NOPS
                                       : NGBE_MACRXFLT_CTL_DROP));
    ngbe_flush(hw);
 ...
 */
static s32 ngbe_fc_hpbthresh_set(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
   ...
   */
  if (marker < 0) {
    PMD_DRV_LOG(WARNING, "Packet Buffer can not provide enough headroom to "
 ...
 * @dv_id: device interface delay
 */
static s32 ngbe_fc_lpbthresh_set(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
...
static int ngbe_add_rar(struct rte_eth_dev *dev,
                        struct rte_ether_addr *mac_addr, uint32_t index,
                        uint32_t pool) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  uint32_t enable_addr = 1;

  return ngbe_set_rar(hw, index, mac_addr->addr_bytes, pool, enable_addr);
}
...
static int ngbe_set_default_mac_addr(struct rte_eth_dev *dev,
...
static int ngbe_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  ...
  wr32m(hw, NGBE_FRMSZ, NGBE_FRMSZ_MAX_MASK, NGBE_FRMSZ_MAX(frame_size));

  return 0;
}

static uint32_t ngbe_uta_vector(struct ngbe_hw *hw,
...
static int ngbe_uc_hash_table_set(struct rte_eth_dev *dev,
 ...
 */
void ngbe_set_ivar_map(struct ngbe_hw *hw, int8_t direction, uint8_t queue,
                       uint8_t msix_vector) {
  uint32_t tmp, idx;
...
static void ngbe_configure_msix(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_dp_is_en(intr_handle) && !(gpie & NGBE_GPIE_MSIX))
    return;
  ...
  wr32(hw, NGBE_ITR(NGBE_MISC_VEC_ID),
       NGBE_ITR_IVAL_1G(NGBE_QUEUE_ITR_INTERVAL_DEFAULT) | NGBE_ITR_WRDSA);
}

static u8 *ngbe_dev_addr_list_itr(__rte_unused struct ngbe_hw *hw,
...
static uint64_t ngbe_read_systime_cyclecounter(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
...
static uint64_t ngbe_read_rx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
...
static uint64_t ngbe_read_tx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
...
static void ngbe_start_timecounters(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  struct ngbe_adapter *adapter = ngbe_dev_adapter(dev);
  uint32_t incval = 0;
  uint32_t shift = 0;
...
static int ngbe_get_reg_length(struct rte_eth_dev *dev __rte_unused) {
  int count = 0;
  int g_ind = 0;
...
static int ngbe_get_regs(struct rte_eth_dev *dev,
  ...
  if (regs->length == 0 || regs->length == (uint32_t)ngbe_get_reg_length(dev)) {
    regs->version = hw->mac.type << 24 | hw->revision_id << 16 | hw->device_id;
    while ((reg_group = reg_set[g_ind++]))
      count += ngbe_read_regs_group(dev, &data[count], reg_group);
    return 0;
  }
