...
static const u64 NGBE_TX_OFFLOAD_MASK =
    (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IPV4 |
 ...
 */
static __rte_always_inline int ngbe_tx_free_bufs(struct ngbe_tx_queue *txq) {
  ...
  if (!(status & rte_cpu_to_le_32(NGBE_TXD_DD))) {
    if (txq->nb_tx_free >> 1 < txq->tx_free_thresh)
      ngbe_set32_masked(txq->tdc_reg_addr, NGBE_TXCFG_FLUSH, NGBE_TXCFG_FLUSH);
    return 0;
  }
...
static inline void tx4(volatile struct ngbe_tx_desc *txdp,
  ...
  for (i = 0; i < 4; ++i, ++txdp, ++pkts) {
    ...
    txdp->dw2 = cpu_to_le32(NGBE_TXD_FLAGS | NGBE_TXD_DATLEN(pkt_len));
...
static inline void tx1(volatile struct ngbe_tx_desc *txdp,
                       struct rte_mbuf **pkts) {
  uint64_t buf_dma_addr;
  uint32_t pkt_len;
 ...
 */
static inline void ngbe_tx_fill_hw_ring(struct ngbe_tx_queue *txq,
                                        struct rte_mbuf **pkts,
                                        uint16_t nb_pkts) {
...
static uint16_t ngbe_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
                                   uint16_t nb_pkts) {
  struct ngbe_tx_queue *txq = (struct ngbe_tx_queue *)tx_queue;
  uint16_t nb_tx = 0;
...
static inline void ngbe_set_xmit_ctx(struct ngbe_tx_queue *txq,
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    case RTE_MBUF_F_TX_UDP_CKSUM:
      mss_l4len_idx |= NGBE_TXD_L4LEN(sizeof(struct rte_udp_hdr));
      tx_offload_mask.l2_len |= ~0;
      tx_offload_mask.l3_len |= ~0;
      break;
    case RTE_MBUF_F_TX_TCP_CKSUM:
      mss_l4len_idx |= NGBE_TXD_L4LEN(sizeof(struct rte_tcp_hdr));
      tx_offload_mask.l2_len |= ~0;
      tx_offload_mask.l3_len |= ~0;
      break;
    case RTE_MBUF_F_TX_SCTP_CKSUM:
      mss_l4len_idx |= NGBE_TXD_L4LEN(sizeof(struct rte_sctp_hdr));
      tx_offload_mask.l2_len |= ~0;
      tx_offload_mask.l3_len |= ~0;
      break;
    default:
 ...
 */
static inline uint32_t what_ctx_update(struct ngbe_tx_queue *txq,
                                       uint64_t flags,
                                       union ngbe_tx_offload tx_offload) {
...
static inline int ngbe_xmit_cleanup(struct ngbe_tx_queue *txq) {
  ...
  PMD_TX_LOG(DEBUG,
             ...
             nb_tx_to_clean, last_desc_cleaned, desc_to_clean_to, txq->port_id,
             txq->queue_id);
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (nb_used > txq->nb_tx_free) {
      ...
      if (unlikely(nb_used > txq->tx_free_thresh)) {
        PMD_TX_LOG(DEBUG,
                   ...
                   "tx_free_thresh=%4u. "
                   "(port=%d queue=%d)",
                   nb_used, txq->nb_tx_free, txq->tx_free_thresh, txq->port_id,
                   txq->queue_id);
        /*
    ...
    if (tx_ol_req) {
      ...
      if (new_ctx) {
        ...
        ctx_txd = (volatile struct ngbe_tx_ctx_desc *)&txr[tx_id];
        ...
        ngbe_set_xmit_ctx(txq, ctx_txd, tx_ol_req, tx_offload);
 ...
 *
 **********************************************************************/
static inline uint32_t ngbe_rxd_pkt_info_to_pkt_type(uint32_t pkt_info,
                                                     uint16_t ptid_mask) {
...
static inline uint64_t ngbe_rxd_pkt_info_to_pkt_flags(uint32_t pkt_info) {
...
static inline uint64_t rx_desc_error_to_pkt_flags(uint32_t rx_status) {
  ...
  if (rx_status & NGBE_RXD_STAT_IPCS)
    pkt_flags |= (rx_status & NGBE_RXD_ERR_IPCS ? RTE_MBUF_F_RX_IP_CKSUM_BAD
                                                : RTE_MBUF_F_RX_IP_CKSUM_GOOD);

  if (rx_status & NGBE_RXD_STAT_L4CS)
    pkt_flags |= (rx_status & NGBE_RXD_ERR_L4CS ? RTE_MBUF_F_RX_L4_CKSUM_BAD
                                                : RTE_MBUF_F_RX_L4_CKSUM_GOOD);

  if (rx_status & NGBE_RXD_STAT_EIPCS && rx_status & NGBE_RXD_ERR_EIPCS)
...
#endif
static inline int ngbe_rx_scan_hw_ring(struct ngbe_rx_queue *rxq) {
  ...
  for (i = 0; i < RTE_PMD_NGBE_RX_MAX_BURST;
    ...
    for (nb_dd = 0; nb_dd < LOOK_AHEAD && (s[nb_dd] & NGBE_RXD_STAT_DD);
         nb_dd++)
      ;
    ...
    for (j = 0; j < nb_dd; ++j) {
      ...
      pkt_flags |= ngbe_rxd_pkt_info_to_pkt_flags(pkt_info[j]);
      mb->ol_flags = pkt_flags;
      mb->packet_type =
          ngbe_rxd_pkt_info_to_pkt_type(pkt_info[j], NGBE_PTID_MASK);
...
static inline int ngbe_rx_alloc_bufs(struct ngbe_rx_queue *rxq,
                                     bool reset_mbuf) {
  volatile struct ngbe_rx_desc *rxdp;
  struct ngbe_rx_entry *rxep;
  struct rte_mbuf *mb;
...
static inline uint16_t ngbe_rx_fill_from_stage(struct ngbe_rx_queue *rxq,
...
ngbe_rx_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, uint16_t nb_pkts) {
...
uint16_t ngbe_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    rxm->packet_type = ngbe_rxd_pkt_info_to_pkt_type(pkt_info, NGBE_PTID_MASK);
  ...
  if (nb_hold > rxq->rx_free_thresh) {
    PMD_RX_LOG(DEBUG, "port_id=%u queue_id=%u rx_tail=%u nb_hold=%u nb_rx=%u",
               (uint16_t)rxq->port_id, (uint16_t)rxq->queue_id, (uint16_t)rx_id,
 ...
 */
static inline void ngbe_fill_cluster_head_buf(struct rte_mbuf *head,
  ...
  head->packet_type = ngbe_rxd_pkt_info_to_pkt_type(pkt_info, NGBE_PTID_MASK);
 ...
 * receive" interface).
 */
static inline uint16_t ngbe_recv_pkts_sc(void *rx_queue,
                                         struct rte_mbuf **rx_pkts,
  ...
  next_desc:
    ...
    if (!bulk_alloc) {
      ...
      if (!ngbe_rx_alloc_bufs(rxq, false)) {
        rte_wmb();
        ngbe_set32_relaxed(rxq->rdt_reg_addr, next_rdt);
...
uint16_t ngbe_recv_pkts_sc_bulk_alloc(void *rx_queue, struct rte_mbuf **rx_pkts,
                                      uint16_t nb_pkts) {
...
static void ngbe_tx_queue_release_mbufs(struct ngbe_tx_queue *txq) {
  unsigned int i;
...
static int ngbe_tx_done_cleanup_full(struct ngbe_tx_queue *txq,
                                     uint32_t free_cnt) {
...
static int ngbe_tx_done_cleanup_simple(struct ngbe_tx_queue *txq,
                                       uint32_t free_cnt) {
  int i, n, cnt;
...
int ngbe_dev_tx_done_cleanup(void *tx_queue, uint32_t free_cnt) {
  struct ngbe_tx_queue *txq = (struct ngbe_tx_queue *)tx_queue;
  if (txq->offloads == 0 && txq->tx_free_thresh >= RTE_PMD_NGBE_TX_MAX_BURST)
    return ngbe_tx_done_cleanup_simple(txq, free_cnt);

  return ngbe_tx_done_cleanup_full(txq, free_cnt);
}

static void ngbe_tx_free_swring(struct ngbe_tx_queue *txq) {
  if (txq != NULL)
    rte_free(txq->sw_ring);
}

static void ngbe_tx_queue_release(struct ngbe_tx_queue *txq) {
  if (txq != NULL) {
    if (txq->ops != NULL) {
...
static void ngbe_reset_tx_queue(struct ngbe_tx_queue *txq) {
...
int ngbe_tx_burst_mode_get(struct rte_eth_dev *dev,
...
uint64_t ngbe_get_tx_port_offloads(struct rte_eth_dev *dev) {
  ...
  tx_offload_capa =
      ...
      RTE_ETH_TX_OFFLOAD_UDP_TSO | RTE_ETH_TX_OFFLOAD_MULTI_SEGS;
...
int ngbe_dev_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (tx_free_thresh >= (nb_desc - 3)) {
    PMD_INIT_LOG(ERR,
                 ...
                 "descriptors minus 3. (tx_free_thresh=%u port=%d queue=%d)",
  ...
  if (nb_desc % tx_free_thresh != 0) {
    PMD_INIT_LOG(ERR,
                 "tx_free_thresh must be a divisor of the number of Tx "
  ...
  tz = rte_eth_dma_zone_reserve(
      ...
      sizeof(struct ngbe_tx_desc) * NGBE_RING_DESC_MAX, NGBE_ALIGN, socket_id);
  if (tz == NULL) {
  ...
  txq->sw_ring =
      rte_zmalloc_socket("txq->sw_ring", sizeof(struct ngbe_tx_entry) * nb_desc,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (txq->sw_ring == NULL) {
    ngbe_tx_queue_release(txq);
    return -ENOMEM;
 ...
 */
static void ngbe_free_sc_cluster(struct rte_mbuf *m) {
...
static void ngbe_rx_queue_release_mbufs(struct ngbe_rx_queue *rxq) {
  unsigned int i;
...
static void ngbe_rx_queue_release(struct ngbe_rx_queue *rxq) {
  if (rxq != NULL) {
...
check_rx_burst_bulk_alloc_preconditions(struct ngbe_rx_queue *rxq) {
  ...
  if (rxq->rx_free_thresh < RTE_PMD_NGBE_RX_MAX_BURST) {
    PMD_INIT_LOG(DEBUG,
                 "Rx Burst Bulk Alloc Preconditions: rxq->rx_free_thresh=%d, "
    ...
    PMD_INIT_LOG(DEBUG,
                 "Rx Burst Bulk Alloc Preconditions: rxq->nb_rx_desc=%d, "
...
static void ngbe_reset_rx_queue(struct ngbe_adapter *adapter,
                                struct ngbe_rx_queue *rxq) {
...
uint64_t ngbe_get_rx_queue_offloads(struct rte_eth_dev *dev __rte_unused) {
...
uint64_t ngbe_get_rx_port_offloads(struct rte_eth_dev *dev) {
  ...
  if (hw->is_pf)
    offloads |=
        (RTE_ETH_RX_OFFLOAD_QINQ_STRIP | RTE_ETH_RX_OFFLOAD_VLAN_EXTEND);

  return offloads;
}

int ngbe_dev_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
                            uint16_t nb_desc, unsigned int socket_id,
  ...
  if (check_rx_burst_bulk_alloc_preconditions(rxq)) {
    PMD_INIT_LOG(DEBUG,
                 "queue[%d] doesn't meet Rx Bulk Alloc preconditions - "
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("rxq->sw_ring", sizeof(struct ngbe_rx_entry) * len,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->sw_ring == NULL) {
    ngbe_rx_queue_release(rxq);
    return -ENOMEM;
  ...
  rxq->sw_sc_ring = rte_zmalloc_socket(
      "rxq->sw_sc_ring", sizeof(struct ngbe_scattered_rx_entry) * len,
      RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->sw_sc_ring == NULL) {
    ngbe_rx_queue_release(rxq);
    return -ENOMEM;
  ...
  PMD_INIT_LOG(DEBUG, "sw_ring=%p sw_sc_ring=%p hw_ring=%p dma_addr=0x%" PRIx64,
...
#define NGBE_RXQ_SCAN_INTERVAL 4
  ...
  while ((desc < rxq->nb_rx_desc) &&
         (rxdp->qw1.lo.status & rte_cpu_to_le_32(NGBE_RXD_STAT_DD))) {
...
static void ngbe_rss_disable(struct rte_eth_dev *dev) {
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
...
int ngbe_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV6_TCP || rss_hf & RTE_ETH_RSS_IPV6_TCP_EX)
    mrqc |= NGBE_RACTL_RSSIPV6TCP;
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV4_UDP)
    mrqc |= NGBE_RACTL_RSSIPV4UDP;
  if (rss_hf & RTE_ETH_RSS_NONFRAG_IPV6_UDP || rss_hf & RTE_ETH_RSS_IPV6_UDP_EX)
...
static void ngbe_rss_configure(struct rte_eth_dev *dev) {
  struct rte_eth_rss_conf rss_conf;
  struct ngbe_adapter *adapter = ngbe_dev_adapter(dev);
  struct ngbe_hw *hw = ngbe_dev_hw(dev);
  uint32_t reta;
...
void ngbe_configure_port(struct rte_eth_dev *dev) {
  ...
  wr32m(hw, NGBE_VLANCTL, NGBE_VLANCTL_TPID_MASK,
        NGBE_VLANCTL_TPID(RTE_ETHER_TYPE_VLAN));
  wr32m(hw, NGBE_DMATXCTRL, NGBE_DMATXCTRL_TPID_MASK,
        NGBE_DMATXCTRL_TPID(RTE_ETHER_TYPE_VLAN));

  /* default vlan tpid filters */
  for (i = 0; i < 8; i++) {
    wr32m(hw, NGBE_TAGTPID(i / 2),
          (i % 2 ? NGBE_TAGTPID_MSB_MASK : NGBE_TAGTPID_LSB_MASK),
          (i % 2 ? NGBE_TAGTPID_MSB(tpids[i]) : NGBE_TAGTPID_LSB(tpids[i])));
  }
}

static int ngbe_alloc_rx_queue_mbufs(struct ngbe_rx_queue *rxq) {
  ...
  for (i = 0; i < rxq->nb_rx_desc; i++) {
    ...
    if (mbuf == NULL) {
      PMD_INIT_LOG(ERR, "Rx mbuf alloc failed queue_id=%u port_id=%u",
                   (unsigned int)rxq->queue_id, (unsigned int)rxq->port_id);
      return -ENOMEM;
    }
...
static int ngbe_dev_mq_rx_configure(struct rte_eth_dev *dev) {
...
void ngbe_set_rx_function(struct rte_eth_dev *dev) {
  ...
  if (ngbe_rx_vec_dev_conf_condition_check(dev) ||
      ...
      rte_vect_get_max_simd_bitwidth() < RTE_VECT_SIMD_128) {
    PMD_INIT_LOG(DEBUG, "Port[%d] doesn't meet Vector Rx preconditions",
                 dev->data->port_id);
    adapter->rx_vec_allowed = false;
  }
  ...
  if (dev->data->scattered_rx) {
    ...
    if (adapter->rx_vec_allowed) {
      PMD_INIT_LOG(DEBUG, "Using Vector Scattered Rx callback (port=%d).",
                   dev->data->port_id);
    ...
    PMD_INIT_LOG(DEBUG,
                 ...
                 RTE_NGBE_DESCS_PER_LOOP, dev->data->port_id);
...
int ngbe_rx_burst_mode_get(struct rte_eth_dev *dev,
                           __rte_unused uint16_t queue_id,
                           struct rte_eth_burst_mode *mode) {
...
int ngbe_dev_rx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    wr32(hw, NGBE_RXBAL(rxq->reg_idx), (uint32_t)(bus_addr & BIT_MASK32));
    wr32(hw, NGBE_RXBAH(rxq->reg_idx), (uint32_t)(bus_addr >> 32));
    ...
    if (dev->data->mtu + NGBE_ETH_OVERHEAD + 2 * RTE_VLAN_HLEN > buf_size)
...
void ngbe_dev_tx_init(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    wr32(hw, NGBE_TXBAL(txq->reg_idx), (uint32_t)(bus_addr & BIT_MASK32));
    wr32(hw, NGBE_TXBAH(txq->reg_idx), (uint32_t)(bus_addr >> 32));
 ...
 */
static inline void ngbe_setup_loopback_link(struct ngbe_hw *hw) {
  PMD_INIT_FUNC_TRACE();
...
int ngbe_dev_rxtx_start(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    wr32m(hw, NGBE_TXCFG(txq->reg_idx),
          NGBE_TXCFG_HTHRESH_MASK | NGBE_TXCFG_WTHRESH_MASK,
          NGBE_TXCFG_HTHRESH(txq->hthresh) | NGBE_TXCFG_WTHRESH(txq->wthresh));
  }
...
int ngbe_dev_tx_queue_start(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  if (poll_ms == 0)
    PMD_INIT_LOG(ERR, "Could not enable Tx Queue %d", tx_queue_id);
...
int ngbe_rx_vec_dev_conf_condition_check(__rte_unused struct rte_eth_dev *dev) {
  return -1;
}

uint16_t ngbe_recv_pkts_vec(__rte_unused void *rx_queue,
                            __rte_unused struct rte_mbuf **rx_pkts,
                            __rte_unused uint16_t nb_pkts) {
  return 0;
}

uint16_t ngbe_recv_scattered_pkts_vec(__rte_unused void *rx_queue,
                                      __rte_unused struct rte_mbuf **rx_pkts,
                                      __rte_unused uint16_t nb_pkts) {
  return 0;
}

int ngbe_rxq_vec_setup(__rte_unused struct ngbe_rx_queue *rxq) { return -1; }

uint16_t ngbe_xmit_fixed_burst_vec(__rte_unused void *tx_queue,
                                   __rte_unused struct rte_mbuf **tx_pkts,
                                   __rte_unused uint16_t nb_pkts) {
  return 0;
}

int ngbe_txq_vec_setup(__rte_unused struct ngbe_tx_queue *txq) { return -1; }

void ngbe_rx_queue_release_mbufs_vec(__rte_unused struct ngbe_rx_queue *rxq) {}
#endif
