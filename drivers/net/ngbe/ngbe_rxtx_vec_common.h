...
static inline uint16_t reassemble_packets(struct ngbe_rx_queue *rxq,
    ...
    if (end != NULL) {
      ...
      if (!split_flags[buf_idx]) {
        ...
        if (end->data_len > rxq->crc_len) {
          ...
          secondlast->data_len -= (rxq->crc_len - end->data_len);
...
static __rte_always_inline void tx_backlog_entry(struct ngbe_tx_entry_v *txep,
...
static inline void _ngbe_tx_queue_release_mbufs_vec(struct ngbe_tx_queue *txq) {
  unsigned int i;
  ...
  for (i = txq->tx_next_dd - (txq->tx_free_thresh - 1); i != txq->tx_tail;
...
static inline void _ngbe_rx_queue_release_mbufs_vec(struct ngbe_rx_queue *rxq) {
  unsigned int i;
  ...
  if (rxq->rxrearm_nb == 0) {
    ...
    for (i = rxq->rx_tail; i != rxq->rxrearm_start;
...
static inline void _ngbe_tx_free_swring_vec(struct ngbe_tx_queue *txq) {
  if (txq == NULL)
    return;
...
static inline void _ngbe_reset_tx_queue_vec(struct ngbe_tx_queue *txq) {
...
static inline int ngbe_rxq_vec_setup_default(struct ngbe_rx_queue *rxq) {
  uintptr_t p;
