...
static const struct reg_info txgbevf_regs_interrupt[] = {{0, 0, 0, ""}};

static const struct reg_info txgbevf_regs_rxdma[] = {{0, 0, 0, ""}};

static const struct reg_info txgbevf_regs_tx[] = {{0, 0, 0, ""}};

/* VF registers */
static const struct reg_info *txgbevf_regs[] = {
    txgbevf_regs_general, txgbevf_regs_interrupt, txgbevf_regs_rxdma,
    txgbevf_regs_tx, NULL};
...
#define TXGBEVF_NB_XSTATS                                                      \
  (sizeof(rte_txgbevf_stats_strings) / sizeof(rte_txgbevf_stats_strings[0]))
 ...
 */
static void txgbevf_negotiate_api(struct txgbe_hw *hw) {
  int32_t i;
...
static void generate_random_mac_addr(struct rte_ether_addr *mac_addr) {
  uint64_t random;
 ...
 */
static int eth_txgbevf_dev_init(struct rte_eth_dev *eth_dev) {
  int err;
  uint32_t tc, tcs;
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("txgbevf", RTE_ETHER_ADDR_LEN * hw->mac.num_rar_entries, 0);
  ...
  if (rte_is_zero_ether_addr(perm_addr)) {
    ...
    PMD_INIT_LOG(
        INFO, "\tAssign randomly generated MAC address " RTE_ETHER_ADDR_PRT_FMT,
  ...
  rte_intr_callback_register(intr_handle, txgbevf_dev_interrupt_handler,
                             eth_dev);
...
static int eth_txgbevf_dev_uninit(struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
...
static int eth_txgbevf_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
  ...
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct txgbe_adapter),
                                       eth_txgbevf_dev_init);
}
...
static int txgbevf_dev_xstats_get_names(__rte_unused struct rte_eth_dev *dev,
  ...
  if (xstats_names != NULL)
    for (i = 0; i < TXGBEVF_NB_XSTATS; i++)
      snprintf(xstats_names[i].name, sizeof(xstats_names[i].name), "%s",
               rte_txgbevf_stats_strings[i].name);
  return TXGBEVF_NB_XSTATS;
}

static void txgbevf_update_stats(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbevf_hw_stats *hw_stats =
      (struct txgbevf_hw_stats *)TXGBE_DEV_STATS(dev);
  unsigned int i;

  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    /* Good Rx packet, include VF loopback */
    TXGBE_UPDCNT32(TXGBE_QPRXPKT(i), hw_stats->qp[i].last_vfgprc,
                   hw_stats->qp[i].vfgprc);

    /* Good Rx octets, include VF loopback */
    TXGBE_UPDCNT36(TXGBE_QPRXOCTL(i), hw_stats->qp[i].last_vfgorc,
                   hw_stats->qp[i].vfgorc);

    /* Rx Multicst Packet */
    TXGBE_UPDCNT32(TXGBE_QPRXMPKT(i), hw_stats->qp[i].last_vfmprc,
                   hw_stats->qp[i].vfmprc);
  }
  hw->rx_loaded = 0;

  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    /* Good Tx packet, include VF loopback */
    TXGBE_UPDCNT32(TXGBE_QPTXPKT(i), hw_stats->qp[i].last_vfgptc,
                   hw_stats->qp[i].vfgptc);

    /* Good Tx octets, include VF loopback */
    TXGBE_UPDCNT36(TXGBE_QPTXOCTL(i), hw_stats->qp[i].last_vfgotc,
                   hw_stats->qp[i].vfgotc);
  }
  hw->offset_loaded = 0;
}
...
static int txgbevf_dev_xstats_get(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < TXGBEVF_NB_XSTATS; i++) {
    xstats[i].id = i;
    xstats[i].value =
        *(uint64_t *)(((char *)hw_stats) + rte_txgbevf_stats_strings[i].offset);
  }

  return TXGBEVF_NB_XSTATS;
}

static int txgbevf_dev_stats_get(struct rte_eth_dev *dev,
...
static int txgbevf_dev_start(struct rte_eth_dev *dev) {
  ...
  if (rte_intr_cap_multiple(intr_handle) && dev->data->dev_conf.intr_conf.rxq) {
...
static int txgbevf_dev_close(struct rte_eth_dev *dev) {
  ...
  rte_intr_disable(intr_handle);
  rte_intr_callback_unregister(intr_handle, txgbevf_dev_interrupt_handler, dev);
 ...
 * Reset VF device
 */
static int txgbevf_dev_reset(struct rte_eth_dev *dev) {
  int ret;
...
static void txgbevf_set_vfta_all(struct rte_eth_dev *dev, bool on) {
  ...
  for (i = 0; i < TXGBE_VFTA_SIZE; i++) {
    ...
    if (vfta) {
      ...
      for (j = 0; j < 32; j++) {
        if (vfta & mask)
          hw->mac.set_vfta(hw, (i << 5) + j, 0, on, false);
        mask <<= 1;
      }
    }
  }
...
static int txgbevf_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                   int on) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_vfta *shadow_vfta = TXGBE_DEV_VFTA(dev);
  uint32_t vid_idx = 0;
...
static void txgbevf_vlan_strip_queue_set(struct rte_eth_dev *dev,
  ...
  wr32m(hw, TXGBE_RXCFG(queue), TXGBE_RXCFG_VLAN | TXGBE_RXCFG_ENA, ctrl);
...
static int txgbevf_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
...
static int txgbevf_dev_rx_queue_intr_enable(struct rte_eth_dev *dev,
                                            uint16_t queue_id) {
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
  struct rte_intr_handle *intr_handle = pci_dev->intr_handle;
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
...
static int txgbevf_dev_rx_queue_intr_disable(struct rte_eth_dev *dev,
                                             uint16_t queue_id) {
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct rte_pci_device *pci_dev = RTE_ETH_DEV_TO_PCI(dev);
...
static void txgbevf_set_ivar_map(struct txgbe_hw *hw, int8_t direction,
...
static void txgbevf_configure_msix(struct rte_eth_dev *dev) {
  ...
  for (q_idx = 0; q_idx < dev->data->nb_rx_queues; q_idx++) {
    ...
    rte_intr_vec_list_index_set(intr_handle, q_idx, vector_idx);
    if (vector_idx < base + rte_intr_nb_efd_get(intr_handle) - 1)
      vector_idx++;
  }
  ...
  wr32(hw, TXGBE_ITR(TXGBE_MISC_VEC_ID),
       TXGBE_ITR_IVAL(TXGBE_QUEUE_ITR_INTERVAL_DEFAULT) | TXGBE_ITR_WRDSA);
}

static int txgbevf_add_mac_addr(struct rte_eth_dev *dev,
  ...
  if (err != 0)
    PMD_DRV_LOG(ERR,
                "Unable to add MAC address " RTE_ETHER_ADDR_PRT_FMT " - err=%d",
...
static void txgbevf_remove_mac_addr(struct rte_eth_dev *dev, uint32_t index) {
   ...
   */
  for (i = 0, mac_addr = dev->data->mac_addrs; i < hw->mac.num_rar_entries;
       i++, mac_addr++) {
    ...
    if (err != 0)
      PMD_DRV_LOG(ERR,
                  "Adding again MAC address " RTE_ETHER_ADDR_PRT_FMT " failed "
                  "err=%d",
                  RTE_ETHER_ADDR_BYTES(mac_addr), err);
  }
}

static int txgbevf_set_default_mac_addr(struct rte_eth_dev *dev,
...
static int txgbevf_dev_set_mtu(struct rte_eth_dev *dev, uint16_t mtu) {
  struct txgbe_hw *hw;
...
static int txgbevf_get_reg_length(struct rte_eth_dev *dev __rte_unused) {
  int count = 0;
  int g_ind = 0;
...
static int txgbevf_get_regs(struct rte_eth_dev *dev,
...
static int txgbevf_dev_allmulticast_enable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  int ret;
...
static int txgbevf_dev_allmulticast_disable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  int ret;
...
static int txgbevf_dev_interrupt_get_status(struct rte_eth_dev *dev) {
  uint32_t eicr;
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
  txgbevf_intr_disable(dev);
...
static int txgbevf_dev_interrupt_action(struct rte_eth_dev *dev) {
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
