...
static const u64 TXGBE_TX_OFFLOAD_MASK =
    (RTE_MBUF_F_TX_IP_CKSUM | RTE_MBUF_F_TX_OUTER_IPV6 |
     RTE_MBUF_F_TX_OUTER_IPV4 | RTE_MBUF_F_TX_IPV6 | RTE_MBUF_F_TX_IPV4 |
     RTE_MBUF_F_TX_VLAN | RTE_MBUF_F_TX_L4_MASK | RTE_MBUF_F_TX_TCP_SEG |
...
static int txgbe_is_vf(struct rte_eth_dev *dev) {
 ...
 */
static __rte_always_inline int txgbe_tx_free_bufs(struct txgbe_tx_queue *txq) {
  ...
  if (!(status & rte_cpu_to_le_32(TXGBE_TXD_DD))) {
    if (txq->nb_tx_free >> 1 < txq->tx_free_thresh)
      txgbe_set32_masked(txq->tdc_reg_addr, TXGBE_TXCFG_FLUSH,
                         TXGBE_TXCFG_FLUSH);
    return 0;
  }
...
static inline void tx4(volatile struct txgbe_tx_desc *txdp,
  ...
  for (i = 0; i < 4; ++i, ++txdp, ++pkts) {
    ...
    txdp->dw2 = cpu_to_le32(TXGBE_TXD_FLAGS | TXGBE_TXD_DATLEN(pkt_len));
...
static inline void tx1(volatile struct txgbe_tx_desc *txdp,
                       struct rte_mbuf **pkts) {
  uint64_t buf_dma_addr;
  uint32_t pkt_len;
 ...
 */
static inline void txgbe_tx_fill_hw_ring(struct txgbe_tx_queue *txq,
                                         struct rte_mbuf **pkts,
                                         uint16_t nb_pkts) {
...
static inline uint16_t tx_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
...
static uint16_t txgbe_xmit_pkts_vec(void *tx_queue, struct rte_mbuf **tx_pkts,
                                    uint16_t nb_pkts) {
  struct txgbe_tx_queue *txq = (struct txgbe_tx_queue *)tx_queue;
  uint16_t nb_tx = 0;
...
txgbe_set_xmit_ctx(struct txgbe_tx_queue *txq,
  ...
  if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
    ...
    case RTE_MBUF_F_TX_UDP_CKSUM:
      mss_l4len_idx |= TXGBE_TXD_L4LEN(sizeof(struct rte_udp_hdr));
      tx_offload_mask.l2_len |= ~0;
      tx_offload_mask.l3_len |= ~0;
      break;
    case RTE_MBUF_F_TX_TCP_CKSUM:
      mss_l4len_idx |= TXGBE_TXD_L4LEN(sizeof(struct rte_tcp_hdr));
      tx_offload_mask.l2_len |= ~0;
      tx_offload_mask.l3_len |= ~0;
      break;
    case RTE_MBUF_F_TX_SCTP_CKSUM:
      mss_l4len_idx |= TXGBE_TXD_L4LEN(sizeof(struct rte_sctp_hdr));
      tx_offload_mask.l2_len |= ~0;
      tx_offload_mask.l3_len |= ~0;
      break;
    default:
  ...
  if (ol_flags & RTE_MBUF_F_TX_SEC_OFFLOAD) {
    union txgbe_crypto_tx_desc_md *md = (union txgbe_crypto_tx_desc_md *)mdata;
    tunnel_seed |= TXGBE_TXD_IPSEC_SAIDX(md->sa_idx);
    type_tucmd_mlhl |=
        md->enc ? (TXGBE_TXD_IPSEC_ESP | TXGBE_TXD_IPSEC_ESPENC) : 0;
 ...
 */
static inline uint32_t what_ctx_update(struct txgbe_tx_queue *txq,
                                       uint64_t flags,
...
static inline uint32_t tx_desc_cksum_flags_to_olinfo(uint64_t ol_flags) {
  uint32_t tmp = 0;
...
static inline uint32_t tx_desc_ol_flags_to_cmdtype(uint64_t ol_flags) {
  uint32_t cmdtype = 0;
...
static inline uint32_t tx_desc_ol_flags_to_ptype(uint64_t oflags) {
  uint32_t ptype;
  bool tun;
  ...
  case RTE_MBUF_F_TX_TUNNEL_VXLAN:
  case RTE_MBUF_F_TX_TUNNEL_VXLAN_GPE:
    ptype |= RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_GRENAT;
    break;
  case RTE_MBUF_F_TX_TUNNEL_GRE:
    ptype |= RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_GRE;
    break;
  case RTE_MBUF_F_TX_TUNNEL_GENEVE:
    ptype |= RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_GENEVE;
    ptype |= RTE_PTYPE_INNER_L2_ETHER;
    break;
  case RTE_MBUF_F_TX_TUNNEL_IPIP:
  case RTE_MBUF_F_TX_TUNNEL_IP:
    ptype |= RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_TUNNEL_IP;
    break;
  }
...
static inline uint8_t tx_desc_ol_flags_to_ptid(uint64_t oflags) {
  uint32_t ptype;
...
static inline int txgbe_xmit_cleanup(struct txgbe_tx_queue *txq) {
  ...
  if (!(status & rte_cpu_to_le_32(TXGBE_TXD_DD))) {
    PMD_TX_FREE_LOG(DEBUG,
                    ...
                    "(port=%d queue=%d)",
                    desc_to_clean_to, txq->port_id, txq->queue_id);
    if (txq->nb_tx_free >> 1 < txq->tx_free_thresh)
      txgbe_set32_masked(txq->tdc_reg_addr, TXGBE_TXCFG_FLUSH,
                         TXGBE_TXCFG_FLUSH);
  ...
  if (last_desc_cleaned > desc_to_clean_to)
    nb_tx_to_clean =
        (uint16_t)((nb_tx_desc - last_desc_cleaned) + desc_to_clean_to);
  else
...
static inline uint8_t txgbe_get_tun_len(struct rte_mbuf *mbuf) {
  ...
  case RTE_MBUF_F_TX_TUNNEL_VXLAN:
  case RTE_MBUF_F_TX_TUNNEL_VXLAN_GPE:
    tun_len = sizeof(struct txgbe_udphdr) + sizeof(struct txgbe_vxlanhdr);
    break;
  case RTE_MBUF_F_TX_TUNNEL_GRE:
    tun_len = sizeof(struct txgbe_nvgrehdr);
    break;
  case RTE_MBUF_F_TX_TUNNEL_GENEVE:
    gh = rte_pktmbuf_read(mbuf, mbuf->outer_l2_len + mbuf->outer_l3_len,
                          sizeof(genevehdr), &genevehdr);
    tun_len = sizeof(struct txgbe_udphdr) + sizeof(struct txgbe_genevehdr) +
              (gh->opt_len << 2);
    break;
  default:
    tun_len = 0;
...
static inline uint8_t txgbe_parse_tun_ptid(struct rte_mbuf *tx_pkt) {
...
uint16_t txgbe_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
    ...
    if (tx_ol_req) {
      ...
      if (use_ipsec) {
        union txgbe_crypto_tx_desc_md *ipsec_mdata =
            (union txgbe_crypto_tx_desc_md *)rte_security_dynfield(tx_pkt);
    ...
    PMD_TX_LOG(DEBUG,
               ...
               (uint16_t)txq->port_id, (uint16_t)txq->queue_id,
               (uint32_t)pkt_len, (uint16_t)tx_id, (uint16_t)tx_last);
    ...
    if (nb_used > txq->nb_tx_free) {
      PMD_TX_FREE_LOG(DEBUG,
                      ...
                      "(port=%d queue=%d)",
                      nb_used, txq->nb_tx_free, txq->port_id, txq->queue_id);
      ...
      if (unlikely(nb_used > txq->tx_free_thresh)) {
        PMD_TX_FREE_LOG(DEBUG,
                        ...
                        "tx_free_thresh=%4u. "
                        "(port=%d queue=%d)",
                        nb_used, txq->nb_tx_free, txq->tx_free_thresh,
                        txq->port_id, txq->queue_id);
        /*
    ...
    if (tx_ol_req) {
      if (ol_flags & RTE_MBUF_F_TX_TCP_SEG) {
         ...
         */
        pkt_len -= (tx_offload.l2_len + tx_offload.l3_len + tx_offload.l4_len);
        pkt_len -= (tx_pkt->ol_flags & RTE_MBUF_F_TX_TUNNEL_MASK)
                       ? tx_offload.outer_l2_len + tx_offload.outer_l3_len
                       : 0;
      }
      ...
      if (new_ctx) {
        ...
        ctx_txd = (volatile struct txgbe_tx_ctx_desc *)&txr[tx_id];
        ...
        txgbe_set_xmit_ctx(txq, ctx_txd, tx_ol_req, tx_offload,
...
end_of_tx:
   ...
   */
  PMD_TX_LOG(DEBUG, "port_id=%u queue_id=%u tx_tail=%u nb_tx=%u",
             (uint16_t)txq->port_id, (uint16_t)txq->queue_id, (uint16_t)tx_id,
             (uint16_t)nb_tx);
...
static inline uint32_t txgbe_rxd_pkt_info_to_pkt_type(uint32_t pkt_info,
                                                      uint16_t ptid_mask) {
...
static inline uint64_t txgbe_rxd_pkt_info_to_pkt_flags(uint32_t pkt_info) {
  ...
  static uint64_t ip_pkt_etqf_map[8] = {
      0, 0, 0, RTE_MBUF_F_RX_IEEE1588_PTP, 0, 0, 0, 0,
  };
...
static inline uint64_t rx_desc_status_to_pkt_flags(uint32_t rx_status,
                                                   uint64_t vlan_flags) {
  uint64_t pkt_flags;
...
static inline uint64_t rx_desc_error_to_pkt_flags(uint32_t rx_status) {
  ...
  if (rx_status & TXGBE_RXD_STAT_IPCS) {
    pkt_flags |= (rx_status & TXGBE_RXD_ERR_IPCS ? RTE_MBUF_F_RX_IP_CKSUM_BAD
                                                 : RTE_MBUF_F_RX_IP_CKSUM_GOOD);
  }

  if (rx_status & TXGBE_RXD_STAT_L4CS) {
    pkt_flags |= (rx_status & TXGBE_RXD_ERR_L4CS ? RTE_MBUF_F_RX_L4_CKSUM_BAD
                                                 : RTE_MBUF_F_RX_L4_CKSUM_GOOD);
  }

  if (rx_status & TXGBE_RXD_STAT_EIPCS && rx_status & TXGBE_RXD_ERR_EIPCS) {
...
#endif
static inline int txgbe_rx_scan_hw_ring(struct txgbe_rx_queue *rxq) {
  ...
  for (i = 0; i < RTE_PMD_TXGBE_RX_MAX_BURST;
    ...
    for (nb_dd = 0; nb_dd < LOOK_AHEAD && (s[nb_dd] & TXGBE_RXD_STAT_DD);
         nb_dd++)
      ;
    ...
    for (j = 0; j < nb_dd; ++j) {
      mb = rxep[j].mbuf;
      pkt_len = rte_le_to_cpu_16(rxdp[j].qw1.hi.len) - rxq->crc_len;
      mb->data_len = pkt_len;
      mb->pkt_len = pkt_len;
      ...
      pkt_flags |= txgbe_rxd_pkt_info_to_pkt_flags(pkt_info[j]);
      mb->ol_flags = pkt_flags;
      mb->packet_type =
          txgbe_rxd_pkt_info_to_pkt_type(pkt_info[j], rxq->pkt_type_mask);

      if (likely(pkt_flags & RTE_MBUF_F_RX_RSS_HASH))
        mb->hash.rss = rte_le_to_cpu_32(rxdp[j].qw0.dw1);
      else if (pkt_flags & RTE_MBUF_F_RX_FDIR) {
        mb->hash.fdir.hash =
            rte_le_to_cpu_16(rxdp[j].qw0.hi.csum) & TXGBE_ATR_HASH_MASK;
        mb->hash.fdir.id = rte_le_to_cpu_16(rxdp[j].qw0.hi.ipid);
      }
    }
...
static inline int txgbe_rx_alloc_bufs(struct txgbe_rx_queue *rxq,
                                      bool reset_mbuf) {
  volatile struct txgbe_rx_desc *rxdp;
  struct txgbe_rx_entry *rxep;
  struct rte_mbuf *mb;
  ...
  diag = rte_mempool_get_bulk(rxq->mb_pool, (void *)rxep, rxq->rx_free_thresh);
  if (unlikely(diag != 0))
    return -ENOMEM;
...
static inline uint16_t txgbe_rx_fill_from_stage(struct txgbe_rx_queue *rxq,
...
static inline uint16_t txgbe_rx_recv_pkts(void *rx_queue,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts) {
...
uint16_t txgbe_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (nb_rx < nb_pkts) {
    ...
    PMD_RX_LOG(DEBUG,
               ...
               (uint16_t)rxq->port_id, (uint16_t)rxq->queue_id, (uint16_t)rx_id,
               (uint32_t)staterr, (uint16_t)rte_le_to_cpu_16(rxd.qw1.hi.len));
     ...
     *    - error flags.
     */
    pkt_len = (uint16_t)(rte_le_to_cpu_16(rxd.qw1.hi.len) - rxq->crc_len);
    ...
    rxm->packet_type =
        txgbe_rxd_pkt_info_to_pkt_type(pkt_info, rxq->pkt_type_mask);

    if (likely(pkt_flags & RTE_MBUF_F_RX_RSS_HASH)) {
      rxm->hash.rss = rte_le_to_cpu_32(rxd.qw0.dw1);
    } else if (pkt_flags & RTE_MBUF_F_RX_FDIR) {
      rxm->hash.fdir.hash =
          rte_le_to_cpu_16(rxd.qw0.hi.csum) & TXGBE_ATR_HASH_MASK;
  ...
  if (nb_hold > rxq->rx_free_thresh) {
    PMD_RX_LOG(DEBUG,
               ...
               "nb_hold=%u nb_rx=%u",
               (uint16_t)rxq->port_id, (uint16_t)rxq->queue_id, (uint16_t)rx_id,
               (uint16_t)nb_hold, (uint16_t)nb_rx);
 ...
 */
static inline void txgbe_fill_cluster_head_buf(struct rte_mbuf *head,
  ...
  if (likely(pkt_flags & RTE_MBUF_F_RX_RSS_HASH)) {
    ...
    head->hash.fdir.hash =
        rte_le_to_cpu_16(desc->qw0.hi.csum) & TXGBE_ATR_HASH_MASK;
 ...
 * receive" interface).
 */
static inline uint16_t txgbe_recv_pkts_lro(void *rx_queue,
                                           struct rte_mbuf **rx_pkts,
                                           uint16_t nb_pkts, bool bulk_alloc) {
  ...
  next_desc:
    ...
    if (!bulk_alloc) {
      ...
      if (!txgbe_rx_alloc_bufs(rxq, false)) {
        rte_wmb();
        txgbe_set32_relaxed(rxq->rdt_reg_addr, next_rdt);
    ...
    if (!bulk_alloc) {
      __le64 dma = rte_cpu_to_le_64(rte_mbuf_data_iova_default(nmb));
      /*
...
uint64_t txgbe_get_rx_queue_offloads(struct rte_eth_dev *dev __rte_unused) {
...
uint64_t txgbe_get_rx_port_offloads(struct rte_eth_dev *dev) {
  ...
  offloads = RTE_ETH_RX_OFFLOAD_IPV4_CKSUM | RTE_ETH_RX_OFFLOAD_UDP_CKSUM |
             RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_KEEP_CRC |
...
static int txgbe_tx_done_cleanup_full(struct txgbe_tx_queue *txq,
                                      uint32_t free_cnt) {
  ...
  for (pkt_cnt = 0; pkt_cnt < free_cnt;) {
    for (i = 0; i < nb_tx_to_clean && pkt_cnt < free_cnt && tx_id != tx_last;
         i++) {
...
static int txgbe_tx_done_cleanup_simple(struct txgbe_tx_queue *txq,
                                        uint32_t free_cnt) {
  int i, n, cnt;
...
static void __rte_cold txgbe_tx_free_swring(struct txgbe_tx_queue *txq) {
  if (txq != NULL && txq->sw_ring != NULL)
    rte_free(txq->sw_ring);
}

static void __rte_cold txgbe_tx_queue_release(struct txgbe_tx_queue *txq) {
...
void __rte_cold txgbe_dev_tx_queue_release(struct rte_eth_dev *dev,
                                           uint16_t qid) {
  txgbe_tx_queue_release(dev->data->tx_queues[qid]);
}

/* (Re)set dynamic txgbe_tx_queue fields to defaults */
static void __rte_cold txgbe_reset_tx_queue(struct txgbe_tx_queue *txq) {
 ...
 */
void __rte_cold txgbe_set_tx_function(struct rte_eth_dev *dev,
                                      struct txgbe_tx_queue *txq) {
  ...
  if (txq->offloads == 0 &&
    ...
    PMD_INIT_LOG(DEBUG, " - offloads = 0x%" PRIx64, txq->offloads);
    PMD_INIT_LOG(DEBUG,
...
uint64_t txgbe_get_tx_port_offloads(struct rte_eth_dev *dev) {
  ...
  tx_offload_capa =
      ...
      RTE_ETH_TX_OFFLOAD_UDP_TSO | RTE_ETH_TX_OFFLOAD_UDP_TNL_TSO |
      RTE_ETH_TX_OFFLOAD_IP_TNL_TSO | RTE_ETH_TX_OFFLOAD_VXLAN_TNL_TSO |
      RTE_ETH_TX_OFFLOAD_GRE_TNL_TSO | RTE_ETH_TX_OFFLOAD_IPIP_TNL_TSO |
      RTE_ETH_TX_OFFLOAD_GENEVE_TNL_TSO | RTE_ETH_TX_OFFLOAD_MULTI_SEGS;
...
int __rte_cold txgbe_dev_tx_queue_setup(struct rte_eth_dev *dev,
   ...
   * of TXGBE_ALIGN.
   */
  if (nb_desc % TXGBE_TXD_ALIGN != 0 || nb_desc > TXGBE_RING_DESC_MAX ||
   ...
   */
  tx_free_thresh =
      (uint16_t)((tx_conf->tx_free_thresh) ? tx_conf->tx_free_thresh
  ...
  if ((nb_desc % tx_free_thresh) != 0) {
    PMD_INIT_LOG(ERR,
                 "tx_free_thresh must be a divisor of the "
  ...
  txq->reg_idx =
      (uint16_t)((RTE_ETH_DEV_SRIOV(dev).active == 0)
                     ? queue_idx
                     : RTE_ETH_DEV_SRIOV(dev).def_pool_q_idx + queue_idx);
  ...
  PMD_INIT_LOG(DEBUG, "sw_ring=%p hw_ring=%p dma_addr=0x%" PRIx64, txq->sw_ring,
               txq->tx_ring, txq->tx_ring_phys_addr);
 ...
 */
static void __rte_cold txgbe_free_sc_cluster(struct rte_mbuf *m) {
...
static void __rte_cold txgbe_rx_queue_release(struct txgbe_rx_queue *rxq) {
  if (rxq != NULL) {
...
void __rte_cold txgbe_dev_rx_queue_release(struct rte_eth_dev *dev,
                                           uint16_t qid) {
...
static void __rte_cold txgbe_reset_rx_queue(struct txgbe_adapter *adapter,
                                            struct txgbe_rx_queue *rxq) {
...
int __rte_cold txgbe_dev_rx_queue_setup(struct rte_eth_dev *dev,
                                        uint16_t queue_idx, uint16_t nb_desc,
                                        unsigned int socket_id,
   ...
   * of TXGBE_ALIGN.
   */
  if (nb_desc % TXGBE_RXD_ALIGN != 0 || nb_desc > TXGBE_RING_DESC_MAX ||
      nb_desc < TXGBE_RING_DESC_MIN) {
    return -EINVAL;
  }
   ...
   */
  if (hw->mac.type == txgbe_mac_raptor_vf) {
    rxq->rdt_reg_addr = TXGBE_REG_ADDR(hw, TXGBE_RXWP(queue_idx));
    rxq->rdh_reg_addr = TXGBE_REG_ADDR(hw, TXGBE_RXRP(queue_idx));
  } else {
    rxq->rdt_reg_addr = TXGBE_REG_ADDR(hw, TXGBE_RXWP(rxq->reg_idx));
    rxq->rdh_reg_addr = TXGBE_REG_ADDR(hw, TXGBE_RXRP(rxq->reg_idx));
  }
  ...
  rxq->sw_ring =
      rte_zmalloc_socket("rxq->sw_ring", sizeof(struct txgbe_rx_entry) * len,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_ring) {
    txgbe_rx_queue_release(rxq);
    return -ENOMEM;
   ...
   * dev_start() flow.
   */
  rxq->sw_sc_ring = rte_zmalloc_socket(
      "rxq->sw_sc_ring", sizeof(struct txgbe_scattered_rx_entry) * len,
      RTE_CACHE_LINE_SIZE, socket_id);
  if (!rxq->sw_sc_ring) {
    txgbe_rx_queue_release(rxq);
    return -ENOMEM;
...
void __rte_cold txgbe_dev_clear_queues(struct rte_eth_dev *dev) {
  unsigned int i;
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
...
static void txgbe_rss_disable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
...
int txgbe_dev_rss_hash_update(struct rte_eth_dev *dev,
  ...
  if (hw->mac.type == txgbe_mac_raptor_vf) {
    ...
    if (rss_hf & RTE_ETH_RSS_IPV6 || rss_hf & RTE_ETH_RSS_IPV6_EX)
...
int txgbe_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  if (hw->mac.type == txgbe_mac_raptor_vf) {
    ...
    if (mrqc & TXGBE_VFPLCFG_RSSIPV6)
      rss_hf |= RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_IPV6_EX;
    if (mrqc & TXGBE_VFPLCFG_RSSIPV6TCP)
      rss_hf |= RTE_ETH_RSS_NONFRAG_IPV6_TCP | RTE_ETH_RSS_IPV6_TCP_EX;
    if (mrqc & TXGBE_VFPLCFG_RSSIPV4UDP)
      rss_hf |= RTE_ETH_RSS_NONFRAG_IPV4_UDP;
    if (mrqc & TXGBE_VFPLCFG_RSSIPV6UDP)
      rss_hf |= RTE_ETH_RSS_NONFRAG_IPV6_UDP | RTE_ETH_RSS_IPV6_UDP_EX;
...
static void txgbe_rss_configure(struct rte_eth_dev *dev) {
  struct rte_eth_rss_conf rss_conf;
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t reta;
...
static void txgbe_vmdq_dcb_configure(struct rte_eth_dev *dev) {
  ...
  wr32m(hw, TXGBE_PORTCTL, TXGBE_PORTCTL_NUMTC_MASK | TXGBE_PORTCTL_NUMVT_MASK,
        mrqc);
  ...
  for (i = 0; i < cfg->nb_pool_maps; i++) {
    ...
    wr32(hw, TXGBE_PSRVLAN,
         (TXGBE_PSRVLAN_EA | (cfg->pool_map[i].vlan_id & 0xFFF)));
 ...
 */
static void txgbe_dcb_tx_hw_config(struct rte_eth_dev *dev,
 ...
 * @dcb_config: pointer to txgbe_dcb_config structure
 */
static void txgbe_vmdq_dcb_hw_tx_config(struct rte_eth_dev *dev,
                                        struct txgbe_dcb_config *dcb_config) {
...
static void txgbe_vmdq_dcb_rx_config(struct rte_eth_dev *dev,
                                     struct txgbe_dcb_config *dcb_config) {
  ...
  for (i = 0; i < RTE_ETH_DCB_NUM_USER_PRIORITIES; i++) {
    ...
    tc = &dcb_config->tc_config[j];
    tc->path[TXGBE_DCB_RX_CONFIG].up_to_tc_bitmap |= (uint8_t)(1 << i);
  }
}

static void txgbe_dcb_vt_tx_config(struct rte_eth_dev *dev,
                                   struct txgbe_dcb_config *dcb_config) {
  struct rte_eth_vmdq_dcb_tx_conf *vmdq_tx_conf =
      &dev->data->dev_conf.tx_adv_conf.vmdq_dcb_tx_conf;
  struct txgbe_dcb_tc_config *tc;
  ...
  for (i = 0; i < RTE_ETH_DCB_NUM_USER_PRIORITIES; i++) {
    ...
    tc = &dcb_config->tc_config[j];
    tc->path[TXGBE_DCB_TX_CONFIG].up_to_tc_bitmap |= (uint8_t)(1 << i);
  }
}

static void txgbe_dcb_rx_config(struct rte_eth_dev *dev,
                                struct txgbe_dcb_config *dcb_config) {
...
static void txgbe_dcb_tx_config(struct rte_eth_dev *dev,
                                struct txgbe_dcb_config *dcb_config) {
 ...
 * @dcb_config: pointer to txgbe_dcb_config structure
 */
static void txgbe_dcb_rx_hw_config(struct rte_eth_dev *dev,
                                   struct txgbe_dcb_config *dcb_config) {
  uint32_t reg;
  uint32_t vlanctrl;
  uint8_t i;
...
static void txgbe_dcb_hw_arbite_rx_config(struct txgbe_hw *hw, uint16_t *refill,
                                          uint16_t *max, uint8_t *bwg_id,
                                          uint8_t *tsa, uint8_t *map) {
  txgbe_dcb_config_rx_arbiter_raptor(hw, refill, max, bwg_id, tsa, map);
}

static void txgbe_dcb_hw_arbite_tx_config(struct txgbe_hw *hw, uint16_t *refill,
                                          uint16_t *max, uint8_t *bwg_id,
                                          uint8_t *tsa, uint8_t *map) {
  switch (hw->mac.type) {
 ...
 * @dcb_config: pointer to txgbe_dcb_config structure
 */
static int txgbe_dcb_hw_configure(struct rte_eth_dev *dev,
                                  struct txgbe_dcb_config *dcb_config) {
  int ret = 0;
  ...
  if (nb_tcs == RTE_ETH_4_TCS) {
    ...
    for (i = 0; i < nb_tcs; i++) {
      ...
      if (bw_conf->tc_num != nb_tcs)
        tc->path[TXGBE_DCB_TX_CONFIG].bwg_percent = (uint8_t)(100 / nb_tcs);
      tc->path[TXGBE_DCB_RX_CONFIG].bwg_percent = (uint8_t)(100 / nb_tcs);
    }
...
void txgbe_configure_port(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  int i = 0;
  uint16_t tpids[8] = {RTE_ETHER_TYPE_VLAN,
                       RTE_ETHER_TYPE_QINQ,
                       0x9100,
                       0x9200,
                       0x0000,
  ...
  for (i = 0; i < 8; i++) {
    wr32m(hw, TXGBE_TAGTPID(i / 2),
          (i % 2 ? TXGBE_TAGTPID_MSB_MASK : TXGBE_TAGTPID_LSB_MASK),
          (i % 2 ? TXGBE_TAGTPID_MSB(tpids[i]) : TXGBE_TAGTPID_LSB(tpids[i])));
  }
 ...
 */
static void txgbe_vmdq_rx_hw_configure(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < cfg->nb_pool_maps; i++) {
    ...
    wr32(hw, TXGBE_PSRVLANIDX, i);
    wr32(hw, TXGBE_PSRVLAN,
         (TXGBE_PSRVLAN_EA | TXGBE_PSRVLAN_VID(cfg->pool_map[i].vlan_id)));
    /*
    ...
    if (((cfg->pool_map[i].pools >> 32) & UINT32_MAX) == 0)
      wr32(hw, TXGBE_PSRVLANPLM(0), (cfg->pool_map[i].pools & UINT32_MAX));
    else
  ...
  if (cfg->enable_loop_back) {
    ...
    for (i = 0; i < 64; i++)
      wr32m(hw, TXGBE_POOLETHCTL(i), TXGBE_POOLETHCTL_LLB,
            TXGBE_POOLETHCTL_LLB);
  }
 ...
 */
static void txgbe_vmdq_tx_hw_configure(struct txgbe_hw *hw) {
  uint32_t reg;
  uint32_t q;
  ...
  wr32m(hw, TXGBE_PORTCTL, TXGBE_PORTCTL_NUMVT_MASK, TXGBE_PORTCTL_NUMVT_64);
...
static int __rte_cold txgbe_alloc_rx_queue_mbufs(struct txgbe_rx_queue *rxq) {
...
static int txgbe_config_vf_rss(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  uint32_t mrqc;
...
static int txgbe_config_vf_default(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t mrqc;
...
static int txgbe_dev_mq_rx_configure(struct rte_eth_dev *dev) {
  if (RTE_ETH_DEV_SRIOV(dev).active == 0) {
    /*
    ...
    case RTE_ETH_MQ_RX_DCB_RSS:
      PMD_INIT_LOG(ERR, "Could not support DCB/RSS with VMDq & SRIOV");
      return -1;
    default:
...
static int txgbe_dev_mq_tx_configure(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t mtqc;
  uint32_t rttdcs;
 ...
 */
static inline uint32_t txgbe_get_rscctl_maxdesc(struct rte_mempool *pool) {
  struct rte_pktmbuf_pool_private *mp_priv = rte_mempool_get_priv(pool);

  uint16_t maxdesc = RTE_IPV4_MAX_PKT_LEN /
 ...
 */
static int txgbe_set_rsc(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    struct txgbe_rx_queue *rxq = dev->data->rx_queues[i];
    uint32_t srrctl = rd32(hw, TXGBE_RXCFG(rxq->reg_idx));
    uint32_t psrtype = rd32(hw, TXGBE_POOLRSS(rxq->reg_idx));
...
void __rte_cold txgbe_set_rx_function(struct rte_eth_dev *dev) {
  ...
  if (dev->data->lro) {
    ...
    PMD_INIT_LOG(DEBUG,
                 ...
                 RTE_TXGBE_DESCS_PER_LOOP, dev->data->port_id);
...
#ifdef RTE_LIB_SECURITY
    rxq->using_ipsec =
        !!(dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_SECURITY);
#endif
  }
}
 ...
 */
int __rte_cold txgbe_dev_rx_init(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  struct txgbe_rx_queue *rxq;
  uint64_t bus_addr;
  uint32_t fctrl;
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    wr32(hw, TXGBE_RXBAL(rxq->reg_idx), (uint32_t)(bus_addr & BIT_MASK32));
    wr32(hw, TXGBE_RXBAH(rxq->reg_idx), (uint32_t)(bus_addr >> 32));
    ...
    if (dev->data->mtu + TXGBE_ETH_OVERHEAD + 2 * RTE_VLAN_HLEN > buf_size)
 ...
 */
void __rte_cold txgbe_dev_tx_init(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  struct txgbe_tx_queue *txq;
  uint64_t bus_addr;
  uint16_t i;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    wr32(hw, TXGBE_TXBAL(txq->reg_idx), (uint32_t)(bus_addr & BIT_MASK32));
    wr32(hw, TXGBE_TXBAH(txq->reg_idx), (uint32_t)(bus_addr >> 32));
 ...
 */
int __rte_cold txgbe_dev_rxtx_start(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  struct txgbe_tx_queue *txq;
  struct txgbe_rx_queue *rxq;
  uint32_t dmatxctl;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    wr32m(hw, TXGBE_TXCFG(txq->reg_idx),
          TXGBE_TXCFG_HTHRESH_MASK | TXGBE_TXCFG_WTHRESH_MASK,
  ...
  if ((dev->data->dev_conf.rxmode.offloads & RTE_ETH_RX_OFFLOAD_SECURITY) ||
    ...
    if (ret != 0) {
      PMD_DRV_LOG(ERR, "txgbe_crypto_enable_ipsec fails with %d.", ret);
      return ret;
    }
  }
#endif
 ...
 */
int __rte_cold txgbe_dev_rx_queue_start(struct rte_eth_dev *dev,
  ...
  if (txgbe_alloc_rx_queue_mbufs(rxq) != 0) {
    PMD_INIT_LOG(ERR, "Could not alloc mbuf for queue:%d", rx_queue_id);
    return -1;
  }
 ...
 */
int __rte_cold txgbe_dev_rx_queue_stop(struct rte_eth_dev *dev,
                                       uint16_t rx_queue_id) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
  struct txgbe_rx_queue *rxq;
 ...
 */
int __rte_cold txgbe_dev_tx_queue_start(struct rte_eth_dev *dev,
 ...
 */
int __rte_cold txgbe_dev_tx_queue_stop(struct rte_eth_dev *dev,
                                       uint16_t tx_queue_id) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_tx_queue *txq;
  uint32_t txdctl;
  ...
  if (!poll_ms)
    PMD_INIT_LOG(ERR, "Tx Queue %d is not empty when stopping.", tx_queue_id);
  ...
  if (!poll_ms)
    PMD_INIT_LOG(ERR, "Could not disable Tx Queue %d", tx_queue_id);
 ...
 */
int __rte_cold txgbevf_dev_rx_init(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  struct txgbe_rx_queue *rxq;
   ...
   */
  if (txgbevf_rlpml_set_vf(hw, (uint16_t)dev->data->mtu + TXGBE_ETH_OVERHEAD)) {
  ...
  psrtype = TXGBE_VFPLCFG_PSRL4HDR | TXGBE_VFPLCFG_PSRL4HDR |
            TXGBE_VFPLCFG_PSRL2HDR | TXGBE_VFPLCFG_PSRTUNHDR |
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    wr32(hw, TXGBE_RXBAL(i), (uint32_t)(bus_addr & BIT_MASK32));
    wr32(hw, TXGBE_RXBAH(i), (uint32_t)(bus_addr >> 32));
    ...
    if (rxmode->offloads & RTE_ETH_RX_OFFLOAD_SCATTER ||
        /* It adds dual VLAN length for supporting dual VLAN */
        (dev->data->mtu + TXGBE_ETH_OVERHEAD + 2 * RTE_VLAN_HLEN) > buf_size) {
 ...
 */
void __rte_cold txgbevf_dev_tx_init(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  struct txgbe_tx_queue *txq;
  uint64_t bus_addr;
  uint16_t i;
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    txq = dev->data->tx_queues[i];
    bus_addr = txq->tx_ring_phys_addr;
    wr32(hw, TXGBE_TXBAL(i), (uint32_t)(bus_addr & BIT_MASK32));
    wr32(hw, TXGBE_TXBAH(i), (uint32_t)(bus_addr >> 32));
 ...
 */
void __rte_cold txgbevf_dev_rxtx_start(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
  struct txgbe_tx_queue *txq;
  struct txgbe_rx_queue *rxq;
  uint32_t txdctl;
...
int txgbe_rss_conf_init(struct txgbe_rte_flow_rss_conf *out,
  ...
  if (in->key_len > RTE_DIM(out->key) || in->queue_num > RTE_DIM(out->queue))
    return -EINVAL;
  out->conf = (struct rte_flow_action_rss){
      ...
      .queue =
          memcpy(out->queue, in->queue, sizeof(*in->queue) * in->queue_num),
  };
  return 0;
}
...
int txgbe_action_rss_same(const struct rte_flow_action_rss *comp,
  ...
  return (comp->func == with->func && comp->level == with->level &&
          comp->types == with->types && comp->key_len == with->key_len &&
...
int txgbe_config_rss_filter(struct rte_eth_dev *dev,
  ...
  uint16_t j;
  struct rte_eth_rss_conf rss_conf = {
      .rss_key = conf->conf.key_len ? (void *)(uintptr_t)conf->conf.key : NULL,
  ...
  if (!add) {
    if (txgbe_action_rss_same(&filter_info->rss_info.conf, &conf->conf)) {
      txgbe_rss_disable(dev);
      memset(&filter_info->rss_info, 0, sizeof(struct txgbe_rte_flow_rss_conf));
      return 0;
    }
    return -EINVAL;
  }
...
int txgbe_rx_vec_dev_conf_condition_check(
    __rte_unused struct rte_eth_dev *dev) {
  return -1;
}

uint16_t txgbe_recv_pkts_vec(__rte_unused void *rx_queue,
                             __rte_unused struct rte_mbuf **rx_pkts,
                             __rte_unused uint16_t nb_pkts) {
  return 0;
}

uint16_t txgbe_recv_scattered_pkts_vec(__rte_unused void *rx_queue,
                                       __rte_unused struct rte_mbuf **rx_pkts,
                                       __rte_unused uint16_t nb_pkts) {
  return 0;
}

int txgbe_rxq_vec_setup(__rte_unused struct txgbe_rx_queue *rxq) { return -1; }

uint16_t txgbe_xmit_fixed_burst_vec(__rte_unused void *tx_queue,
                                    __rte_unused struct rte_mbuf **tx_pkts,
                                    __rte_unused uint16_t nb_pkts) {
  return 0;
}

int txgbe_txq_vec_setup(__rte_unused struct txgbe_tx_queue *txq) { return -1; }

void txgbe_rx_queue_release_mbufs_vec(__rte_unused struct txgbe_rx_queue *rxq) {
}
#endif
