...
static inline void txgbe_rxq_rearm(struct txgbe_rx_queue *rxq) {
  int i;
  uint16_t rx_id;
  ...
  if (rte_mempool_get_bulk(rxq->mb_pool, (void *)rxep,
    ...
    if (rxq->rxrearm_nb + RTE_TXGBE_RXQ_REARM_THRESH >= rxq->nb_rx_desc) {
      dma_addr0 = _mm_setzero_si128();
      for (i = 0; i < RTE_TXGBE_DESCS_PER_LOOP; i++) {
        rxep[i].mbuf = &rxq->fake_mbuf;
        _mm_store_si128((__m128i *)(uintptr_t)&rxdp[i], dma_addr0);
      }
    }
  ...
  rx_id = (uint16_t)((rxq->rxrearm_start == 0) ? (rxq->nb_rx_desc - 1)
                                               : (rxq->rxrearm_start - 1));
...
static inline void desc_to_olflags_v_ipsec(__m128i descs[4],
  ...
  const __m128i ipsec_sterr_msk =
      _mm_set1_epi32(TXGBE_RXD_STAT_SECP | TXGBE_RXD_ERR_SECERR);
  const __m128i ipsec_proc_msk = _mm_set1_epi32(TXGBE_RXD_STAT_SECP);
  const __m128i ipsec_err_flag = _mm_set1_epi32(
      RTE_MBUF_F_RX_SEC_OFFLOAD_FAILED | RTE_MBUF_F_RX_SEC_OFFLOAD);
  const __m128i ipsec_proc_flag = _mm_set1_epi32(RTE_MBUF_F_RX_SEC_OFFLOAD);

  rearm = _mm_set_epi32(*rearm3, *rearm2, *rearm1, *rearm0);
  sterr = _mm_set_epi32(
      _mm_extract_epi32(descs[3], 2), _mm_extract_epi32(descs[2], 2),
      _mm_extract_epi32(descs[1], 2), _mm_extract_epi32(descs[0], 2));
...
static inline void desc_to_olflags_v(__m128i descs[4], __m128i mbuf_init,
                                     uint8_t vlan_flags,
                                     struct rte_mbuf **rx_pkts) {
  ...
  const __m128i rss_flags =
      _mm_set_epi8(RTE_MBUF_F_RX_FDIR, 0, 0, 0, 0, 0, 0, RTE_MBUF_F_RX_RSS_HASH,
  ...
  const __m128i vlan_csum_msk = _mm_set_epi16(
      ...
      (TXGBE_RXD_ERR_L4CS | TXGBE_RXD_ERR_IPCS) >> 16,
      (TXGBE_RXD_ERR_L4CS | TXGBE_RXD_ERR_IPCS) >> 16,
      (TXGBE_RXD_ERR_L4CS | TXGBE_RXD_ERR_IPCS) >> 16, TXGBE_RXD_STAT_VLAN,
      TXGBE_RXD_STAT_VLAN, TXGBE_RXD_STAT_VLAN, TXGBE_RXD_STAT_VLAN);
  ...
  const __m128i vlan_csum_map_lo = _mm_set_epi8(
      ...
      vlan_flags | RTE_MBUF_F_RX_IP_CKSUM_GOOD, 0, 0, 0, 0,
  ...
  const __m128i vlan_csum_map_hi = _mm_set_epi8(
      0, 0, 0, 0, 0, RTE_MBUF_F_RX_L4_CKSUM_GOOD >> sizeof(uint8_t), 0,
      RTE_MBUF_F_RX_L4_CKSUM_GOOD >> sizeof(uint8_t), 0, 0, 0, 0, 0,
      RTE_MBUF_F_RX_L4_CKSUM_GOOD >> sizeof(uint8_t), 0,
      RTE_MBUF_F_RX_L4_CKSUM_GOOD >> sizeof(uint8_t));

  const __m128i vtag_msk = _mm_set_epi16(0x0000, 0x0000, 0x0000, 0x0000, 0x000F,
...
static inline void desc_to_ptype_v(__m128i descs[4], uint16_t pkt_type_mask,
                                   struct rte_mbuf **rx_pkts) {
  __m128i ptype_mask =
      _mm_set_epi32(pkt_type_mask, pkt_type_mask, pkt_type_mask, pkt_type_mask);
  ...
  ptype0 =
      _mm_and_si128(_mm_srli_epi32(ptype0, TXGBE_RXD_PTID_SHIFT), ptype_mask);
 ...
 */
static inline uint16_t _recv_raw_pkts_vec(struct txgbe_rx_queue *rxq,
                                          struct rte_mbuf **rx_pkts,
                                          uint16_t nb_pkts,
  ...
  for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
       pos += RTE_TXGBE_DESCS_PER_LOOP, rxdp += RTE_TXGBE_DESCS_PER_LOOP) {
    ...
    _mm_storeu_si128((void *)&rx_pkts[pos + 3]->rx_descriptor_fields1, pkt_mb4);
    _mm_storeu_si128((void *)&rx_pkts[pos + 2]->rx_descriptor_fields1, pkt_mb3);
    ...
    if (split_packet) {
      __m128i eop_shuf_mask =
          _mm_set_epi8(0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                       0xFF, 0xFF, 0xFF, 0x04, 0x0C, 0x00, 0x08);
    ...
    _mm_storeu_si128((void *)&rx_pkts[pos + 1]->rx_descriptor_fields1, pkt_mb2);
    _mm_storeu_si128((void *)&rx_pkts[pos]->rx_descriptor_fields1, pkt_mb1);
 ...
 * - floor align nb_pkts to a RTE_TXGBE_DESC_PER_LOOP power-of-two
 */
static uint16_t txgbe_recv_scattered_burst_vec(void *rx_queue,
                                               struct rte_mbuf **rx_pkts,
                                               uint16_t nb_pkts) {
  ...
  if (rxq->pkt_first_seg == NULL && split_fl64[0] == 0 && split_fl64[1] == 0 &&
  ...
  return i + reassemble_packets(rxq, &rx_pkts[i], nb_bufs - i, &split_flags[i]);
}
...
uint16_t txgbe_recv_scattered_pkts_vec(void *rx_queue,
  ...
  while (nb_pkts > RTE_TXGBE_MAX_RX_BURST) {
    ...
    burst = txgbe_recv_scattered_burst_vec(rx_queue, rx_pkts + retval,
  ...
  return retval +
         txgbe_recv_scattered_burst_vec(rx_queue, rx_pkts + retval, nb_pkts);
}

static inline void vtx1(volatile struct txgbe_tx_desc *txdp,
                        struct rte_mbuf *pkt, uint64_t flags) {
  __m128i descriptor =
      _mm_set_epi64x((uint64_t)pkt->pkt_len << 45 | flags | pkt->data_len,
                     pkt->buf_iova + pkt->data_off);
  _mm_store_si128((__m128i *)(uintptr_t)txdp, descriptor);
}

static inline void vtx(volatile struct txgbe_tx_desc *txdp,
...
void __rte_cold txgbe_rx_queue_release_mbufs_vec(struct txgbe_rx_queue *rxq) {
...
int __rte_cold txgbe_rxq_vec_setup(struct txgbe_rx_queue *rxq) {
  return txgbe_rxq_vec_setup_default(rxq);
}

int __rte_cold txgbe_txq_vec_setup(struct txgbe_tx_queue *txq) {
  return txgbe_txq_vec_setup_default(txq, &vec_txq_ops);
}

int __rte_cold txgbe_rx_vec_dev_conf_condition_check(struct rte_eth_dev *dev) {
