...
static const struct reg_info txgbe_regs_nvm[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_interrupt[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_fctl_others[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_rxdma[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_rx[] = {{0, 0, 0, ""}};

static struct reg_info txgbe_regs_tx[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_wakeup[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_dcb[] = {{0, 0, 0, ""}};

static const struct reg_info txgbe_regs_mac[] = {{0, 0, 0, ""}};
...
static const struct reg_info *txgbe_regs_others[] = {
    txgbe_regs_general,     txgbe_regs_nvm,        txgbe_regs_interrupt,
    txgbe_regs_fctl_others, txgbe_regs_rxdma,      txgbe_regs_rx,
    txgbe_regs_tx,          txgbe_regs_wakeup,     txgbe_regs_dcb,
    txgbe_regs_mac,         txgbe_regs_diagnostic, NULL};
...
static int txgbe_dev_link_update(struct rte_eth_dev *dev, int wait_to_complete);
...
#define TXGBE_CLEAR_HWSTRIP(h, q)                                              \
  do {                                                                         \
    uint32_t idx = (q) / (sizeof((h)->bitmap[0]) * NBBY);                      \
    uint32_t bit = (q) % (sizeof((h)->bitmap[0]) * NBBY);                      \
    (h)->bitmap[idx] &= ~(1 << bit);                                           \
...
#define TXGBE_GET_HWSTRIP(h, q, r)                                             \
  do {                                                                         \
    uint32_t idx = (q) / (sizeof((h)->bitmap[0]) * NBBY);                      \
    uint32_t bit = (q) % (sizeof((h)->bitmap[0]) * NBBY);                      \
...
#define HW_XSTAT(m)                                                            \
  { #m, offsetof(struct txgbe_hw_stats, m) }
#define HW_XSTAT_NAME(m, n)                                                    \
  { n, offsetof(struct txgbe_hw_stats, m) }
...
#define TXGBE_NB_HW_STATS                                                      \
  (sizeof(rte_txgbe_stats_strings) / sizeof(rte_txgbe_stats_strings[0]))

/* Per-priority statistics */
#define UP_XSTAT(m)                                                            \
  { #m, offsetof(struct txgbe_hw_stats, up[0].m) }
static const struct rte_txgbe_xstats_name_off rte_txgbe_up_strings[] = {
    UP_XSTAT(rx_up_packets),         UP_XSTAT(tx_up_packets),
    UP_XSTAT(rx_up_bytes),           UP_XSTAT(tx_up_bytes),
    UP_XSTAT(rx_up_drop_packets),

    UP_XSTAT(tx_up_xon_packets),     UP_XSTAT(rx_up_xon_packets),
    UP_XSTAT(tx_up_xoff_packets),    UP_XSTAT(rx_up_xoff_packets),
    UP_XSTAT(rx_up_dropped),         UP_XSTAT(rx_up_mbuf_alloc_errors),
    UP_XSTAT(tx_up_xon2off_packets),
};

#define TXGBE_NB_UP_STATS                                                      \
  (sizeof(rte_txgbe_up_strings) / sizeof(rte_txgbe_up_strings[0]))

/* Per-queue statistics */
#define QP_XSTAT(m)                                                            \
  { #m, offsetof(struct txgbe_hw_stats, qp[0].m) }
static const struct rte_txgbe_xstats_name_off rte_txgbe_qp_strings[] = {
    QP_XSTAT(rx_qp_packets), QP_XSTAT(tx_qp_packets),    QP_XSTAT(rx_qp_bytes),
    QP_XSTAT(tx_qp_bytes),   QP_XSTAT(rx_qp_mc_packets),
};

#define TXGBE_NB_QP_STATS                                                      \
  (sizeof(rte_txgbe_qp_strings) / sizeof(rte_txgbe_qp_strings[0]))

static inline int txgbe_is_sfp(struct txgbe_hw *hw) {
  switch (hw->phy.type) {
...
static inline int32_t txgbe_pf_reset_hw(struct txgbe_hw *hw) {
  uint32_t ctrl_ext;
  int32_t status;
...
static inline void txgbe_enable_intr(struct rte_eth_dev *dev) {
...
static void txgbe_disable_intr(struct txgbe_hw *hw) {
  PMD_INIT_FUNC_TRACE();
...
static int txgbe_dev_queue_stats_mapping_set(struct rte_eth_dev *eth_dev,
                                             uint16_t queue_id,
                                             uint8_t stat_idx, uint8_t is_rx) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(eth_dev);
  struct txgbe_stat_mappings *stat_mappings = TXGBE_DEV_STAT_MAPPINGS(eth_dev);
  ...
  PMD_INIT_LOG(DEBUG, "Setting port %d, %s queue_id %d to stat index %d",
               (int)(eth_dev->data->port_id), is_rx ? "RX" : "TX", queue_id,
               stat_idx);
...
static void txgbe_dcb_init(struct txgbe_hw *hw,
 ...
 */
static void txgbe_swfw_lock_reset(struct txgbe_hw *hw) {
  uint16_t mask;
   ...
   * semaphore.
   */
  mask = TXGBE_MNGSEM_SWPHY | TXGBE_MNGSEM_SWMBX | TXGBE_MNGSEM_SWFLASH;
...
static int txgbe_handle_devarg(__rte_unused const char *key, const char *value,
                               void *extra_args) {
...
static void txgbe_parse_devargs(struct txgbe_hw *hw,
  ...
  rte_kvargs_process(kvlist, TXGBE_DEVARG_BP_AUTO, &txgbe_handle_devarg,
                     &auto_neg);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_KR_POLL, &txgbe_handle_devarg, &poll);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_KR_PRESENT, &txgbe_handle_devarg,
                     &present);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_KX_SGMII, &txgbe_handle_devarg,
                     &sgmii);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_FFE_SET, &txgbe_handle_devarg,
                     &ffe_set);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_FFE_MAIN, &txgbe_handle_devarg,
                     &ffe_main);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_FFE_PRE, &txgbe_handle_devarg,
                     &ffe_pre);
  rte_kvargs_process(kvlist, TXGBE_DEVARG_FFE_POST, &txgbe_handle_devarg,
                     &ffe_post);
...
static int eth_txgbe_dev_init(struct rte_eth_dev *eth_dev,
  ...
  if (pci_dev->id.subsystem_vendor_id == PCI_VENDOR_ID_WANGXUN) {
    ...
    if (ssid == 0x1) {
      PMD_INIT_LOG(ERR, "Read of internal subsystem device id failed\n");
      return -ENODEV;
    }
  ...
  mz = rte_eth_dma_zone_reserve(eth_dev, "txgbe_driver", -1, 16, TXGBE_ALIGN,
                                SOCKET_ID_ANY);
  if (mz == NULL)
    return -ENOMEM;
  ...
  eth_dev->data->mac_addrs =
      rte_zmalloc("txgbe", RTE_ETHER_ADDR_LEN * hw->mac.num_rar_entries, 0);
  ...
  eth_dev->data->hash_mac_addrs =
      rte_zmalloc("txgbe", RTE_ETHER_ADDR_LEN * TXGBE_VMDQ_NUM_UC_MAC, 0);
  ...
  if (txgbe_is_sfp(hw) && hw->phy.sfp_type != txgbe_sfp_type_not_present)
    PMD_INIT_LOG(DEBUG, "MAC: %d, PHY: %d, SFP+: %d", (int)hw->mac.type,
                 (int)hw->phy.type, (int)hw->phy.sfp_type);
  else
    PMD_INIT_LOG(DEBUG, "MAC: %d, PHY: %d", (int)hw->mac.type,
                 (int)hw->phy.type);
  ...
  rte_intr_callback_register(intr_handle, txgbe_dev_interrupt_handler, eth_dev);
  ...
  memset(filter_info, 0, sizeof(struct txgbe_filter_info));
...
static int eth_txgbe_dev_uninit(struct rte_eth_dev *eth_dev) {
  PMD_INIT_FUNC_TRACE();
...
static int txgbe_ntuple_filter_uninit(struct rte_eth_dev *eth_dev) {
  ...
  while ((p_5tuple = TAILQ_FIRST(&filter_info->fivetuple_list))) {
    TAILQ_REMOVE(&filter_info->fivetuple_list, p_5tuple, entries);
    rte_free(p_5tuple);
  }
...
static int txgbe_fdir_filter_init(struct rte_eth_dev *eth_dev) {
  ...
  snprintf(fdir_hash_name, RTE_HASH_NAMESIZE, "fdir_%s", TDEV_NAME(eth_dev));
  fdir_info->hash_handle = rte_hash_create(&fdir_hash_params);
  if (!fdir_info->hash_handle) {
    PMD_INIT_LOG(ERR, "Failed to create fdir hash table!");
    return -EINVAL;
  }
  fdir_info->hash_map = rte_zmalloc(
      "txgbe", sizeof(struct txgbe_fdir_filter *) * TXGBE_MAX_FDIR_FILTER_NUM,
      0);
  if (!fdir_info->hash_map) {
    PMD_INIT_LOG(ERR, "Failed to allocate memory for fdir hash map!");
    return -ENOMEM;
  }
...
static int txgbe_l2_tn_filter_init(struct rte_eth_dev *eth_dev) {
  ...
  snprintf(l2_tn_hash_name, RTE_HASH_NAMESIZE, "l2_tn_%s", TDEV_NAME(eth_dev));
  l2_tn_info->hash_handle = rte_hash_create(&l2_tn_hash_params);
  if (!l2_tn_info->hash_handle) {
    PMD_INIT_LOG(ERR, "Failed to create L2 TN hash table!");
    return -EINVAL;
  }
  l2_tn_info->hash_map = rte_zmalloc(
      "txgbe", sizeof(struct txgbe_l2_tn_filter *) * TXGBE_MAX_L2_TN_FILTER_NUM,
      0);
  if (!l2_tn_info->hash_map) {
    PMD_INIT_LOG(ERR, "Failed to allocate memory for L2 TN hash map!");
    return -ENOMEM;
  }
...
static int eth_txgbe_pci_probe(struct rte_pci_driver *pci_drv __rte_unused,
                               struct rte_pci_device *pci_dev) {
  return rte_eth_dev_create(
      &pci_dev->device, pci_dev->device.name, sizeof(struct txgbe_adapter),
      eth_dev_pci_specific_init, pci_dev, eth_txgbe_dev_init, NULL);
}
...
static int txgbe_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vlan_id,
                                 int on) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
static void txgbe_vlan_strip_queue_set(struct rte_eth_dev *dev, uint16_t queue,
                                       int on) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  ...
  if (rxq->offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP) {
    restart = (rxcfg & TXGBE_RXCFG_ENA) && !(rxcfg & TXGBE_RXCFG_VLAN);
...
static int txgbe_vlan_tpid_set(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_VLAN_TYPE_INNER:
    if (vlan_ext) {
      wr32m(hw, TXGBE_VLANCTL, TXGBE_VLANCTL_TPID_MASK,
            TXGBE_VLANCTL_TPID(tpid));
      wr32m(hw, TXGBE_DMATXCTRL, TXGBE_DMATXCTRL_TPID_MASK,
    ...
    if (qinq) {
      wr32m(hw, TXGBE_TAGTPID(0), TXGBE_TAGTPID_LSB_MASK,
  ...
  case RTE_ETH_VLAN_TYPE_OUTER:
    if (vlan_ext) {
      ...
      wr32m(hw, TXGBE_EXTAG, TXGBE_EXTAG_VLAN_MASK, TXGBE_EXTAG_VLAN(tpid));
    } else {
      wr32m(hw, TXGBE_VLANCTL, TXGBE_VLANCTL_TPID_MASK,
            TXGBE_VLANCTL_TPID(tpid));
      wr32m(hw, TXGBE_DMATXCTRL, TXGBE_DMATXCTRL_TPID_MASK,
    ...
    if (qinq) {
      wr32m(hw, TXGBE_TAGTPID(0), TXGBE_TAGTPID_MSB_MASK,
...
void txgbe_vlan_hw_strip_bitmap_set(struct rte_eth_dev *dev, uint16_t queue,
                                    bool on) {
...
static void txgbe_vlan_hw_strip_enable(struct rte_eth_dev *dev,
                                       uint16_t queue) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t ctrl;
...
static void txgbe_vlan_hw_extend_disable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t ctrl;
...
static void txgbe_vlan_hw_extend_enable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t ctrl;
...
static void txgbe_qinq_hw_strip_disable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t ctrl;
...
static void txgbe_qinq_hw_strip_enable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t ctrl;
...
void txgbe_config_vlan_strip_on_all_queues(struct rte_eth_dev *dev, int mask) {
  uint16_t i;
...
static int txgbe_vlan_offload_config(struct rte_eth_dev *dev, int mask) {
  struct rte_eth_rxmode *rxmode;
  rxmode = &dev->data->dev_conf.rxmode;
...
static int txgbe_vlan_offload_set(struct rte_eth_dev *dev, int mask) {
...
static void txgbe_vmdq_vlan_hw_filter_enable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
static int txgbe_check_vf_rss_rxq_num(struct rte_eth_dev *dev,
                                      uint16_t nb_rx_q) {
...
static int txgbe_check_mq_mode(struct rte_eth_dev *dev) {
  ...
  if (RTE_ETH_DEV_SRIOV(dev).active != 0) {
    ...
    if ((nb_rx_q > RTE_ETH_DEV_SRIOV(dev).nb_q_per_pool) ||
      ...
      PMD_INIT_LOG(ERR,
                   ...
                   nb_rx_q, nb_tx_q, RTE_ETH_DEV_SRIOV(dev).nb_q_per_pool);
      return -EINVAL;
    }
  } else {
    ...
    if (dev_conf->rxmode.mq_mode == RTE_ETH_MQ_RX_VMDQ_DCB) {
      ...
      if (nb_rx_q != TXGBE_VMDQ_DCB_NB_QUEUES) {
        PMD_INIT_LOG(ERR, "VMDQ+DCB, nb_rx_q != %d.", TXGBE_VMDQ_DCB_NB_QUEUES);
        return -EINVAL;
      }
    ...
    if (dev_conf->rxmode.mq_mode == RTE_ETH_MQ_RX_DCB) {
      ...
      if (!(conf->nb_tcs == RTE_ETH_4_TCS || conf->nb_tcs == RTE_ETH_8_TCS)) {
        PMD_INIT_LOG(ERR,
...
static int txgbe_dev_configure(struct rte_eth_dev *dev) {
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
  ...
  if (ret != 0) {
    PMD_DRV_LOG(ERR, "txgbe_check_mq_mode fails with %d.", ret);
    return ret;
  }
...
static void txgbe_dev_phy_intr_setup(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
 ...
 */
static int txgbe_dev_start(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  ...
  if ((rte_intr_cap_multiple(intr_handle) || !RTE_ETH_DEV_SRIOV(dev).active) &&
    ...
    for (vf = 0; vf < pci_dev->max_vfs; vf++)
      for (idx = 0; idx < TXGBE_MAX_QUEUE_NUM_PER_VF; idx++)
        if (vfinfo[vf].tx_rate[idx] != 0)
          txgbe_set_vf_rate_limit(dev, vf, vfinfo[vf].tx_rate[idx], 1 << idx);
  }
  ...
  if (hw->mac.type == txgbe_mac_raptor && dev->data->dev_conf.lpbk_mode)
  ...
  allowed_speeds =
      RTE_ETH_LINK_SPEED_100M | RTE_ETH_LINK_SPEED_1G | RTE_ETH_LINK_SPEED_10G;
  ...
  if (*link_speeds == RTE_ETH_LINK_SPEED_AUTONEG) {
    speed = (TXGBE_LINK_SPEED_100M_FULL | TXGBE_LINK_SPEED_1GB_FULL |
...
skip_link_setup:
  ...
  if (rte_intr_allow_others(intr_handle)) {
    ...
    rte_intr_callback_unregister(intr_handle, txgbe_dev_interrupt_handler, dev);
    if (dev->data->dev_conf.intr_conf.lsc != 0)
  ...
  if (tm_conf->root && !tm_conf->committed)
    PMD_DRV_LOG(WARNING, "please call hierarchy_commit() "
 ...
 */
static int txgbe_dev_stop(struct rte_eth_dev *dev) {
  struct rte_eth_link link;
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_vf_info *vfinfo = *TXGBE_DEV_VFDATA(dev);
  ...
  if (!rte_intr_allow_others(intr_handle))
    ...
    rte_intr_callback_register(intr_handle, txgbe_dev_interrupt_handler,
                               (void *)dev);
...
static int txgbe_dev_close(struct rte_eth_dev *dev) {
  ...
  do {
    ret = rte_intr_callback_unregister(intr_handle, txgbe_dev_interrupt_handler,
                                       dev);
    if (ret >= 0 || ret == -ENOENT) {
      break;
    } else if (ret != -EAGAIN) {
 ...
 * Reset PF device.
 */
static int txgbe_dev_reset(struct rte_eth_dev *dev) {
  int ret;
...
void txgbe_read_stats_registers(struct txgbe_hw *hw,
  ...
  for (i = 0; i < hw->nb_rx_queues; i++) {
    UPDATE_QP_COUNTER_32bit(TXGBE_QPRXPKT(i), hw->qp_last[i].rx_qp_packets,
    ...
    UPDATE_QP_COUNTER_32bit(TXGBE_QPRXMPKT(i), hw->qp_last[i].rx_qp_mc_packets,
                            hw_stats->qp[i].rx_qp_mc_packets);
  }

  for (i = 0; i < hw->nb_tx_queues; i++) {
    UPDATE_QP_COUNTER_32bit(TXGBE_QPTXPKT(i), hw->qp_last[i].tx_qp_packets,
  ...
  for (i = 0; i < TXGBE_MAX_UP; i++) {
    hw_stats->up[i].rx_up_xon_packets += rd32(hw, TXGBE_PBRXUPXON(i));
    hw_stats->up[i].rx_up_xoff_packets += rd32(hw, TXGBE_PBRXUPXOFF(i));
    hw_stats->up[i].tx_up_xon_packets += rd32(hw, TXGBE_PBTXUPXON(i));
    hw_stats->up[i].tx_up_xoff_packets += rd32(hw, TXGBE_PBTXUPXOFF(i));
    hw_stats->up[i].tx_up_xon2off_packets += rd32(hw, TXGBE_PBTXUPOFF(i));
    hw_stats->up[i].rx_up_dropped += rd32(hw, TXGBE_PBRXMISS(i));
  }
  ...
  hw_stats->tx_macsec_pkts_encrypted += rd32(hw, TXGBE_LSECTX_ENCPKT);
  hw_stats->tx_macsec_pkts_protected += rd32(hw, TXGBE_LSECTX_PROTPKT);
  hw_stats->tx_macsec_octets_encrypted += rd32(hw, TXGBE_LSECTX_ENCOCT);
  hw_stats->tx_macsec_octets_protected += rd32(hw, TXGBE_LSECTX_PROTOCT);
  ...
  for (i = 0; i < 2; i++) {
    hw_stats->rx_macsec_sa_pkts_ok += rd32(hw, TXGBE_LSECRX_OKPKT(i));
    hw_stats->rx_macsec_sa_pkts_invalid += rd32(hw, TXGBE_LSECRX_INVPKT(i));
    hw_stats->rx_macsec_sa_pkts_notvalid += rd32(hw, TXGBE_LSECRX_BADPKT(i));
  }
  hw_stats->rx_macsec_sa_pkts_unusedsa += rd32(hw, TXGBE_LSECRX_INVSAPKT);
  hw_stats->rx_macsec_sa_pkts_notusingsa += rd32(hw, TXGBE_LSECRX_BADSAPKT);

  hw_stats->rx_total_missed_packets = 0;
  for (i = 0; i < TXGBE_MAX_UP; i++) {
    hw_stats->rx_total_missed_packets += hw_stats->up[i].rx_up_dropped;
  }
}

static int txgbe_dev_stats_get(struct rte_eth_dev *dev,
                               struct rte_eth_stats *stats) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_hw_stats *hw_stats = TXGBE_DEV_STATS(dev);
  struct txgbe_stat_mappings *stat_mappings = TXGBE_DEV_STAT_MAPPINGS(dev);
  uint32_t i, j;
  ...
  for (i = 0; i < TXGBE_MAX_QP; i++) {
    ...
    j = (q_map < RTE_ETHDEV_QUEUE_STAT_CNTRS
             ? q_map
             : q_map % RTE_ETHDEV_QUEUE_STAT_CNTRS);
    stats->q_ipackets[j] += hw_stats->qp[i].rx_qp_packets;
    stats->q_ibytes[j] += hw_stats->qp[i].rx_qp_bytes;

    q_map = (stat_mappings->tqsm[n] >> offset) & QMAP_FIELD_RESERVED_BITS_MASK;
    j = (q_map < RTE_ETHDEV_QUEUE_STAT_CNTRS
             ? q_map
             : q_map % RTE_ETHDEV_QUEUE_STAT_CNTRS);
  ...
  /* Rx Errors */
  stats->imissed = hw_stats->rx_total_missed_packets + hw_stats->rx_dma_drop;
  stats->ierrors =
      hw_stats->rx_crc_errors + hw_stats->rx_mac_short_packet_dropped +
      hw_stats->rx_length_errors + hw_stats->rx_undersize_errors +
      hw_stats->rx_rdb_drop + hw_stats->rx_illegal_byte_errors +
      hw_stats->rx_error_bytes + hw_stats->rx_fragment_errors +
      hw_stats->rx_fcoe_crc_errors + hw_stats->rx_fcoe_mbuf_allocation_errors;
...
static unsigned txgbe_xstats_calc_num(struct rte_eth_dev *dev) {
  int nb_queues = max(dev->data->nb_rx_queues, dev->data->nb_tx_queues);
  return TXGBE_NB_HW_STATS + TXGBE_NB_UP_STATS * TXGBE_MAX_UP +
         TXGBE_NB_QP_STATS * nb_queues;
}

static inline int txgbe_get_name_by_id(uint32_t id, char *name, uint32_t size) {
  int nb, st;

  /* Extended stats from txgbe_hw_stats */
  if (id < TXGBE_NB_HW_STATS) {
    snprintf(name, size, "[hw]%s", rte_txgbe_stats_strings[id].name);
    return 0;
  }
  ...
  if (id < TXGBE_NB_UP_STATS * TXGBE_MAX_UP) {
    ...
    snprintf(name, size, "[p%u]%s", nb, rte_txgbe_up_strings[st].name);
    return 0;
  }
  ...
  if (id < TXGBE_NB_QP_STATS * TXGBE_MAX_QP) {
    ...
    snprintf(name, size, "[q%u]%s", nb, rte_txgbe_qp_strings[st].name);
    return 0;
  }
...
static inline int txgbe_get_offset_by_id(uint32_t id, uint32_t *offset) {
  int nb, st;
...
static int
txgbe_dev_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                                 struct rte_eth_xstat_name *xstats_names,
                                 unsigned int limit) {
  unsigned int i;
...
static int txgbe_dev_xstats_get(struct rte_eth_dev *dev,
...
static int txgbe_dev_xstats_get_(struct rte_eth_dev *dev, uint64_t *values,
                                 unsigned int limit) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_hw_stats *hw_stats = TXGBE_DEV_STATS(dev);
  unsigned int i, count;
...
static int txgbe_dev_xstats_get_by_id(struct rte_eth_dev *dev,
...
static int txgbe_fw_version_get(struct rte_eth_dev *dev, char *fw_version,
                                size_t fw_size) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  u32 etrack_id;
  int ret;
...
static int txgbe_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->rx_offload_capa =
      (txgbe_get_rx_port_offloads(dev) | dev_info->rx_queue_offload_capa);
...
static void txgbe_dev_sfp_event(struct rte_eth_dev *dev) {
  struct txgbe_interrupt *intr = TXGBE_DEV_INTR(dev);
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  u32 reg;
...
static void txgbe_dev_overheat(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  s32 temp_state;
 ...
 */
int txgbe_dev_wait_setup_link_complete(struct rte_eth_dev *dev,
                                       uint32_t timeout_ms) {
...
static uint32_t txgbe_dev_setup_link_thread_handler(void *param) {
  struct rte_eth_dev *dev = (struct rte_eth_dev *)param;
  struct txgbe_adapter *ad = TXGBE_DEV_ADAPTER(dev);
...
int txgbe_dev_link_update_share(struct rte_eth_dev *dev, int wait_to_complete) {
  struct txgbe_adapter *ad = TXGBE_DEV_ADAPTER(dev);
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct rte_eth_link link;
  ...
  if (link_up == 0) {
    if ((hw->subsystem_device_id & 0xFF) == TXGBE_DEV_ID_KR_KX_KX4) {
      ...
      if (!__atomic_test_and_set(&ad->link_thread_running, __ATOMIC_SEQ_CST)) {
         ...
         */
        intr->flags |= TXGBE_FLAG_NEED_LINK_CONFIG;
        if (rte_thread_create_internal_control(
                &ad->link_thread_tid, "txgbe-link",
                txgbe_dev_setup_link_thread_handler, dev) < 0) {
          PMD_DRV_LOG(ERR, "Create link thread failed!");
          __atomic_clear(&ad->link_thread_running, __ATOMIC_SEQ_CST);
        }
      } else {
        PMD_DRV_LOG(ERR, "Other link thread is running now!");
      }
    }
    return rte_eth_linkstatus_set(dev, &link);
  ...
  /* Re configure MAC RX */
  if (hw->mac.type == txgbe_mac_raptor)
    wr32m(hw, TXGBE_MACRXFLT, TXGBE_MACRXFLT_PROMISC, TXGBE_MACRXFLT_PROMISC);
...
static int txgbe_dev_promiscuous_enable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t fctrl;
...
static int txgbe_dev_promiscuous_disable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t fctrl;
...
static void txgbe_dev_link_status_print(struct rte_eth_dev *dev) {
  ...
  if (link.link_status) {
    PMD_INIT_LOG(INFO, "Port %d: Link Up - speed %u Mbps - %s",
                 (int)(dev->data->port_id), (unsigned int)link.link_speed,
                 link.link_duplex == RTE_ETH_LINK_FULL_DUPLEX ? "full-duplex"
                                                              : "half-duplex");
  } else {
    PMD_INIT_LOG(INFO, " Port %d: Link Down", (int)(dev->data->port_id));
  }
  PMD_INIT_LOG(DEBUG, "PCI Address: " PCI_PRI_FMT, pci_dev->addr.domain,
...
static int txgbe_dev_interrupt_action(struct rte_eth_dev *dev,
  ...
  if (intr->flags & TXGBE_FLAG_NEED_LINK_UPDATE) {
    ...
    txgbe_dev_link_status_print(dev);
    if (rte_eal_alarm_set(timeout * 1000, txgbe_dev_interrupt_delayed_handler,
                          (void *)dev) < 0) {
...
static int txgbe_dev_led_on(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
...
static int txgbe_dev_led_off(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw;
...
static int txgbe_flow_ctrl_get(struct rte_eth_dev *dev,
...
static int txgbe_flow_ctrl_set(struct rte_eth_dev *dev,
                               struct rte_eth_fc_conf *fc_conf) {
  struct txgbe_hw *hw;
  int err;
  ...
  enum txgbe_fc_mode rte_fcmode_2_txgbe_fcmode[] = {
      txgbe_fc_none, txgbe_fc_rx_pause, txgbe_fc_tx_pause, txgbe_fc_full};
  ...
  if (err == 0 || err == TXGBE_ERR_FC_NOT_NEGOTIATED) {
    wr32m(hw, TXGBE_MACRXFLT, TXGBE_MACRXFLT_CTL_MASK,
          (fc_conf->mac_ctrl_frame_fwd ? TXGBE_MACRXFLT_CTL_NOPS
                                       : TXGBE_MACRXFLT_CTL_DROP));
    txgbe_flush(hw);
...
static int txgbe_priority_flow_ctrl_set(struct rte_eth_dev *dev,
...
int txgbe_dev_rss_reta_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i += 4) {
    ...
    for (j = 0; j < 4; j++) {
      if (RS8(mask, j, 0x1)) {
        reta &= ~(MS32(8 * j, 0xFF));
        reta |= LS32(reta_conf[idx].reta[shift + j], 8 * j, 0xFF);
      }
    }
...
int txgbe_dev_rss_reta_query(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i += 4) {
    ...
    for (j = 0; j < 4; j++) {
      if (RS8(mask, j, 0x1))
        reta_conf[idx].reta[shift + j] = (uint16_t)RS32(reta, 8 * j, 0xFF);
    }
  }
...
static int txgbe_add_rar(struct rte_eth_dev *dev,
                         struct rte_ether_addr *mac_addr, uint32_t index,
                         uint32_t pool) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t enable_addr = 1;

  return txgbe_set_rar(hw, index, mac_addr->addr_bytes, pool, enable_addr);
}
...
static int txgbe_set_default_mac_addr(struct rte_eth_dev *dev,
...
static int txgbe_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  ...
  if (hw->mode)
    ...
    wr32m(hw, TXGBE_FRMSZ, TXGBE_FRMSZ_MAX_MASK, TXGBE_FRMSZ_MAX(frame_size));

  return 0;
}

static uint32_t txgbe_uta_vector(struct txgbe_hw *hw,
                                 struct rte_ether_addr *uc_addr) {
  uint32_t vector = 0;
  ...
  case 3: /* use bits [43:32] of the address */
    vector =
        ((uc_addr->addr_bytes[4]) | (((uint16_t)uc_addr->addr_bytes[5]) << 8));
    break;
...
static int txgbe_uc_hash_table_set(struct rte_eth_dev *dev,
 ...
 */
void txgbe_set_ivar_map(struct txgbe_hw *hw, int8_t direction, uint8_t queue,
                        uint8_t msix_vector) {
  uint32_t tmp, idx;
...
static void txgbe_configure_msix(struct rte_eth_dev *dev) {
  ...
  if (!rte_intr_dp_is_en(intr_handle) && !(gpie & TXGBE_GPIE_MSIX))
    return;
  ...
  wr32(hw, TXGBE_ITR(TXGBE_MISC_VEC_ID),
       TXGBE_ITR_IVAL_10G(TXGBE_QUEUE_ITR_INTERVAL_DEFAULT) | TXGBE_ITR_WRDSA);
}
...
int txgbe_syn_filter_set(struct rte_eth_dev *dev,
                         struct rte_eth_syn_filter *filter, bool add) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
 ...
 *    - On failure, a negative value.
 */
static int txgbe_add_5tuple_filter(struct rte_eth_dev *dev,
                                   struct txgbe_5tuple_filter *filter) {
  struct txgbe_filter_info *filter_info = TXGBE_DEV_FILTER(dev);
  int i, idx, shift;
  ...
  for (i = 0; i < TXGBE_MAX_FTQF_FILTERS; i++) {
    ...
    if (!(filter_info->fivetuple_mask[idx] & (1 << shift))) {
      ...
      filter->index = i;
      TAILQ_INSERT_TAIL(&filter_info->fivetuple_list, filter, entries);
      break;
    }
  }
 ...
 */
static void txgbe_remove_5tuple_filter(struct rte_eth_dev *dev,
                                       struct txgbe_5tuple_filter *filter) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_filter_info *filter_info = TXGBE_DEV_FILTER(dev);
...
ntuple_filter_to_5tuple(struct rte_eth_ntuple_filter *filter,
  ...
  case UINT8_MAX:
    ...
    filter_info->proto = convert_protocol_type(filter->proto);
    break;
  case 0:
...
int txgbe_add_del_ntuple_filter(struct rte_eth_dev *dev,
  ...
  filter =
      txgbe_5tuple_filter_lookup(&filter_info->fivetuple_list, &filter_5tuple);
  if (filter != NULL && add) {
...
int txgbe_add_del_ethertype_filter(struct rte_eth_dev *dev,
  ...
  if (ret >= 0 && add) {
    PMD_DRV_LOG(ERR, "ethertype (0x%04x) filter exists.", filter->ether_type);
    return -EEXIST;
  }
  if (ret < 0 && !add) {
  ...
  if (add) {
    ...
    ret = txgbe_ethertype_filter_insert(filter_info, &ethertype_filter);
    if (ret < 0) {
...
static int txgbe_dev_flow_ops_get(__rte_unused struct rte_eth_dev *dev,
                                  const struct rte_flow_ops **ops) {
  *ops = &txgbe_flow_ops;
  return 0;
}

static u8 *txgbe_dev_addr_list_itr(__rte_unused struct txgbe_hw *hw,
...
static uint64_t txgbe_read_systime_cyclecounter(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
static uint64_t txgbe_read_rx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
static uint64_t txgbe_read_tx_tstamp_cyclecounter(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
static void txgbe_start_timecounters(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
  struct rte_eth_link link;
  uint32_t incval = 0;
...
static int txgbe_timesync_adjust_time(struct rte_eth_dev *dev, int64_t delta) {
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
...
static int txgbe_timesync_write_time(struct rte_eth_dev *dev,
...
static int txgbe_timesync_read_time(struct rte_eth_dev *dev,
                                    struct timespec *ts) {
...
static int txgbe_timesync_enable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t tsync_ctl;
...
static int txgbe_timesync_disable(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t tsync_ctl;
...
static int txgbe_timesync_read_rx_timestamp(struct rte_eth_dev *dev,
...
static int txgbe_timesync_read_tx_timestamp(struct rte_eth_dev *dev,
                                            struct timespec *timestamp) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_adapter *adapter = TXGBE_DEV_ADAPTER(dev);
...
static int txgbe_get_reg_length(struct rte_eth_dev *dev __rte_unused) {
  int count = 0;
  int g_ind = 0;
...
static int txgbe_get_regs(struct rte_eth_dev *dev,
  ...
  if (regs->length == 0 ||
    ...
    while ((reg_group = reg_set[g_ind++]))
      count += txgbe_read_regs_group(dev, &data[count], reg_group);
    return 0;
  }
...
static int txgbe_get_eeprom_length(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
static int txgbe_get_eeprom(struct rte_eth_dev *dev,
  ...
  if (first > hw->rom.word_size || ((first + length) > hw->rom.word_size))
    return -EINVAL;
...
static int txgbe_set_eeprom(struct rte_eth_dev *dev,
                            struct rte_dev_eeprom_info *in_eeprom) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_rom_info *eeprom = &hw->rom;
  uint16_t *data = in_eeprom->data;
...
static int txgbe_get_module_info(struct rte_eth_dev *dev,
  ...
  status = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_SFF_8472_COMP, &sff8472_rev);
  if (status != 0)
    return -EIO;

  /* addressing mode is not supported */
  status = hw->phy.read_i2c_eeprom(hw, TXGBE_SFF_SFF_8472_SWAP, &addr_mode);
  if (status != 0)
    return -EIO;
...
static int txgbe_get_module_eeprom(struct rte_eth_dev *dev,
...
static int txgbe_dev_get_dcb_info(struct rte_eth_dev *dev,
  ...
  if (dcb_config->vt_mode) { /* vt is enabled */
    ...
    if (RTE_ETH_DEV_SRIOV(dev).active > 0) {
      ...
      for (i = 0; i < vmdq_rx_conf->nb_queue_pools; i++) {
        for (j = 0; j < nb_tcs; j++) {
          tc_queue->tc_rxq[i][j].base = i * nb_tcs + j;
          tc_queue->tc_rxq[i][j].nb_queue = 1;
          tc_queue->tc_txq[i][j].base = i * nb_tcs + j;
...
static int txgbe_update_e_tag_eth_type(struct txgbe_hw *hw,
                                       uint16_t ether_type) {
  uint32_t etag_etype;
...
/* Enable e-tag tunnel */
static int txgbe_e_tag_enable(struct txgbe_hw *hw) {
  uint32_t etag_etype;
...
static int txgbe_e_tag_filter_del(struct rte_eth_dev *dev,
  ...
  for (i = 1; i < rar_entries; i++) {
    ...
    if ((rar_high & TXGBE_ETHADDRH_VLD) && (rar_high & TXGBE_ETHADDRH_ETAG) &&
        (TXGBE_ETHADDRL_ETAG(rar_low) == l2_tunnel->tunnel_id)) {
...
static int txgbe_e_tag_filter_add(struct rte_eth_dev *dev,
                                  struct txgbe_l2_tunnel_conf *l2_tunnel) {
  int ret = 0;
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  uint32_t i, rar_entries;
...
txgbe_insert_l2_tn_filter(struct txgbe_l2_tn_info *l2_tn_info,
  ...
  ret = rte_hash_add_key(l2_tn_info->hash_handle, &l2_tn_filter->key);
...
static inline int txgbe_remove_l2_tn_filter(struct txgbe_l2_tn_info *l2_tn_info,
                                            struct txgbe_l2_tn_key *key) {
  int ret;
  struct txgbe_l2_tn_filter *l2_tn_filter;
  ...
  if (ret < 0) {
    PMD_DRV_LOG(ERR, "No such L2 tunnel filter to delete %d!", ret);
    return ret;
  }
...
int txgbe_dev_l2_tunnel_filter_add(struct rte_eth_dev *dev,
                                   struct txgbe_l2_tunnel_conf *l2_tunnel,
                                   bool restore) {
  int ret;
  ...
  if (!restore) {
    ...
    node = rte_zmalloc("txgbe_l2_tn", sizeof(struct txgbe_l2_tn_filter), 0);
    if (!node)
      return -ENOMEM;

    rte_memcpy(&node->key, &key, sizeof(struct txgbe_l2_tn_key));
...
int txgbe_dev_l2_tunnel_filter_del(struct rte_eth_dev *dev,
                                   struct txgbe_l2_tunnel_conf *l2_tunnel) {
  int ret;
  struct txgbe_l2_tn_info *l2_tn_info = TXGBE_DEV_L2_TN(dev);
  struct txgbe_l2_tn_key key;
...
static int txgbe_e_tag_forwarding_en_dis(struct rte_eth_dev *dev, bool en) {
  int ret = 0;
  uint32_t ctrl;
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
...
txgbe_dev_udp_tunnel_port_del(struct rte_eth_dev *dev,
  ...
  case RTE_ETH_TUNNEL_TYPE_VXLAN:
    ...
    if (cur_port != udp_tunnel->udp_port) {
      PMD_DRV_LOG(ERR, "Port %u does not exist.", udp_tunnel->udp_port);
      ret = -EINVAL;
      break;
    }
...
static inline void txgbe_ntuple_filter_restore(struct rte_eth_dev *dev) {
  struct txgbe_filter_info *filter_info = TXGBE_DEV_FILTER(dev);
...
static inline void txgbe_ethertype_filter_restore(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_filter_info *filter_info = TXGBE_DEV_FILTER(dev);
  int i;
  ...
  for (i = 0; i < TXGBE_ETF_ID_MAX; i++) {
    if (filter_info->ethertype_mask & (1 << i)) {
      wr32(hw, TXGBE_ETFLT(i), filter_info->ethertype_filters[i].etqf);
      wr32(hw, TXGBE_ETCLS(i), filter_info->ethertype_filters[i].etqs);
      txgbe_flush(hw);
    }
  }
}
...
/* restore SYN filter */
static inline void txgbe_syn_filter_restore(struct rte_eth_dev *dev) {
  struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
  struct txgbe_filter_info *filter_info = TXGBE_DEV_FILTER(dev);
  uint32_t synqf;
...
static inline void txgbe_l2_tn_filter_restore(struct rte_eth_dev *dev) {
  struct txgbe_l2_tn_info *l2_tn_info = TXGBE_DEV_L2_TN(dev);
  struct txgbe_l2_tn_filter *node;
...
/* restore rss filter */
static inline void txgbe_rss_filter_restore(struct rte_eth_dev *dev) {
  struct txgbe_filter_info *filter_info = TXGBE_DEV_FILTER(dev);

  if (filter_info->rss_info.conf.queue_num)
    txgbe_config_rss_filter(dev, &filter_info->rss_info, TRUE);
}
...
void txgbe_clear_all_ethertype_filter(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < TXGBE_ETF_ID_MAX; i++) {
    if (filter_info->ethertype_mask & (1 << i) &&
      ...
      (void)txgbe_ethertype_filter_remove(filter_info, (uint8_t)i);
...
RTE_PMD_REGISTER_PARAM_STRING(net_txgbe, TXGBE_DEVARG_BP_AUTO
                              "=<0|1>" TXGBE_DEVARG_KR_POLL
                              "=<0|1>" TXGBE_DEVARG_KR_PRESENT
                              "=<0|1>" TXGBE_DEVARG_KX_SGMII
                              "=<0|1>" TXGBE_DEVARG_FFE_SET
                              "=<0-4>" TXGBE_DEVARG_FFE_MAIN
                              "=<uint16>" TXGBE_DEVARG_FFE_PRE
                              "=<uint16>" TXGBE_DEVARG_FFE_POST "=<uint16>");
