...
#define AF_XDP_LOG(level, fmt, args...)                                        \
  rte_log(RTE_LOG_##level, af_xdp_logtype, "%s(): " fmt, __func__, ##args)
...
static const char *const valid_arguments[] = {ETH_AF_XDP_IFACE_ARG,
...
static inline int reserve_fill_queue_zc(struct xsk_umem_info *umem,
  ...
  for (i = 0; i < reserve_size; i++) {
    ...
    addr =
        (uint64_t)bufs[i] - (uint64_t)umem->buffer - umem->mb_pool->header_size;
    *fq_addr = addr;
  }
...
#else
static inline int reserve_fill_queue_cp(struct xsk_umem_info *umem,
                                        uint16_t reserve_size,
  ...
  if (unlikely(!xsk_ring_prod__reserve(fq, reserve_size, &idx))) {
    AF_XDP_LOG(DEBUG, "Failed to reserve enough fq descs.\n");
    rte_ring_enqueue_bulk(umem->buf_ring, addrs, reserve_size, NULL);
    return -1;
  }
...
static inline int reserve_fill_queue(struct xsk_umem_info *umem,
                                     uint16_t reserve_size,
                                     struct rte_mbuf **bufs,
                                     struct xsk_ring_prod *fq) {
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
...
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
static uint16_t af_xdp_rx_zc(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  ...
  if (nb_pkts == 0) {
     ...
     */
    if (rxq->busy_budget) {
      (void)recvfrom(xsk_socket__fd(rxq->xsk), NULL, 0, MSG_DONTWAIT, NULL,
                     NULL);
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    bufs[i] = (struct rte_mbuf *)xsk_umem__get_data(
        umem->buffer, addr + umem->mb_pool->header_size);
...
#else
static uint16_t af_xdp_rx_cp(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  struct pkt_rx_queue *rxq = queue;
  struct xsk_ring_cons *rx = &rxq->rx;
  struct xsk_umem_info *umem = rxq->umem;
...
static uint16_t af_xdp_rx(void *queue, struct rte_mbuf **bufs,
                          uint16_t nb_pkts) {
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
...
static uint16_t eth_af_xdp_rx(void *queue, struct rte_mbuf **bufs,
                              uint16_t nb_pkts) {
  uint16_t nb_rx;
...
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
    addr = xsk_umem__extract_addr(addr);
    rte_pktmbuf_free((struct rte_mbuf *)xsk_umem__get_data(
        umem->buffer, addr + umem->mb_pool->header_size));
#else
    rte_ring_enqueue(umem->buf_ring, (void *)addr);
#endif
...
static void kick_tx(struct pkt_tx_queue *txq, struct xsk_ring_cons *cq) {
  struct xsk_umem_info *umem = txq->umem;

  pull_umem_cq(umem, XSK_RING_CONS__DEFAULT_NUM_DESCS, cq);

  if (tx_syscall_needed(&txq->tx))
    while (send(xsk_socket__fd(txq->pair->xsk), NULL, 0, MSG_DONTWAIT) < 0) {
...
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
static uint16_t af_xdp_tx_zc(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  ...
  for (i = 0; i < nb_pkts; i++) {
    ...
    if (mbuf->pool == umem->mb_pool) {
      ...
      addr =
          (uint64_t)mbuf - (uint64_t)umem->buffer - umem->mb_pool->header_size;
      ...
      addr = (uint64_t)local_mbuf - (uint64_t)umem->buffer -
             umem->mb_pool->header_size;
      offset = rte_pktmbuf_mtod(local_mbuf, uint64_t) - (uint64_t)local_mbuf +
               umem->mb_pool->header_size;
...
#else
static uint16_t af_xdp_tx_cp(void *queue, struct rte_mbuf **bufs,
                             uint16_t nb_pkts) {
  struct pkt_tx_queue *txq = queue;
  struct xsk_umem_info *umem = txq->umem;
  struct rte_mbuf *mbuf;
  ...
  nb_pkts = rte_ring_dequeue_bulk(umem->buf_ring, addrs, nb_pkts, NULL);
  if (nb_pkts == 0)
    return 0;
...
static uint16_t af_xdp_tx_cp_batch(void *queue, struct rte_mbuf **bufs,
                                   uint16_t nb_pkts) {
  uint16_t nb_tx;
...
static uint16_t eth_af_xdp_tx(void *queue, struct rte_mbuf **bufs,
                              uint16_t nb_pkts) {
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
...
find_internal_resource(struct pmd_internals *port_int) {
  ...
  TAILQ_FOREACH(list, &internal_list, next) {
    struct pmd_internals *list_int = list->eth_dev->data->dev_private;
...
static int eth_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (internal->shared_umem) {
    ...
    list = rte_zmalloc_socket(name, sizeof(*list), 0, dev->device->numa_node);
    if (list == NULL)
      return -1;
...
static int eth_get_monitor_addr(void *rx_queue,
...
static int eth_dev_info(struct rte_eth_dev *dev,
...
#if defined(XDP_UMEM_UNALIGNED_CHUNK_FLAG)
  dev_info->max_rx_pktlen = getpagesize() - sizeof(struct rte_mempool_objhdr) -
...
static int eth_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    ret = fd >= 0 ? getsockopt(fd, SOL_XDP, XDP_STATISTICS, &xdp_stats, &optlen)
                  : -1;
    if (ret != 0) {
...
static int eth_stats_reset(struct rte_eth_dev *dev) {
  ...
  for (i = 0; i < internals->queue_cnt; i++) {
    memset(&internals->rx_queues[i].stats, 0, sizeof(struct rx_stats));
    memset(&internals->tx_queues[i].stats, 0, sizeof(struct tx_stats));
  }
...
static int remove_xdp_program(struct pmd_internals *internals) {
  ...
  ret = bpf_xdp_detach(internals->if_index, XDP_FLAGS_UPDATE_IF_NOEXIST, NULL);
  if (ret != 0)
...
static int remove_xdp_program(struct pmd_internals *internals) {
  ...
  ret =
      bpf_set_link_xdp_fd(internals->if_index, -1, XDP_FLAGS_UPDATE_IF_NOEXIST);
  if (ret != 0)
...
static inline bool ctx_exists(struct pkt_rx_queue *rxq, const char *ifname,
...
static inline int get_shared_umem(struct pkt_rx_queue *rxq, const char *ifname,
                                  struct xsk_umem_info **umem) {
  struct internal_list *list;
  ...
  TAILQ_FOREACH(list, &internal_list, next) {
    ...
    for (i = 0; i < internals->queue_cnt; i++) {
      struct pkt_rx_queue *list_rxq = &internals->rx_queues[i];
      if (rxq == list_rxq)
        continue;
      if (mb_pool == internals->rx_queues[i].mb_pool) {
        if (ctx_exists(rxq, ifname, list_rxq, internals->if_name)) {
          ret = -1;
          goto out;
        }
...
static struct xsk_umem_info *xdp_umem_configure(struct pmd_internals *internals,
                                                struct pkt_rx_queue *rxq) {
  struct xsk_umem_info *umem = NULL;
  int ret;
  struct xsk_umem_config usr_config = {.fill_size =
                                           ETH_AF_XDP_DFLT_NUM_DESCS * 2,
  ...
  if (internals->shared_umem) {
    ...
    if (umem != NULL &&
        __atomic_load_n(&umem->refcnt, __ATOMIC_ACQUIRE) < umem->max_xsks) {
  ...
  if (umem == NULL) {
    usr_config.frame_size =
        rte_mempool_calc_obj_size(mb_pool->elt_size, mb_pool->flags, NULL);
    usr_config.frame_headroom = mb_pool->header_size + sizeof(struct rte_mbuf) +
    ...
    umem_size =
        (uint64_t)mb_pool->populated_size * (uint64_t)usr_config.frame_size +
        align;

    ret = xsk_umem__create(&umem->umem, base_addr, umem_size, &rxq->fq,
                           &rxq->cq, &usr_config);
    if (ret) {
    ...
    if (internals->shared_umem) {
      umem->max_xsks = mb_pool->populated_size / ETH_AF_XDP_NUM_BUFFERS;
...
static struct xsk_umem_info *xdp_umem_configure(struct pmd_internals *internals,
  ...
  struct xsk_umem_info *umem;
  const struct rte_memzone *mz;
  struct xsk_umem_config usr_config = {.fill_size = ETH_AF_XDP_DFLT_NUM_DESCS,
                                       .comp_size = ETH_AF_XDP_DFLT_NUM_DESCS,
  ...
  umem->buf_ring =
      rte_ring_create(ring_name, ETH_AF_XDP_NUM_BUFFERS, rte_socket_id(), 0x0);
  ...
  snprintf(mz_name, sizeof(mz_name), "af_xdp_umem_%s_%u", internals->if_name,
           rxq->xsk_queue_idx);
  mz = rte_memzone_reserve_aligned(
      mz_name, ETH_AF_XDP_NUM_BUFFERS * ETH_AF_XDP_FRAME_SIZE, rte_socket_id(),
      RTE_MEMZONE_IOVA_CONTIG, getpagesize());
  if (mz == NULL) {
...
static int get_pinned_map(const char *dp_path, int *map_fd) {
...
static int load_custom_xdp_prog(const char *prog_path, int if_index,
                                struct bpf_map **map) {
  int ret, prog_fd;
  ...
  *map = bpf_object__find_map_by_name(obj, "xsks_map");
  ...
  ret = link_xdp_prog_with_dev(if_index, prog_fd, XDP_FLAGS_UPDATE_IF_NOEXIST);
  if (ret) {
...
static int configure_preferred_busy_poll(struct pkt_rx_queue *rxq) {
  int sock_opt = 1;
...
static int init_uds_sock(struct sockaddr_un *server, const char *dp_path) {
  int sock;
...
static int send_msg(int sock, char *request, int *fd, const char *dp_path) {
  int snd;
  struct iovec iov;
  struct msghdr msgh;
  struct cmsghdr *cmsg;
...
static int read_msg(int sock, char *response, struct sockaddr_un *s, int *fd) {
  int msglen;
  struct msghdr msgh;
  struct iovec iov;
  char control[CMSG_SPACE(sizeof(*fd))];
...
static int make_request_dp(int sock, struct sockaddr_un *server, char *request,
...
static int check_response(char *response, char *exp_resp, long size) {
  return strncmp(response, exp_resp, size);
}

static int uds_get_xskmap_fd(char *if_name, const char *dp_path) {
  ...
  if (make_request_dp(sock, &server, request, NULL, response, &out_fd,
                      dp_path) < 0) {
...
static int xsk_configure(struct pmd_internals *internals,
                         struct pkt_rx_queue *rxq, int ring_size) {
  ...
  if (strnlen(internals->prog_path, PATH_MAX)) {
    if (!internals->custom_prog_configured) {
      ret = load_custom_xdp_prog(internals->prog_path, internals->if_index,
                                 &internals->map);
      if (ret) {
  ...
  if (internals->shared_umem)
    ...
    ret = xsk_socket__create(&rxq->xsk, internals->if_name, rxq->xsk_queue_idx,
  ...
  if (internals->custom_prog_configured) {
    ...
    err = bpf_map_update_elem(bpf_map__fd(internals->map), &rxq->xsk_queue_idx,
                              &fd, 0);
    if (err) {
  ...
  if (internals->use_cni || internals->use_pinned_map) {
    ...
    if (internals->use_cni) {
      ...
      if (map_fd < 0) {
        AF_XDP_LOG(ERR,
                   "Failed to receive xskmap fd from AF_XDP Device Plugin\n");
        goto out_xsk;
      }
    } else {
...
static int eth_rx_queue_setup(struct rte_eth_dev *dev, uint16_t rx_queue_id,
                              uint16_t nb_rx_desc,
...
static int parse_budget_arg(const char *key __rte_unused, const char *value,
                            void *extra_args) {
...
static int parse_integer_arg(const char *key __rte_unused, const char *value,
                             void *extra_args) {
  int *i = (int *)extra_args;
  char *end;
...
/** parse name argument */
static int parse_name_arg(const char *key __rte_unused, const char *value,
                          void *extra_args) {
...
static int parse_prog_arg(const char *key __rte_unused, const char *value,
                          void *extra_args) {
...
static int xdp_get_channels_info(const char *if_name, int *max_queues,
                                 int *combined_queues) {
...
static int parse_parameters(struct rte_kvargs *kvlist, char *if_name,
                            int *start_queue, int *queue_cnt, int *shared_umem,
                            char *prog_path, int *busy_budget, int *force_copy,
                            int *use_cni, int *use_pinned_map, char *dp_path) {
  int ret;

  ret = rte_kvargs_process(kvlist, ETH_AF_XDP_IFACE_ARG, &parse_name_arg,
                           if_name);
  if (ret < 0)
    goto free_kvlist;
  ...
  ret = rte_kvargs_process(kvlist, ETH_AF_XDP_PROG_ARG, &parse_prog_arg,
                           prog_path);
  if (ret < 0)
    goto free_kvlist;

  ret = rte_kvargs_process(kvlist, ETH_AF_XDP_BUDGET_ARG, &parse_budget_arg,
                           busy_budget);
  if (ret < 0)
    goto free_kvlist;
  ...
  ret = rte_kvargs_process(kvlist, ETH_AF_XDP_USE_CNI_ARG, &parse_integer_arg,
                           use_cni);
  if (ret < 0)
    goto free_kvlist;
  ...
  ret = rte_kvargs_process(kvlist, ETH_AF_XDP_DP_PATH_ARG, &parse_prog_arg,
                           dp_path);
  if (ret < 0)
    goto free_kvlist;
...
static int get_iface_info(const char *if_name, struct rte_ether_addr *eth_addr,
                          int *if_index) {
  struct ifreq ifr;
...
init_internals(struct rte_vdev_device *dev, const char *if_name,
  ...
  if (queue_cnt > internals->combined_queue_cnt) {
    AF_XDP_LOG(
        ERR,
        "Specified queue count %d is larger than combined queue count %d.\n",
        queue_cnt, internals->combined_queue_cnt);
    goto err_free_internals;
  }

  internals->rx_queues = rte_zmalloc_socket(
      NULL, sizeof(struct pkt_rx_queue) * queue_cnt, 0, numa_node);
  if (internals->rx_queues == NULL) {
    AF_XDP_LOG(ERR, "Failed to allocate memory for rx queues.\n");
    goto err_free_internals;
  }

  internals->tx_queues = rte_zmalloc_socket(
      NULL, sizeof(struct pkt_tx_queue) * queue_cnt, 0, numa_node);
  ...
  process_private = (struct pmd_process_private *)rte_zmalloc_socket(
      name, sizeof(struct pmd_process_private), RTE_CACHE_LINE_SIZE, numa_node);
...
static int afxdp_mp_request_fds(const char *name, struct rte_eth_dev *dev) {
  struct pmd_process_private *process_private = dev->process_private;
  ...
  strlcpy(request_param->port_name, name, sizeof(request_param->port_name));
...
static int afxdp_mp_send_fds(const struct rte_mp_msg *request,
                             const void *peer) {
  struct rte_eth_dev *dev;
  struct pmd_process_private *process_private;
  struct rte_mp_msg reply;
  const struct ipc_hdr *request_param = (const struct ipc_hdr *)request->param;
  struct ipc_hdr *reply_param = (struct ipc_hdr *)reply.param;
  ...
  strlcpy(reply_param->port_name, request_name, sizeof(reply_param->port_name));
...
static int rte_pmd_af_xdp_probe(struct rte_vdev_device *dev) {
  ...
  if (rte_eal_process_type() == RTE_PROC_SECONDARY) {
    ...
    eth_dev->process_private = (struct pmd_process_private *)rte_zmalloc_socket(
        name, sizeof(struct pmd_process_private), RTE_CACHE_LINE_SIZE,
  ...
  if (parse_parameters(kvlist, if_name, &xsk_start_queue_idx, &xsk_queue_cnt,
                       &shared_umem, prog_path, &busy_budget, &force_copy,
                       &use_cni, &use_pinned_map, dp_path) < 0) {
  ...
  if (use_cni && use_pinned_map) {
    AF_XDP_LOG(ERR,
               "When '%s' parameter is used, '%s' parameter is not valid\n",
  ...
  if ((use_cni || use_pinned_map) && busy_budget > 0) {
    AF_XDP_LOG(
        ERR,
        "When '%s' or '%s' parameter is used, '%s' parameter is not valid\n",
        ETH_AF_XDP_USE_CNI_ARG, ETH_AF_XDP_USE_PINNED_MAP_ARG,
        ETH_AF_XDP_BUDGET_ARG);
    return -EINVAL;
  }
  ...
  if (use_cni && !strnlen(dp_path, PATH_MAX)) {
    snprintf(dp_path, sizeof(dp_path), "%s/%s/%s", DP_BASE_PATH, if_name,
             DP_UDS_SOCK);
  ...
  if ((!use_cni && !use_pinned_map) && strnlen(dp_path, PATH_MAX)) {
    AF_XDP_LOG(ERR,
               "'%s' parameter is set, but '%s' or '%s' were not enabled\n",
  ...
  busy_budget = busy_budget == -1 ? ETH_AF_XDP_DFLT_BUSY_BUDGET : busy_budget;

  eth_dev = init_internals(dev, if_name, xsk_start_queue_idx, xsk_queue_cnt,
                           shared_umem, prog_path, busy_budget, force_copy,
                           use_cni, use_pinned_map, dp_path);
  if (eth_dev == NULL) {
...
static int rte_pmd_af_xdp_remove(struct rte_vdev_device *dev) {
  struct rte_eth_dev *eth_dev = NULL;
...
RTE_PMD_REGISTER_PARAM_STRING(net_af_xdp, "iface=<string> "
                                          "start_queue=<int> "
                                          "queue_count=<int> "
                                          "shared_umem=<int> "
                                          "xdp_prog=<string> "
