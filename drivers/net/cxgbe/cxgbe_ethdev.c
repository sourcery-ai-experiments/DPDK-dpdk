...
uint16_t cxgbe_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
  ...
  while (total_sent < nb_pkts) {
    ...
    for (pkts_sent = 0; pkts_sent < pkts_remain; pkts_sent++) {
      ...
      if ((idx + 1) < nb_pkts)
        rte_prefetch0(rte_pktmbuf_mtod(tx_pkts[idx + 1], volatile void *));
...
int cxgbe_dev_promiscuous_enable(struct rte_eth_dev *eth_dev) {
  ...
  return t4_set_rxmode(adapter, adapter->mbox, pi->viid, -1, 1, -1, 1, -1,
                       false);
}
...
int cxgbe_dev_promiscuous_disable(struct rte_eth_dev *eth_dev) {
  ...
  return t4_set_rxmode(adapter, adapter->mbox, pi->viid, -1, 0, -1, 1, -1,
                       false);
}
...
int cxgbe_dev_allmulticast_disable(struct rte_eth_dev *eth_dev) {
  ...
  return t4_set_rxmode(adapter, adapter->mbox, pi->viid, -1, -1, 0, 1, -1,
                       false);
}

int cxgbe_dev_link_update(struct rte_eth_dev *eth_dev, int wait_to_complete) {
  struct port_info *pi = eth_dev->data->dev_private;
  ...
  for (i = 0; i < CXGBE_LINK_STATUS_POLL_CNT; i++) {
    ...
    if (pi->link_cfg.link_ok != old_link || cxgbe_force_linkup(adapter))
      break;
  ...
  new_link.link_status =
      cxgbe_force_linkup(adapter) ? RTE_ETH_LINK_UP : pi->link_cfg.link_ok;
...
int cxgbe_dev_mtu_set(struct rte_eth_dev *eth_dev, uint16_t mtu) {
  ...
  return t4_set_rxmode(adapter, adapter->mbox, pi->viid, new_mtu, -1, -1, -1,
                       -1, true);
}
...
int cxgbe_dev_configure(struct rte_eth_dev *eth_dev) {
  ...
  if (eth_dev->data->dev_conf.rxmode.mq_mode & RTE_ETH_MQ_RX_RSS_FLAG)
    eth_dev->data->dev_conf.rxmode.offloads |= RTE_ETH_RX_OFFLOAD_RSS_HASH;
...
int cxgbe_dev_tx_queue_start(struct rte_eth_dev *eth_dev,
  ...
  struct sge_eth_txq *txq =
      (struct sge_eth_txq *)(eth_dev->data->tx_queues[tx_queue_id]);
...
int cxgbe_dev_tx_queue_setup(
  ...
  dev_debug(adapter,
            "%s: eth_dev->data->nb_tx_queues = %d; queue_idx = %d; nb_desc = "
            "%d; socket_id = %d; pi->first_qset = %u\n",
  ...
  if (nb_desc < CXGBE_MIN_RING_DESC_SIZE) {
    dev_warn(adapter,
             "%s: number of descriptors must be >= %d. Using default [%d]\n",
             __func__, CXGBE_MIN_RING_DESC_SIZE, CXGBE_DEFAULT_TX_DESC_SIZE);
    temp_nb_desc = CXGBE_DEFAULT_TX_DESC_SIZE;
  } else if (nb_desc > CXGBE_MAX_RING_DESC_SIZE) {
    dev_err(adapter,
            "%s: number of descriptors must be between %d and %d inclusive. "
            "Default [%d]\n",
            __func__, CXGBE_MIN_RING_DESC_SIZE, CXGBE_MAX_RING_DESC_SIZE,
...
void cxgbe_dev_tx_queue_release(struct rte_eth_dev *eth_dev, uint16_t qid) {
  ...
  if (txq) {
    struct port_info *pi =
        (struct port_info *)(txq->eth_dev->data->dev_private);
    struct adapter *adap = pi->adapter;

    dev_debug(adapter, "%s: pi->port_id = %d; tx_queue_id = %d\n", __func__,
              pi->port_id, txq->q.cntxt_id);
...
int cxgbe_dev_rx_queue_setup(struct rte_eth_dev *eth_dev, uint16_t queue_idx,
  ...
  dev_debug(adapter,
            "%s: eth_dev->data->nb_rx_queues = %d; queue_idx = %d; nb_desc = "
            "%d; socket_id = %d; mp = %p\n",
  ...
  if (err != 0) {
    dev_err(adap, "%s: error during getting ethernet device info", __func__);
    return err;
  }
  ...
  if (nb_desc < CXGBE_MIN_RING_DESC_SIZE) {
    dev_warn(adapter,
             "%s: number of descriptors must be >= %d. Using default [%d]\n",
             __func__, CXGBE_MIN_RING_DESC_SIZE, CXGBE_DEFAULT_RX_DESC_SIZE);
  ...
  err = t4_sge_alloc_rxq(
      adapter, &rxq->rspq, false, eth_dev, msi_idx, &rxq->fl, NULL,
      is_pf4(adapter) ? t4_get_tp_ch_map(adapter, pi->tx_chan) : 0, mp,
      queue_idx, socket_id);

  dev_debug(adapter, "%s: err = %d; port_id = %d; cntxt_id = %u; abs_id = %u\n",
            __func__, err, pi->port_id, rxq->rspq.cntxt_id, rxq->rspq.abs_id);
  return err;
}
...
void cxgbe_dev_rx_queue_release(struct rte_eth_dev *eth_dev, uint16_t qid) {
  ...
  if (rxq) {
    struct port_info *pi =
        (struct port_info *)(rxq->rspq.eth_dev->data->dev_private);
    struct adapter *adap = pi->adapter;

    dev_debug(adapter, "%s: pi->port_id = %d; rx_queue_id = %d\n", __func__,
              pi->port_id, rxq->rspq.cntxt_id);
...
static int cxgbe_dev_stats_get(struct rte_eth_dev *eth_dev,
  ...
  /* RX Stats */
  eth_stats->imissed = ps.rx_ovflow0 + ps.rx_ovflow1 + ps.rx_ovflow2 +
                       ps.rx_ovflow3 + ps.rx_trunc0 + ps.rx_trunc1 +
                       ps.rx_trunc2 + ps.rx_trunc3;
  for (i = 0; i < NCHAN; i++)
    eth_stats->imissed += ps.rx_tp_tnl_cong_drops[i];

  eth_stats->ierrors = ps.rx_symbol_err + ps.rx_fcs_err + ps.rx_jabber +
                       ps.rx_too_long + ps.rx_runt + ps.rx_len_err;
...
static const struct cxgbe_dev_xstats_name_off cxgbe_dev_txq_stats_strings[] = {
    ...
    {"packet_mapping_errors", offsetof(struct sge_eth_tx_stats, mapping_err)},
...
static const struct cxgbe_dev_xstats_name_off cxgbe_dev_port_stats_strings[] = {
    ...
    {"rx_symbol_error_packets", offsetof(struct port_stats, rx_symbol_err)},
...
static const struct cxgbe_dev_xstats_name_off cxgbevf_dev_port_stats_strings[] =
    {
        {"tx_bytes", offsetof(struct port_stats, tx_octets)},
        {"tx_broadcast_packets", offsetof(struct port_stats, tx_bcast_frames)},
        {"tx_multicast_packets", offsetof(struct port_stats, tx_mcast_frames)},
...
static int cxgbe_dev_xstats(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < nstats; i++, count++) {
    if (xstats_names != NULL)
      snprintf(xstats_names[count].name, sizeof(xstats_names[count].name), "%s",
               xstats_str[i].name);
    if (xstats != NULL) {
      stats_ptr = RTE_PTR_ADD(&ps, xstats_str[i].offset);
    ...
    for (i = 0; i < CXGBE_NB_TXQ_STATS; i++, count++) {
      if (xstats_names != NULL)
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "tx_q%u_%s", qid, xstats_str[i].name);
      if (xstats != NULL) {
        stats_ptr = RTE_PTR_ADD(&txq->stats, xstats_str[i].offset);
        xstats[count].value = *stats_ptr;
        xstats[count].id = count;
      }
    }
  ...
  for (qid = 0; qid < pi->n_rx_qsets; qid++) {
    ...
    for (i = 0; i < CXGBE_NB_RXQ_STATS; i++, count++) {
      if (xstats_names != NULL)
        snprintf(xstats_names[count].name, sizeof(xstats_names[count].name),
                 "rx_q%u_%s", qid, xstats_str[i].name);
      if (xstats != NULL) {
        stats_ptr = RTE_PTR_ADD(&rxq->stats, xstats_str[i].offset);
        xstats[count].value = *stats_ptr;
        xstats[count].id = count;
      }
    }
...
int cxgbe_dev_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
...
int cxgbe_dev_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
                         unsigned int n) {
...
static int cxgbe_flow_ctrl_set(struct rte_eth_dev *eth_dev,
  ...
  ret = t4_set_link_pause(pi, fc_conf->autoneg, tx_pause, rx_pause, &new_caps);
  if (ret != 0)
    return ret;
...
static int cxgbe_dev_rss_hash_conf_get(struct rte_eth_dev *dev,
  ...
  err = t4_read_config_vi_rss(adapter, adapter->mbox, pi->viid, &flags, NULL);
 ...
 */
static int eeprom_ptov(unsigned int phys_addr, unsigned int fn,
                       unsigned int sz) {
  fn *= sz;
  if (phys_addr < 1024)
 ...
 */
static int eeprom_rd_phys(struct adapter *adap, unsigned int phys_addr,
                          u32 *v) {
...
static int cxgbe_set_eeprom(struct rte_eth_dev *dev,
  ...
  if (aligned_offset != eeprom->offset || aligned_len != eeprom->length) {
    ...
    if (!err && aligned_len > 4)
      err = eeprom_rd_phys(adapter, aligned_offset + aligned_len - 4,
                           (u32 *)&buf[aligned_len - 4]);
    if (err)
      goto out;
    rte_memcpy(buf + (eeprom->offset & 3), eeprom->data, eeprom->length);
  } else {
    buf = eeprom->data;
  }
...
static int cxgbe_fec_get_capa_speed_to_fec(struct link_config *lc,
  ...
  if (lc->pcaps & FW_PORT_CAP32_SPEED_100G) {
    if (capa_arr) {
      ...
      capa_arr[num].capa =
          RTE_ETH_FEC_MODE_CAPA_MASK(NOFEC) | RTE_ETH_FEC_MODE_CAPA_MASK(RS);
    }
    num++;
  }
  ...
  if (lc->pcaps & FW_PORT_CAP32_SPEED_50G) {
    if (capa_arr) {
      ...
      capa_arr[num].capa =
          RTE_ETH_FEC_MODE_CAPA_MASK(NOFEC) | RTE_ETH_FEC_MODE_CAPA_MASK(BASER);
    }
    num++;
  }
...
static int eth_cxgbe_dev_init(struct rte_eth_dev *eth_dev) {
  ...
  if (rte_eal_process_type() != RTE_PROC_PRIMARY) {
    ...
    for (i = 1; i < MAX_NPORTS; i++) {
      ...
      snprintf(namei, sizeof(namei), "%s_%d", pci_dev->device.name, i);
      rest_eth_dev = rte_eth_dev_attach_secondary(namei);
      if (rest_eth_dev) {
        rest_eth_dev->device = &pci_dev->device;
        rest_eth_dev->dev_ops = eth_dev->dev_ops;
        rest_eth_dev->rx_pkt_burst = eth_dev->rx_pkt_burst;
        rest_eth_dev->tx_pkt_burst = eth_dev->tx_pkt_burst;
...
RTE_PMD_REGISTER_PARAM_STRING(net_cxgbe, CXGBE_DEVARG_CMN_KEEP_OVLAN
                              "=<0|1> " CXGBE_DEVARG_CMN_TX_MODE_LATENCY
                              "=<0|1> " CXGBE_DEVARG_PF_FILTER_MODE
                              "=<uint32> " CXGBE_DEVARG_PF_FILTER_MASK
                              "=<uint32> ");
