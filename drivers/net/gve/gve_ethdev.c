...
static void gve_write_version(uint8_t *driver_version_register) {
...
static int gve_alloc_queue_page_list(struct gve_priv *priv, uint32_t id,
                                     uint32_t pages) {
  ...
  if (priv->num_registered_pages + pages > priv->max_registered_pages) {
    PMD_DRV_LOG(ERR, "Pages %" PRIu64 " > max registered pages %" PRIu64,
                priv->num_registered_pages + pages, priv->max_registered_pages);
    return -EINVAL;
  }
  qpl = &priv->qpl[id];
  snprintf(z_name, sizeof(z_name), "gve_%s_qpl%d", priv->pci_dev->device.name,
           id);
  mz = rte_memzone_reserve_aligned(z_name, pages * PAGE_SIZE, rte_socket_id(),
  ...
  qpl->page_buses =
      rte_zmalloc("qpl page buses", pages * sizeof(dma_addr_t), 0);
...
static int gve_dev_configure(struct rte_eth_dev *dev) {
  ...
  if (priv->rss_config.indir) {
    ...
    if (err)
      PMD_DRV_LOG(ERR, "Could not reconfigure RSS redirection table.");
    else
...
static int gve_link_update(struct rte_eth_dev *dev,
...
static int gve_alloc_stats_report(struct gve_priv *priv, uint16_t nb_tx_queues,
  ...
  tx_stats_cnt =
      (GVE_TX_STATS_REPORT_NUM + NIC_TX_STATS_REPORT_NUM) * nb_tx_queues;
  rx_stats_cnt =
      (GVE_RX_STATS_REPORT_NUM + NIC_RX_STATS_REPORT_NUM) * nb_rx_queues;
  ...
  priv->stats_report_mem = rte_memzone_reserve_aligned(
      z_name, priv->stats_report_len, rte_socket_id(), RTE_MEMZONE_IOVA_CONTIG,
      PAGE_SIZE);
...
static void gve_get_imissed_from_nic(struct rte_eth_dev *dev) {
  ...
  if (!priv->stats_report_mem)
    return;
  stats_report = (struct gve_stats_report *)priv->stats_report_mem->addr;
...
static int gve_start_queues(struct rte_eth_dev *dev) {
  struct gve_priv *priv = dev->data->dev_private;
  uint16_t num_queues;
  uint16_t i;
  int ret;
...
static int gve_dev_start(struct rte_eth_dev *dev) {
  struct gve_priv *priv;
  int ret;
  ...
  if (gve_is_gqi(priv)) {
    ret = gve_alloc_stats_report(priv, dev->data->nb_tx_queues,
                                 dev->data->nb_rx_queues);
    if (ret != 0) {
      PMD_DRV_LOG(ERR, "Failed to allocate region for stats reporting.");
      return ret;
    }
...
static int gve_dev_stop(struct rte_eth_dev *dev) {
...
static int gve_dev_close(struct rte_eth_dev *dev) {
  struct gve_priv *priv = dev->data->dev_private;
  int err = 0;
  uint16_t i;
...
static int gve_verify_driver_compatibility(struct gve_priv *priv) {
  ...
  driver_info_mem = rte_memzone_reserve_aligned(
      "verify_driver_compatibility", sizeof(struct gve_driver_info),
      rte_socket_id(), RTE_MEMZONE_IOVA_CONTIG, PAGE_SIZE);
  ...
  *driver_info = (struct gve_driver_info){
  ...
  err = gve_adminq_verify_driver_compatibility(
      priv, sizeof(struct gve_driver_info), (dma_addr_t)driver_info_mem->iova);
...
static int gve_dev_info_get(struct rte_eth_dev *dev,
  ...
  dev_info->tx_offload_capa =
      RTE_ETH_TX_OFFLOAD_MULTI_SEGS | RTE_ETH_TX_OFFLOAD_UDP_CKSUM |
  ...
  if (!gve_is_gqi(priv)) {
    ...
    dev_info->rx_offload_capa |=
        ...
        RTE_ETH_RX_OFFLOAD_TCP_CKSUM | RTE_ETH_RX_OFFLOAD_TCP_LRO;
  }
...
static int gve_dev_stats_get(struct rte_eth_dev *dev,
...
static int gve_dev_stats_reset(struct rte_eth_dev *dev) {
  uint16_t i;
...
static int gve_dev_mtu_set(struct rte_eth_dev *dev, uint16_t mtu) {
  struct gve_priv *priv = dev->data->dev_private;
  int err;
...
static int gve_xstats_count(struct rte_eth_dev *dev) {
  uint16_t i, count = 0;
...
static int gve_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
                          unsigned int size) {
  ...
  for (i = 0; i < dev->data->nb_tx_queues; i++) {
    ...
    for (j = 0; j < RTE_DIM(tx_xstats_name_offset); j++, count++) {
      ...
      xstats[count].value =
          *(const uint64_t *)(stats + tx_xstats_name_offset[j].offset);
    }
  }
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    for (j = 0; j < RTE_DIM(rx_xstats_name_offset); j++, count++) {
      xstats[count].id = count;
      xstats[count].value =
          *(const uint64_t *)(stats + rx_xstats_name_offset[j].offset);
    }
  }
...
static int gve_xstats_get_names(struct rte_eth_dev *dev,
...
static int gve_rss_hash_update(struct rte_eth_dev *dev,
  ...
  rss_reta_size =
      priv->rss_config.indir ? priv->rss_config.indir_size : GVE_RSS_INDIR_SIZE;
  err =
      gve_init_rss_config(&gve_rss_conf, rss_conf->rss_key_len, rss_reta_size);
  if (err)
    return err;
...
static int gve_rss_hash_conf_get(struct rte_eth_dev *dev,
                                 struct rte_eth_rss_conf *rss_conf) {
  struct gve_priv *priv = dev->data->dev_private;
...
static int gve_rss_reta_update(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < priv->rss_config.indir_size; i++) {
    ...
    if (reta_conf[table_id].mask & (1ULL << table_entry))
      gve_rss_conf.indir[i] = reta_conf[table_id].reta[table_entry];
...
static int gve_rss_reta_query(struct rte_eth_dev *dev,
                              struct rte_eth_rss_reta_entry64 *reta_conf,
                              uint16_t reta_size) {
  struct gve_priv *priv = dev->data->dev_private;
  int table_id;
  ...
  for (i = 0; i < priv->rss_config.indir_size; i++) {
    int table_entry = i % RTE_ETH_RETA_GROUP_SIZE;
    if (reta_conf[table_id].mask & (1ULL << table_entry))
      reta_conf[table_id].reta[table_entry] = priv->rss_config.indir[i];
...
static void gve_free_irq_db(struct gve_priv *priv) {
...
static int pci_dev_msix_vec_count(struct rte_pci_device *pdev) {
  off_t msix_pos = rte_pci_find_capability(pdev, RTE_PCI_CAP_ID_MSIX);
  uint16_t control;

  if (msix_pos > 0 &&
      rte_pci_read_config(pdev, &control, sizeof(control),
...
static int gve_setup_device_resources(struct gve_priv *priv) {
  ...
  snprintf(z_name, sizeof(z_name), "gve_%s_cnt_arr",
           priv->pci_dev->device.name);
  mz = rte_memzone_reserve_aligned(
      z_name, priv->num_event_counters * sizeof(*priv->cnt_array),
      rte_socket_id(), RTE_MEMZONE_IOVA_CONTIG, PAGE_SIZE);
  if (mz == NULL) {
  ...
  mz = rte_memzone_reserve_aligned(
      z_name, sizeof(*priv->irq_dbs) * (priv->num_ntfy_blks), rte_socket_id(),
      RTE_MEMZONE_IOVA_CONTIG, PAGE_SIZE);
  if (mz == NULL) {
  ...
  err = gve_adminq_configure_device_resources(
      priv, priv->cnt_array_mz->iova, priv->num_event_counters,
      priv->irq_dbs_mz->iova, priv->num_ntfy_blks);
  if (unlikely(err)) {
  ...
  if (!gve_is_gqi(priv)) {
    priv->ptype_lut_dqo =
        rte_zmalloc("gve_ptype_lut_dqo", sizeof(struct gve_ptype_lut), 0);
...
static int gve_init_priv(struct gve_priv *priv, bool skip_describe_device) {
  ...
  PMD_DRV_LOG(INFO, "Max TX queues %d, Max RX queues %d", priv->max_nb_txq,
              priv->max_nb_rxq);
  ...
  if (priv->queue_format == GVE_GQI_QPL_FORMAT) {
    priv->qpl = rte_zmalloc("gve_qpl",
                            (priv->max_nb_txq + priv->max_nb_rxq) *
                                sizeof(struct gve_queue_page_list),
                            0);
    if (priv->qpl == NULL) {
...
static int gve_dev_init(struct rte_eth_dev *eth_dev) {
...
static int gve_dev_uninit(struct rte_eth_dev *eth_dev) {
  struct gve_priv *priv = eth_dev->data->dev_private;
...
static int gve_pci_probe(__rte_unused struct rte_pci_driver *pci_drv,
                         struct rte_pci_device *pci_dev) {
  return rte_eth_dev_pci_generic_probe(pci_dev, sizeof(struct gve_priv),
                                       gve_dev_init);
}

static int gve_pci_remove(struct rte_pci_device *pci_dev) {
