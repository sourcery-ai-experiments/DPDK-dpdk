...
static inline void gve_rx_refill_dqo(struct gve_rx_queue *rxq) {
...
gve_parse_csum_ol_flags(volatile struct gve_rx_compl_desc_dqo *rx_desc,
  ...
  uint64_t ol_flags = 0;
  struct gve_ptype ptype = priv->ptype_lut_dqo->ptypes[rx_desc->packet_type];
...
uint16_t gve_rx_burst_dqo(void *rx_queue, struct rte_mbuf **rx_pkts,
                          uint16_t nb_pkts) {
  ...
  while (nb_rx < nb_pkts) {
    ...
    rxm->ol_flags = 0;
    rxm->ol_flags |=
        RTE_MBUF_F_RX_RSS_HASH | gve_parse_csum_ol_flags(rx_desc, rxq->hw);
...
static inline void gve_release_rxq_mbufs_dqo(struct gve_rx_queue *rxq) {
  uint16_t i;
...
static void gve_reset_rxq_dqo(struct gve_rx_queue *rxq) {
...
int gve_rx_queue_setup_dqo(struct rte_eth_dev *dev, uint16_t queue_id,
  ...
  rxq = rte_zmalloc_socket("gve rxq", sizeof(struct gve_rx_queue),
  ...
  free_thresh =
      conf->rx_free_thresh ? conf->rx_free_thresh : GVE_DEFAULT_RX_FREE_THRESH;
  ...
  mbuf_len = rte_pktmbuf_data_room_size(rxq->mpool) - RTE_PKTMBUF_HEADROOM;
  rxq->rx_buf_len = RTE_MIN((uint16_t)GVE_RX_MAX_BUF_SIZE_DQO,
                            RTE_ALIGN_FLOOR(mbuf_len, GVE_RX_BUF_ALIGN_DQO));

  /* Allocate software ring */
  rxq->sw_ring =
      rte_zmalloc_socket("gve rx sw ring", nb_desc * sizeof(struct rte_mbuf *),
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->sw_ring == NULL) {
  ...
  mz = rte_eth_dma_zone_reserve(dev, "rxq_res", queue_id,
                                sizeof(struct gve_queue_resources), PAGE_SIZE,
                                socket_id);
  if (mz == NULL) {
...
static int gve_rxq_mbufs_alloc_dqo(struct gve_rx_queue *rxq) {
  struct rte_mbuf *nmb;
  uint16_t i;
  int diag;
