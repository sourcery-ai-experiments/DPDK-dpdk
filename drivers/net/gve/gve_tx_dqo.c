...
static inline void gve_tx_clean_dqo(struct gve_tx_queue *txq) {
  ...
  case GVE_COMPL_TYPE_DQO_DESC:
    ...
    if (aim_txq->last_desc_cleaned > compl_tag)
      nb_desc_clean =
          aim_txq->nb_tx_desc - aim_txq->last_desc_cleaned + compl_tag;
    else
...
uint16_t gve_tx_burst_dqo(void *tx_queue, struct rte_mbuf **tx_pkts,
                          uint16_t nb_pkts) {
...
static inline void gve_release_txq_mbufs_dqo(struct gve_tx_queue *txq) {
  uint16_t i;
...
static int check_tx_thresh_dqo(uint16_t nb_desc, uint16_t tx_rs_thresh,
...
static void gve_reset_txq_dqo(struct gve_tx_queue *txq) {
  struct rte_mbuf **sw_ring;
  uint32_t size, i;
...
int gve_tx_queue_setup_dqo(struct rte_eth_dev *dev, uint16_t queue_id,
  ...
  free_thresh =
      conf->tx_free_thresh ? conf->tx_free_thresh : GVE_DEFAULT_TX_FREE_THRESH;
  rs_thresh =
      conf->tx_rs_thresh ? conf->tx_rs_thresh : GVE_DEFAULT_TX_RS_THRESH;
  ...
  txq->sw_ring =
      rte_zmalloc_socket("gve tx sw ring", sw_size * sizeof(struct rte_mbuf *),
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (txq->sw_ring == NULL) {
...
int gve_tx_queue_start_dqo(struct rte_eth_dev *dev, uint16_t tx_queue_id) {
  ...
  txq->qtx_head = &hw->cnt_array[rte_be_to_cpu_32(txq->qres->counter_index)];
