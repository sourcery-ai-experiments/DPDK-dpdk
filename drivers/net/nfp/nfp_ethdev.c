...
static int nfp_devarg_handle_int(const char *key, const char *value,
                                 void *extra_args) {
  char *end_ptr;
...
static void nfp_devarg_parse_force_reload_fw(struct rte_kvargs *kvlist,
  ...
  ret = rte_kvargs_process(kvlist, NFP_PF_FORCE_RELOAD_FW,
                           &nfp_devarg_handle_int, &value);
  if (ret != 0)
    return;
...
static void nfp_devargs_parse(struct nfp_devargs *nfp_devargs_param,
...
static void nfp_net_pf_read_mac(struct nfp_app_fw_nic *app_fw_nic,
                                uint16_t port) {
...
static uint32_t nfp_net_speed_bitmap2speed(uint32_t speeds_bitmap) {
...
static int nfp_net_speed_configure(struct rte_eth_dev *dev,
  ...
  /* NFP4000 does not allow the port 0 25Gbps and port 1 10Gbps at the same
   * time. */
  if (net_hw->device_id == PCI_DEVICE_ID_NFP4000_PF_NIC) {
    ret = nfp_net_nfp4000_speed_configure_check(net_hw->idx, configure_speed,
                                                nfp_eth_table);
    if (ret != 0) {
...
static int nfp_net_start(struct rte_eth_dev *dev) {
  ...
  if (dev->data->dev_conf.intr_conf.rxq != 0) {
    ...
    if (rte_intr_type_get(intr_handle) == RTE_INTR_HANDLE_UIO) {
       ...
       */
      rte_intr_callback_unregister(intr_handle, nfp_net_dev_interrupt_handler,
                                   (void *)dev);
  ...
  if (dev->data->mtu > net_hw->flbufsz) {
    PMD_INIT_LOG(
        ERR, "MTU (%u) can't be larger than the current NFP_FRAME_SIZE (%u)",
  ...
  if ((cap_extend & NFP_NET_CFG_CTRL_IPSEC) != 0)
    ctrl_extend |= NFP_NET_CFG_CTRL_IPSEC | NFP_NET_CFG_CTRL_IPSEC_SM_LOOKUP |
...
static uint8_t nfp_function_id_get(const struct nfp_pf_dev *pf_dev,
                                   uint8_t phy_port) {
...
static void nfp_net_beat_timer(void *arg) {
  uint64_t cur_sec;
  struct nfp_multi_pf *multi_pf = arg;

  cur_sec = rte_rdtsc();
  nn_writeq(cur_sec,
            multi_pf->beat_addr + NFP_BEAT_OFFSET(multi_pf->function_id));

  /* Beat once per second. */
  if (rte_eal_alarm_set(1000 * 1000, nfp_net_beat_timer, (void *)multi_pf) <
      0) {
    PMD_DRV_LOG(ERR, "Error setting alarm");
  }
}

static int nfp_net_keepalive_init(struct nfp_cpp *cpp,
...
static void nfp_net_keepalive_uninit(struct nfp_multi_pf *multi_pf) {
  nfp_cpp_area_release_free(multi_pf->beat_area);
}

static int nfp_net_keepalive_start(struct nfp_multi_pf *multi_pf) {
  if (rte_eal_alarm_set(1000 * 1000, nfp_net_beat_timer, (void *)multi_pf) <
      0) {
    PMD_DRV_LOG(ERR, "Error setting alarm");
    return -EIO;
...
static void nfp_net_keepalive_clear(uint8_t *beat_addr, uint8_t function_id) {
  nn_writeq(0, beat_addr + NFP_BEAT_OFFSET(function_id));
}

static void nfp_net_keepalive_clear_others(const struct nfp_dev_info *dev_info,
                                           struct nfp_multi_pf *multi_pf) {
  uint8_t port_num;
...
static void nfp_net_keepalive_stop(struct nfp_multi_pf *multi_pf) {
  /* Cancel keepalive for multiple PF setup */
  rte_eal_alarm_cancel(nfp_net_beat_timer, (void *)multi_pf);
}

static void nfp_net_uninit(struct rte_eth_dev *eth_dev) {
  struct nfp_net_hw *net_hw;
...
static void nfp_cleanup_port_app_fw_nic(struct nfp_pf_dev *pf_dev, uint8_t id) {
...
static void nfp_uninit_app_fw_nic(struct nfp_pf_dev *pf_dev) {
...
static int nfp_pf_secondary_uninit(struct nfp_pf_dev *pf_dev) {
  free(pf_dev->sym_tbl);
  nfp_cpp_free(pf_dev->cpp);
  nfp_sync_free(pf_dev->sync);
  rte_free(pf_dev);
...
static int nfp_net_find_vxlan_idx(struct nfp_net_hw *hw, uint16_t port,
                                  uint32_t *idx) {
  uint32_t i;
  int free_idx = -1;
...
static int nfp_udp_tunnel_port_add(struct rte_eth_dev *dev,
...
static int nfp_udp_tunnel_port_del(struct rte_eth_dev *dev,
                                   struct rte_eth_udp_tunnel *tunnel_udp) {
  int ret;
  uint32_t idx;
  uint16_t vxlan_port;
...
static inline void nfp_net_ethdev_ops_mount(struct nfp_net_hw *hw,
                                            struct rte_eth_dev *eth_dev) {
...
static int nfp_net_init(struct rte_eth_dev *eth_dev) {
  ...
  PMD_INIT_LOG(DEBUG,
               "Working with physical port number: %hu, "
  ...
  PMD_INIT_LOG(DEBUG, "ctrl_bar: %p, tx_bar: %p, rx_bar: %p", hw->ctrl_bar,
               net_hw->tx_bar, net_hw->rx_bar);
  ...
  PMD_INIT_LOG(INFO,
               "port %d VendorID=%#x DeviceID=%#x "
               "mac=" RTE_ETHER_ADDR_PRT_FMT,
               eth_dev->data->port_id, pci_dev->id.vendor_id,
               pci_dev->id.device_id, RTE_ETHER_ADDR_BYTES(&hw->mac_addr));
...
static int nfp_fw_get_name(struct rte_pci_device *dev, struct nfp_nsp *nsp,
                           char *card, char *fw_name, size_t fw_size) {
  char serial[40];
  uint16_t interface;
  ...
  snprintf(serial, sizeof(serial),
           "serial-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x", cpp_serial[0],
           cpp_serial[1], cpp_serial[2], cpp_serial[3], cpp_serial[4],
           cpp_serial[5], interface >> 8, interface & 0xff);
  ...
  snprintf(fw_name, fw_size, "%s/pci-%s.nffw", DEFAULT_FW_PATH, dev->name);
...
static int nfp_fw_upload(struct nfp_nsp *nsp, char *fw_name) {
  int err;
  void *fw_buf;
  size_t fsize;
  ...
  PMD_DRV_LOG(INFO, "Firmware file found at %s with size: %zu", fw_name, fsize);
...
static void nfp_fw_unload(struct nfp_cpp *cpp) {
  struct nfp_nsp *nsp;
...
static int nfp_fw_check_change(struct nfp_cpp *cpp, char *fw_name,
                               bool *fw_changed) {
  int ret;
  struct nfp_net_hw hw;
  ...
  if (new_version != old_version) {
    PMD_DRV_LOG(INFO, "FW version is changed, new %u, old %u", new_version,
                old_version);
    *fw_changed = true;
  } else {
...
static int nfp_fw_reload(struct nfp_nsp *nsp, char *fw_name) {
  int err;
...
static bool nfp_fw_skip_load(const struct nfp_dev_info *dev_info,
                             struct nfp_multi_pf *multi_pf, bool *reload_fw) {
  uint8_t i;
  uint64_t tmp_beat;
  uint32_t port_num;
  uint8_t in_use = 0;
  ...
  if (in_use != 0) {
    PMD_DRV_LOG(WARNING,
                "Abnormal %u != 0, the nic has port which is exit abnormally.",
                abnormal);
    return true;
  }
...
static int nfp_fw_reload_for_single_pf(struct nfp_nsp *nsp, char *fw_name,
                                       struct nfp_cpp *cpp,
                                       bool force_reload_fw) {
  int ret;
...
static int nfp_fw_reload_for_multi_pf(struct nfp_nsp *nsp, char *fw_name,
                                      struct nfp_cpp *cpp,
...
static int nfp_fw_setup(struct rte_pci_device *dev, struct nfp_cpp *cpp,
                        struct nfp_eth_table *nfp_eth_table,
                        struct nfp_hwinfo *hwinfo,
                        const struct nfp_dev_info *dev_info,
                        struct nfp_multi_pf *multi_pf, bool force_reload_fw) {
  int err;
  char fw_name[125];
  char card_desc[100];
  struct nfp_nsp *nsp;
...
static inline bool nfp_check_multi_pf_from_fw(uint32_t total_vnics) {
  if (total_vnics == 1)
    return true;
...
static inline bool nfp_check_multi_pf_from_nsp(struct rte_pci_device *pci_dev,
                                               struct nfp_cpp *cpp) {
  bool flag;
  struct nfp_nsp *nsp;
...
static int nfp_enable_multi_pf(struct nfp_pf_dev *pf_dev) {
  int err = 0;
  uint64_t tx_base;
  uint8_t *ctrl_bar;
  struct nfp_hw *hw;
  ...
  snprintf(name, sizeof(name), "_pf%u_net_bar0", pf_dev->multi_pf.function_id);
  ctrl_bar = nfp_rtsym_map(pf_dev->sym_tbl, name, NFP_NET_CFG_BAR_SZ, &area);
  if (ctrl_bar == NULL) {
...
static int nfp_init_app_fw_nic(struct nfp_pf_dev *pf_dev,
                               const struct nfp_dev_info *dev_info) {
  uint8_t i;
  uint8_t id;
  int ret = 0;
  ...
  pf_dev->ctrl_bar = nfp_rtsym_map(
      pf_dev->sym_tbl, bar_name,
      app_fw_nic->total_phyports * NFP_NET_CFG_BAR_SZ, &pf_dev->ctrl_area);
  ...
  for (i = 0; i < app_fw_nic->total_phyports; i++) {
    ...
    eth_dev->data->dev_private = rte_zmalloc_socket(
        port_name, sizeof(struct nfp_net_hw), RTE_CACHE_LINE_SIZE, numa_node);
...
static int nfp_net_hwinfo_set(uint8_t function_id,
  ...
  if (app_fw_id != NFP_APP_FW_FLOWER_NIC) {
    ...
    snprintf(app_cap_name, sizeof(app_cap_name), "_pf%u_net_app_cap",
             function_id);
...
static int nfp_net_speed_capa_get_real(struct nfp_eth_media_buf *media_buf,
                                       struct nfp_pf_dev *pf_dev) {
  uint32_t i;
  uint32_t j;
  uint32_t offset;
...
static int nfp_net_speed_capa_get(struct nfp_pf_dev *pf_dev, uint32_t port_id) {
  int ret;
  struct nfp_nsp *nsp;
...
static int nfp_pf_init(struct rte_pci_device *pci_dev) {
  ...
  if (nfp_fw_setup(pci_dev, cpp, nfp_eth_table, hwinfo, dev_info,
                   &pf_dev->multi_pf, pf_dev->devargs.force_reload_fw) != 0) {
  ...
  pf_dev->qc_bar = nfp_cpp_map_area(pf_dev->cpp, cpp_id, addr,
                                    dev_info->qc_area_sz, &pf_dev->qc_area);
...
static int nfp_secondary_init_app_fw_nic(struct nfp_pf_dev *pf_dev) {
  uint32_t i;
  int err = 0;
  int ret = 0;
  uint8_t function_id;
...
static const struct rte_pci_id pci_id_nfp_pf_net_map[] = {
    ...
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_NETRONOME, PCI_DEVICE_ID_NFP6000_PF_NIC)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CORIGINE, PCI_DEVICE_ID_NFP3800_PF_NIC)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CORIGINE, PCI_DEVICE_ID_NFP4000_PF_NIC)},
    {RTE_PCI_DEVICE(PCI_VENDOR_ID_CORIGINE, PCI_DEVICE_ID_NFP6000_PF_NIC)},
    {
        .vendor_id = 0,
    },
};
...
RTE_PMD_REGISTER_KMOD_DEP(NFP_PF_DRIVER_NAME,
