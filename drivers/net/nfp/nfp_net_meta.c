...
enum nfp_net_meta_ipsec_layer {
  ...
  NFP_NET_META_IPSEC_SEQLOW, /**< Order of Sequence Number (low 32bits) in
                                metadata */
  NFP_NET_META_IPSEC_SEQHI,  /**< Order of Sequence Number (high 32bits) in
                                metadata */
};

/* Parse the chained metadata from packet */
static bool nfp_net_meta_parse_chained(uint8_t *meta_base,
  ...
  for (; meta_info != 0;
       meta_info >>= NFP_NET_META_FIELD_SIZE, meta_offset += 4) {
    ...
    case NFP_NET_META_VLAN:
      ...
      meta->vlan[meta->vlan_layer].tci = vlan_info & NFP_NET_META_VLAN_MASK;
 ...
 */
static inline void nfp_net_meta_parse_single(uint8_t *meta_base,
                                             rte_be32_t meta_header,
                                             struct nfp_net_meta_parsed *meta) {
  meta->flags |= (1 << NFP_NET_META_HASH);
...
static void nfp_net_meta_parse_hash(const struct nfp_net_meta_parsed *meta,
...
static void nfp_net_meta_parse_vlan(const struct nfp_net_meta_parsed *meta,
 ...
 *   mbuf->vlan_tci (inner) and mbuf->vlan_tci_outer (outer).
 *
 * qinq set & vlan set : meta->vlan_layer>=2, meta->vlan[0].offload=1,
 * meta->vlan[1].offload=1 qinq set & vlan not set: meta->vlan_layer>=2,
 * meta->vlan[1].offload=1,meta->vlan[0].offload=0 qinq not set & vlan set:
 * meta->vlan_layer=1, meta->vlan[0].offload=1 qinq not set & vlan not set:
 * meta->vlan_layer=0
 */
static void nfp_net_meta_parse_qinq(const struct nfp_net_meta_parsed *meta,
                                    struct nfp_net_rxq *rxq,
                                    struct rte_mbuf *mb) {
 ...
 */
static void nfp_net_meta_parse_ipsec(struct nfp_net_meta_parsed *meta,
                                     struct nfp_net_rxq *rxq,
                                     struct rte_mbuf *mbuf) {
  int offset;
  uint32_t sa_idx;
...
static void nfp_net_meta_parse_mark(const struct nfp_net_meta_parsed *meta,
                                    struct rte_mbuf *mbuf) {
...
void nfp_net_meta_parse(struct nfp_net_rx_desc *rxds, struct nfp_net_rxq *rxq,
                        struct nfp_net_hw *hw, struct rte_mbuf *mb,
                        struct nfp_net_meta_parsed *meta) {
  uint8_t *meta_base;
  rte_be32_t meta_header;
...
void nfp_net_meta_init_format(struct nfp_net_hw *hw) {
  /*
   * ABI 4.x and ctrl vNIC always use chained metadata, in other cases we allow
   * use of single metadata if only RSS(v1) is supported by hw capability, and
   * RSS(v2) also indicate that we are using chained metadata.
   */
  if (hw->ver.major == 4) {
     ...
     * to reduce the confusion to allow us can use hw->super.cap to identify RSS
     * later.
     */
...
void nfp_net_meta_set_vlan(struct nfp_net_meta_raw *meta_data,
                           struct rte_mbuf *pkt, uint8_t layer) {
  uint16_t tpid;
  uint16_t vlan_tci;
...
void nfp_net_meta_set_ipsec(struct nfp_net_meta_raw *meta_data,
                            struct nfp_net_txq *txq, struct rte_mbuf *pkt,
                            uint8_t layer, uint8_t ipsec_layer) {
  int offset;
  struct nfp_net_hw *hw;
  struct nfp_tx_ipsec_desc_msg *desc_md;
