...
static uint64_t nfp_net_nfdk_tx_cksum(struct nfp_net_txq *txq,
                                      struct rte_mbuf *mb, uint64_t flags) {
  uint64_t ol_flags;
...
static uint64_t nfp_net_nfdk_tx_tso(struct nfp_net_txq *txq,
                                    struct rte_mbuf *mb) {
  uint8_t outer_len;
  uint64_t ol_flags;
...
uint32_t nfp_flower_nfdk_pkt_add_metadata(struct rte_mbuf *mbuf,
  ...
  header =
      NFP_NET_META_PORTID << NFP_NET_META_NFDK_LENGTH | FLOWER_PKT_DATA_OFFSET;
...
static inline uint16_t nfp_net_nfdk_headlen_to_segs(uint16_t headlen) {
...
static inline void nfp_net_nfdk_tx_close_block(struct nfp_net_txq *txq,
                                               uint32_t nop_slots) {
  uint32_t i;
  uint32_t wr_p;
...
static int nfp_net_nfdk_set_meta_data(struct rte_mbuf *pkt,
...
uint16_t nfp_net_nfdk_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts,
                                uint16_t nb_pkts) {
...
uint16_t nfp_net_nfdk_xmit_pkts_common(void *tx_queue,
                                       ...
                                       uint16_t nb_pkts, bool repr_flag) {
  uint32_t buf_idx;
  uint64_t dma_addr;
  uint32_t free_descs;
  uint32_t npkts = 0;
  ...
  while (npkts < nb_pkts && free_descs > 0) {
    ...
    ktxds->raw =
        rte_cpu_to_le_64(nfp_net_nfdk_tx_cksum(txq, temp_pkt, metadata));
    ktxds++;
...
int nfp_net_nfdk_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (dev->data->tx_queues[queue_idx] != NULL) {
    PMD_TX_LOG(DEBUG, "Freeing memory prior to re-allocation %d", queue_idx);
  ...
  txq->txbufs =
      rte_zmalloc_socket("txq->txbufs", sizeof(*txq->txbufs) * txq->tx_count,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (txq->txbufs == NULL) {
    nfp_net_tx_queue_release(dev, queue_idx);
    return -ENOMEM;
  ...
  if (hw->txrwb_mz != NULL) {
    ...
    txq->txrwb_dma =
        (uint64_t)hw->txrwb_mz->iova + queue_idx * sizeof(uint64_t);
  ...
  nn_cfg_writeb(&hw->super, NFP_NET_CFG_TXR_SZ(queue_idx),
