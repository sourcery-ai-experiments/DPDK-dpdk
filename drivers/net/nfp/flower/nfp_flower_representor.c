...
static int nfp_flower_repr_link_update(struct rte_eth_dev *dev,
...
static int nfp_flower_repr_dev_infos_get(__rte_unused struct rte_eth_dev *dev,
...
static int nfp_flower_repr_dev_start(struct rte_eth_dev *dev) {
  struct nfp_flower_representor *repr;
  ...
  if (repr->repr_type == NFP_REPR_TYPE_PHYS_PORT) {
    nfp_eth_set_configured(app_fw_flower->pf_hw->pf_dev->cpp, repr->nfp_idx, 1);
  }
...
static int nfp_flower_repr_dev_stop(struct rte_eth_dev *dev) {
  struct nfp_flower_representor *repr;
  struct nfp_app_fw_flower *app_fw_flower;
  uint16_t i;
  ...
  if (repr->repr_type == NFP_REPR_TYPE_PHYS_PORT) {
    nfp_eth_set_configured(app_fw_flower->pf_hw->pf_dev->cpp, repr->nfp_idx, 0);
  }
...
static int nfp_flower_repr_rx_queue_setup(
    struct rte_eth_dev *dev, uint16_t rx_queue_id,
    __rte_unused uint16_t nb_rx_desc, unsigned int socket_id,
...
static int nfp_flower_repr_tx_queue_setup(
    struct rte_eth_dev *dev, uint16_t tx_queue_id,
    __rte_unused uint16_t nb_tx_desc, unsigned int socket_id,
...
static int nfp_flower_repr_stats_get(struct rte_eth_dev *ethdev,
...
static int nfp_flower_repr_stats_reset(struct rte_eth_dev *ethdev) {
  struct nfp_flower_representor *repr;
...
static int nfp_flower_repr_mac_addr_set(struct rte_eth_dev *ethdev,
...
static uint16_t nfp_flower_repr_rx_burst(void *rx_queue,
  ...
  if (unlikely(repr->ring == NULL)) {
    PMD_RX_LOG(ERR, "representor %s has no ring configured!", repr->name);
    return 0;
  }

  total_dequeue =
      rte_ring_dequeue_burst(repr->ring, (void *)rx_pkts, nb_pkts, &available);
  if (total_dequeue != 0) {
    PMD_RX_LOG(DEBUG,
               "Representor Rx burst for %s, port_id: %#x, "
               "received: %u, available: %u",
               repr->name, repr->port_id, total_dequeue, available);
...
static uint16_t nfp_flower_repr_tx_burst(void *tx_queue,
  ...
  for (i = 0; i < nb_pkts; i++)
    nfp_flower_pkt_add_metadata(repr->app_fw_flower, tx_pkts[i], repr->port_id);
  ...
  if (sent != 0) {
    PMD_TX_LOG(DEBUG,
               "Representor Tx burst for %s, port_id: %#x transmitted: %hu",
...
static void nfp_flower_repr_free_queue(struct nfp_flower_representor *repr) {
  uint16_t i;
...
static void nfp_flower_repr_close_queue(struct nfp_flower_representor *repr) {
...
static int nfp_flower_pf_repr_uninit(struct rte_eth_dev *eth_dev) {
...
static void nfp_flower_repr_free(struct nfp_flower_representor *repr,
...
static int nfp_flower_repr_dev_close(struct rte_eth_dev *dev) {
  uint16_t i;
...
static uint32_t nfp_flower_get_phys_port_id(uint8_t port) {
  return (NFP_FLOWER_CMSG_PORT_TYPE_PHYS_PORT << 28) | port;
}

static uint32_t nfp_get_pcie_port_id(struct nfp_cpp *cpp, int type,
                                     uint8_t vnic, uint8_t queue) {
  uint8_t nfp_pcie;
  uint32_t port_id;

  nfp_pcie = NFP_CPP_INTERFACE_UNIT_of(nfp_cpp_interface(cpp));
  port_id = ((nfp_pcie & 0x3) << 14) | ((type & 0x3) << 12) |
            ((vnic & 0x3f) << 6) | (queue & 0x3f) |
            ((NFP_FLOWER_CMSG_PORT_TYPE_PCIE_PORT & 0xf) << 28);

  return port_id;
}

static int nfp_flower_pf_repr_init(struct rte_eth_dev *eth_dev,
                                   void *init_params) {
  struct nfp_flower_representor *repr;
...
static int nfp_flower_repr_init(struct rte_eth_dev *eth_dev,
                                void *init_params) {
  int ret;
  uint16_t index;
  ...
  if (repr->repr_type == NFP_REPR_TYPE_PHYS_PORT)
    ...
    eth_dev->data->representor_id =
        repr->vf_id + app_fw_flower->num_phyport_reprs + 1;
...
static void nfp_flower_repr_free_all(struct nfp_app_fw_flower *app_fw_flower) {
  uint32_t i;
  struct nfp_flower_representor *repr;
...
static int nfp_flower_repr_alloc(struct nfp_app_fw_flower *app_fw_flower) {
  int i;
  int ret;
  const char *pci_name;
  ...
  snprintf(flower_repr.name, sizeof(flower_repr.name), "%s_repr_pf", pci_name);

  /* Create a eth_dev for this representor */
  ret = rte_eth_dev_create(eth_dev->device, flower_repr.name,
                           sizeof(struct nfp_flower_representor), NULL, NULL,
                           nfp_flower_pf_repr_init, &flower_repr);
  if (ret != 0) {
  ...
  for (i = 0; i < app_fw_flower->num_phyport_reprs; i++) {
    ...
    snprintf(flower_repr.name, sizeof(flower_repr.name), "%s_repr_p%d",
             pci_name, i);
    ...
    ret = rte_eth_dev_create(eth_dev->device, flower_repr.name,
                             sizeof(struct nfp_flower_representor), NULL, NULL,
                             nfp_flower_repr_init, &flower_repr);
    if (ret != 0) {
  ...
  for (i = 0; i < app_fw_flower->num_vf_reprs; i++) {
    ...
    flower_repr.port_id = nfp_get_pcie_port_id(
        app_fw_flower->pf_hw->cpp, NFP_FLOWER_CMSG_PORT_VNIC_TYPE_VF, i, 0);
    flower_repr.nfp_idx = 0;
    flower_repr.vf_id = i;

    /* VF reprs get a random MAC address */
    rte_eth_random_addr(flower_repr.mac_addr.addr_bytes);
    snprintf(flower_repr.name, sizeof(flower_repr.name), "%s_repr_vf%d",
             pci_name, i);
...
int nfp_flower_repr_create(struct nfp_app_fw_flower *app_fw_flower) {
  ...
  app_fw_flower->num_vf_reprs =
      eth_da.nb_representor_ports - nfp_eth_table->count - 1;
