...
#define NFP_PTYPE2RTE(tunnel, type)                                            \
  ((tunnel) ? RTE_PTYPE_INNER_##type : RTE_PTYPE_##type)
...
static int nfp_net_rx_fill_freelist(struct nfp_net_rxq *rxq) {
  ...
  PMD_RX_LOG(DEBUG, "Fill Rx Freelist for %hu descriptors", rxq->rx_count);
  ...
  for (i = 0; i < rxq->rx_count; i++) {
    ...
    if (mbuf == NULL) {
      PMD_DRV_LOG(ERR, "RX mbuf alloc failed queue_id=%hu", rxq->qidx);
      return -ENOMEM;
    }
 ...
 */
static void nfp_net_set_ptype(const struct nfp_ptype_parsed *nfp_ptype,
                              struct rte_mbuf *mb) {
 ...
 *   Mbuf to set the packet type.
 */
static void nfp_net_parse_ptype(struct nfp_net_rxq *rxq,
                                struct nfp_net_rx_desc *rxds,
                                struct rte_mbuf *mb) {
  struct nfp_net_hw *hw = rxq->hw;
  ...
  nfp_ptype.l4_ptype =
      (rxd_ptype & NFP_NET_PTYPE_L4_MASK) >> NFP_NET_PTYPE_L4_OFFSET;
  nfp_ptype.l3_ptype =
      (rxd_ptype & NFP_NET_PTYPE_L3_MASK) >> NFP_NET_PTYPE_L3_OFFSET;
  nfp_ptype.tunnel_ptype =
      (rxd_ptype & NFP_NET_PTYPE_TUNNEL_MASK) >> NFP_NET_PTYPE_TUNNEL_OFFSET;
...
uint16_t nfp_net_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts,
  ...
  while (avail + avail_multiplexed < nb_pkts) {
    ...
    PMD_RX_LOG(DEBUG, "Packet len: %u, mbuf_size: %u", rxds->rxd.data_len,
               rxq->mbuf_size);
  ...
  if (nb_hold > rxq->rx_free_thresh) {
    PMD_RX_LOG(DEBUG, "port=%hu queue=%hu nb_hold=%hu avail=%hu", rxq->port_id,
               rxq->qidx, nb_hold, avail);
...
int nfp_net_rx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (rx_desc_sz % NFP_ALIGN_RING_DESC != 0 || nb_desc > max_rx_desc ||
      nb_desc < min_rx_desc) {
  ...
  rxq->rxbufs =
      rte_zmalloc_socket("rxq->rxbufs", sizeof(*rxq->rxbufs) * nb_desc,
                         RTE_CACHE_LINE_SIZE, socket_id);
  if (rxq->rxbufs == NULL) {
    nfp_net_rx_queue_release(dev, queue_idx);
    dev->data->rx_queues[queue_idx] = NULL;
  ...
  nn_cfg_writeb(&hw->super, NFP_NET_CFG_RXR_SZ(queue_idx),
                rte_log2_u32(nb_desc));

  return 0;
}

static inline uint32_t nfp_net_read_tx_free_qcp(struct nfp_net_txq *txq) {
  /*
...
uint32_t nfp_net_tx_free_bufs(struct nfp_net_txq *txq) {
  ...
  PMD_TX_LOG(DEBUG,
             "queue %hu. Check for descriptor with a complete"
             " status",
             txq->qidx);
  ...
  if (qcp_rd_p == txq->rd_p) {
    PMD_TX_LOG(DEBUG,
               "queue %hu: It seems harrier is not sending "
               "packets (%u, %u)",
               txq->qidx, qcp_rd_p, txq->rd_p);
    return 0;
  }
  ...
  PMD_TX_LOG(DEBUG, "qcp_rd_p %u, txq->rd_p: %u, qcp->rd_p: %u", qcp_rd_p,
             txq->rd_p, txq->rd_p);
...
int nfp_net_tx_queue_setup(struct rte_eth_dev *dev, uint16_t queue_idx,
  ...
  if (hw->ver.extend == NFP_NET_CFG_VERSION_DP_NFD3)
    return nfp_net_nfd3_tx_queue_setup(dev, queue_idx, nb_desc, socket_id,
                                       tx_conf);
  else
    return nfp_net_nfdk_tx_queue_setup(dev, queue_idx, nb_desc, socket_id,
                                       tx_conf);
}
