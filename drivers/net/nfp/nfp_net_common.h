...
/* RSS capability*/
#define NFP_NET_RSS_CAP                                                        \
  (RTE_ETH_RSS_IPV4 | RTE_ETH_RSS_IPV6 | RTE_ETH_RSS_NONFRAG_IPV4_TCP |        \
...
static inline uint32_t nfp_qcp_queue_offset(const struct nfp_dev_info *dev_info,
                                            uint16_t queue) {
  return dev_info->qc_addr_offset +
         NFP_QCP_QUEUE_ADDR_SZ * (queue & dev_info->qc_idx_mask);
}
...
int nfp_net_link_update_common(struct rte_eth_dev *dev, struct nfp_net_hw *hw,
                               struct rte_eth_link *link, uint32_t link_status);
...
void nfp_net_get_fw_version(struct nfp_net_hw *hw, uint32_t *fw_version);
