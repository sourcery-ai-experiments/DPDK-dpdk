...
enum nfp_xstat_group { NFP_XSTAT_GROUP_NET, NFP_XSTAT_GROUP_MAC };
...
#define NFP_XSTAT_NET(_name, _offset)                                          \
  {                                                                            \
    .name = _name, .offset = NFP_NET_CFG_STATS_##_offset,                      \
    .group = NFP_XSTAT_GROUP_NET,                                              \
  }

#define NFP_XSTAT_MAC(_name, _offset)                                          \
  {                                                                            \
    .name = _name, .offset = NFP_MAC_STATS_##_offset,                          \
...
static const struct nfp_xstat nfp_net_xstats[] = {
    ...
    /*
     * MAC xstats available only on PF. These statistics are not available for
     * VFs as the PF is not initialized when the VF is initialized as it is
     * still bound to the kernel driver. As such, the PMD cannot obtain a CPP
     * handle and access the rtsym_table in order to get the pointer to the
     * start of the MAC statistics counters.
     */
    ...
    NFP_XSTAT_MAC("mac.rx_frame_check_sequence_errors",
                  RX_FRAME_CHECK_SEQ_ERRORS),
...
static uint16_t nfp_net_link_speed_rte2nfp(uint16_t speed) {
  uint16_t i;
...
static void nfp_net_notify_port_speed(struct nfp_net_hw *hw,
 ...
 */
int nfp_net_mbox_reconfig(struct nfp_net_hw *net_hw, uint32_t mbox_cmd) {
  int ret;
  uint32_t mbox;
 ...
 *
 * This function must be invoked first before any other function in the Ethernet
 * API. This function can also be re-invoked when a device is in the stopped
 * state.
 *
...
int nfp_net_configure(struct rte_eth_dev *dev) {
  ...
  if (rxmode->mtu > hw->max_mtu + NFP_ETH_OVERHEAD) {
    PMD_DRV_LOG(ERR,
                "MTU (%u) larger than the maximum possible frame size (%u)",
...
void nfp_net_log_device_information(const struct nfp_net_hw *hw) {
  ...
  PMD_INIT_LOG(INFO, "VER: %u.%u, Maximum supported MTU: %d", hw->ver.major,
               hw->ver.minor, hw->max_mtu);
  ...
  PMD_INIT_LOG(INFO, "max_rx_queues: %u, max_tx_queues: %u", hw->max_rx_queues,
               hw->max_tx_queues);
}

static inline void nfp_net_enable_rxvlan_cap(struct nfp_net_hw *hw,
                                             uint32_t *ctrl) {
...
int nfp_net_set_mac_addr(struct rte_eth_dev *dev,
...
static int nfp_net_set_allmulticast_mode(struct rte_eth_dev *dev, bool enable) {
  int ret;
  uint32_t update;
  struct nfp_hw *hw;
  uint32_t cap_extend;
...
static int nfp_net_speed_aneg_update(struct rte_eth_dev *dev,
                                     struct nfp_net_hw *hw,
                                     struct rte_eth_link *link) {
  uint32_t i;
  uint32_t speed;
...
int nfp_net_link_update_common(struct rte_eth_dev *dev, struct nfp_net_hw *hw,
                               struct rte_eth_link *link,
...
int nfp_net_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats) {
  ...
  for (i = 0; i < dev->data->nb_rx_queues; i++) {
    ...
    nfp_dev_stats.q_ipackets[i] -= hw->eth_stats_base.q_ipackets[i];

    nfp_dev_stats.q_ibytes[i] =
        nn_cfg_readq(&hw->super, NFP_NET_CFG_RXR_STATS(i) + 0x8);
    nfp_dev_stats.q_ibytes[i] -= hw->eth_stats_base.q_ibytes[i];
  }
  ...
  nfp_dev_stats.obytes = nn_cfg_readq(&hw->super, NFP_NET_CFG_STATS_TX_OCTETS);
  nfp_dev_stats.obytes -= hw->eth_stats_base.obytes;

  /* Reading general device stats */
  nfp_dev_stats.ierrors = nn_cfg_readq(&hw->super, NFP_NET_CFG_STATS_RX_ERRORS);
  nfp_dev_stats.ierrors -= hw->eth_stats_base.ierrors;

  nfp_dev_stats.oerrors = nn_cfg_readq(&hw->super, NFP_NET_CFG_STATS_TX_ERRORS);
...
nfp_net_xstats_info(const struct rte_eth_dev *dev, uint32_t index) {
...
static uint64_t nfp_net_xstats_value(const struct rte_eth_dev *dev,
  ...
  /*
   * A baseline value of each statistic counter is recorded when stats are
   * "reset". Thus, the value returned by this function need to be decremented
   * by this baseline value. The result is the count of this statistic since the
...
/* NOTE: All callers ensure dev is always set. */
int nfp_net_xstats_get(struct rte_eth_dev *dev, struct rte_eth_xstat *xstats,
                       unsigned int n) {
  uint32_t id;
  uint32_t nfp_size;
  uint32_t read_size;
 ...
 */
int nfp_net_xstats_get_names_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
                                   struct rte_eth_xstat_name *xstats_names,
                                   unsigned int size) {
  uint32_t i;
  uint32_t read_size;
  ...
  for (i = 0; i < read_size; i++) {
    ...
    rte_strlcpy(xstats_names[i].name, xstat->name, RTE_ETH_XSTATS_NAME_SIZE);
  }
 ...
 */
int nfp_net_xstats_get_by_id(struct rte_eth_dev *dev, const uint64_t *ids,
...
void nfp_net_rx_desc_limits(struct nfp_net_hw *hw, uint16_t *min_rx_desc,
                            uint16_t *max_rx_desc) {
  *max_rx_desc = hw->dev_info->max_qc_size;
  *min_rx_desc = hw->dev_info->min_qc_size;
}

void nfp_net_tx_desc_limits(struct nfp_net_hw *hw, uint16_t *min_tx_desc,
...
static void nfp_net_dev_link_status_print(struct rte_eth_dev *dev) {
  struct rte_eth_link link;
  ...
  PMD_DRV_LOG(INFO, "PCI Address: " PCI_PRI_FMT, pci_dev->addr.domain,
...
static int nfp_net_rss_reta_write(struct rte_eth_dev *dev,
  ...
  if (reta_size != NFP_NET_CFG_RSS_ITBL_SZ) {
    PMD_DRV_LOG(ERR,
                "The size of hash lookup table configured (%hu)"
...
int nfp_net_reta_query(struct rte_eth_dev *dev,
  ...
  for (i = 0; i < reta_size; i += 4) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      reta_conf[idx].reta[shift + j] = (uint8_t)((reta >> (8 * j)) & 0xF);
    }
  }
...
int nfp_net_set_vxlan_port(struct nfp_net_hw *net_hw, size_t idx,
                           uint16_t port) {
  int ret;
  uint32_t i;
...
int nfp_net_txrwb_alloc(struct rte_eth_dev *eth_dev) {
  ...
  net_hw->txrwb_mz = rte_memzone_reserve_aligned(
      mz_name, net_hw->max_tx_queues * sizeof(uint64_t), rte_socket_id(),
      RTE_MEMZONE_IOVA_CONTIG, RTE_CACHE_LINE_SIZE);
  if (net_hw->txrwb_mz == NULL) {
    PMD_INIT_LOG(ERR, "Failed to alloc %s for TX ring write back", mz_name);
    return -ENOMEM;
  }
...
static void nfp_net_get_nsp_info(struct nfp_net_hw *hw, char *nsp_version) {
  struct nfp_nsp *nsp;
  ...
  snprintf(nsp_version, FW_VER_LEN, "%hu.%hu", nfp_nsp_get_abi_ver_major(nsp),
           nfp_nsp_get_abi_ver_minor(nsp));

  nfp_nsp_close(nsp);
}

void nfp_net_get_fw_version(struct nfp_net_hw *hw, uint32_t *mip_version) {
  struct nfp_mip *mip;
...
static void nfp_net_get_mip_name(struct nfp_net_hw *hw, char *mip_name) {
  struct nfp_mip *mip;
...
static void nfp_net_get_app_name(struct nfp_net_hw *hw, char *app_name) {
...
int nfp_net_firmware_version_get(struct rte_eth_dev *dev, char *fw_version,
                                 size_t fw_size) {
  struct nfp_net_hw *hw;
  ...
  if (!rte_eth_dev_is_repr(dev)) {
    snprintf(vnic_version, FW_VER_LEN, "%d.%d.%d.%d", hw->ver.extend,
             hw->ver.class, hw->ver.major, hw->ver.minor);
  } else {
  ...
  snprintf(fw_version, FW_VER_LEN, "%s %s %s %s", vnic_version, nsp_version,
           mip_name, app_name);
...
static int nfp_net_pause_frame_set(struct nfp_net_hw *net_hw,
                                   struct nfp_eth_table_port *eth_port,
                                   enum rte_eth_fc_mode mode) {
  int err;
  bool flag;
...
int nfp_net_flow_ctrl_set(struct rte_eth_dev *dev,
  ...
  eth_port->tx_pause_enabled =
      (set_mode & RTE_ETH_FC_TX_PAUSE) == 0 ? false : true;
  eth_port->rx_pause_enabled =
      (set_mode & RTE_ETH_FC_RX_PAUSE) == 0 ? false : true;
...
static uint32_t nfp_net_fec_nfp_to_rte(enum nfp_eth_fec fec) {
  switch (fec) {
...
int nfp_net_fec_get(struct rte_eth_dev *dev, uint32_t *fec_capa) {
  struct nfp_net_hw *hw;
  struct nfp_eth_table *nfp_eth_table;
  struct nfp_eth_table_port *eth_port;
...
static enum nfp_eth_fec nfp_net_fec_rte_to_nfp(uint32_t fec) {
  switch (fec) {
...
int nfp_net_fec_set(struct rte_eth_dev *dev, uint32_t fec_capa) {
